...
def _ParseReservationIdentifier(identifier):
  ...
  """,
    ...
    match = re.search(pattern, identifier)
    if not match:
        raise BigqueryError("Could not parse reservation identifier: %s" % identifier)
...
def _ParseReservationAssignmentPath(path):
  ...
  """,
    ...
    match = re.search(pattern, path)
    if not match:
        raise BigqueryError("Could not parse reservation assignment path: %s" % path)
...
def _ParseConnectionIdentifier(identifier):
    ...
    if num_tokens > 4:
        raise BigqueryError("Could not parse connection identifier: %s" % identifier)
...
class BigqueryClient(object):
    ...
    def BuildDiscoveryNextApiClient(self):
        ...
        try:
            models_discovery_document = pkgutil.get_data(
                "bigquery_client", "discovery_next/bigquery.json"
            )
        except IOError:
        ...
        try:
            ...
            logging.error(
                "Error building from models document: %s", models_discovery_document
            )
            raise
    ...
    def BuildIAMPolicyApiClient(self):
        ...
        try:
            iam_policy_discovery_document = pkgutil.get_data(
                "bigquery_client", "discovery_next/iam-policy.json"
            )
        except IOError:
        ...
        try:
            ...
            logging.error(
                "Error building from iam policy document: %s",
                iam_policy_discovery_document,
            )
            raise
    ...
    def GetTransferV1ApiClient(self, transferserver_address=None):
        ...
        if not self._op_transfer_client:
            ...
            self._op_transfer_client = self.BuildApiClient(discovery_url=discovery_url)
    ...
    def ValidatePrintFormat(print_format):
        if print_format not in ["show", "list", "view", "materialized_view", "make"]:
    ...
    def GetProjectReference(self, identifier=""):
        ...
        project_id, dataset_id, table_id = BigqueryClient._ParseIdentifier(identifier)
        try:
    ...
    def GetRoutineReference(self, identifier=""):
        ...
        try:
            ...
            raise BigqueryError("Cannot determine routine described by %s" % identifier)

    def GetQueryDefaultDataset(self, identifier):
        parsed_project_id, parsed_dataset_id = self._ParseDatasetIdentifier(identifier)
    ...
    def GetReservationReference(
        self, identifier=None, default_location=None, default_reservation_id=None
    ):
    ...
    def GetBodyForCreateReservation(
        ...
        if autoscale_budget_slot_hours is not None:
            if self.api_version != "autoscale_preview":
                raise BigqueryError(
                    "autoscale_budget_slot_hours is only supported in "
                    "autoscale_preview. Please specify '--api_version="
                    "autoscale_preview' and retry."
                )
            reservation["autoscale"] = {}
            reservation["autoscale"]["budget_slot_hours"] = autoscale_budget_slot_hours
    ...
    def UpdateBiReservation(self, reference, reservation_size):
        ...
        if reservation_size.upper().endswith("GB") and reservation_size[:-2].isdigit():
            ...
            raise ValueError(
                """Invalid reservation size. The unit for BI reservations
    ...
    def CreateReservationAssignment(
        self, reference, job_type, priority, assignee_type, assignee_id
    ):
    ...
    def MoveReservationAssignment(
        self, reference, destination_reservation_id, default_location
    ):
        ...
        destination_reservation_reference = self.GetReservationReference(
            identifier=destination_reservation_id, default_location=default_location
        )
        client = self.GetReservationApiClient()
    ...
    def SearchReservationAssignments(
        self, location, job_type, assignee_type, assignee_id
    ):
    ...
    def CreateConnection(
        ...
        if property_name:
            ...
            if connection_credential:
                connection[property_name]["credential"] = json.loads(
    ...
    def UpdateConnection(
        ...
        return (
            ...
            .connections()
            .patch(
                name=reference.path(), updateMask=",".join(update_mask), body=connection
            )
            .execute()
        )
    ...
    def ReadSchemaAndRows(
        self, table_dict, start_row=None, max_rows=None, selected_fields=None
    ):
        ...
        table_reader = _TableTableReader(
    ...
    def ReadSchemaAndJobRows(
        ...
        if flags.FLAGS.jobs_query_use_results_from_response and result_first_page:
            reader = _QueryTableReader(
                self.apiclient, self.max_rows_per_request, job_ref, result_first_page
            )
        else:
            reader = _JobTableReader(self.apiclient, self.max_rows_per_request, job_ref)
    ...
    def FormatInfoByType(object_info, object_type):
        ...
        elif issubclass(object_type, ApiClientHelper.ReservationAssignmentReference):
    ...
    def FormatRoutineDataType(data_type):
        ...
        if type_kind == "ARRAY":
            ...
            struct_fields = [
                "{} {}".format(
                    field["name"], BigqueryClient.FormatRoutineDataType(field["type"])
                )
    ...
    def FormatRoutineTableType(table_type):
        ...
        columns = [
            "{} {}".format(
                column["name"], BigqueryClient.FormatRoutineDataType(column["type"])
            )
    ...
    def FormatTableInfo(table_info):
        ...
        if "type" in result:
            ...
            if "materializedView" in result and "query" in result["materializedView"]:
                ...
                if "enableRefresh" in result["materializedView"]:
                    result["Enable Refresh"] = result["materializedView"][
                        "enableRefresh"
                    ]
        ...
        if "snapshotDefinition" in result:
            ...
            result["Snapshot TimeStamp"] = result["snapshotDefinition"]["snapshotTime"]
        return result
    ...
    def FormatReservationInfo(reservation, reference_type):
        ...
        if (
            reference_type == ApiClientHelper.BetaReservationReference
            and "maxConcurrency" not in list(result.keys())
        ):
            result["maxConcurrency"] = "0 (auto)"
        if (
            reference_type == ApiClientHelper.BetaReservationReference
            and "enableQueuingAndPriorities" not in list(result.keys())
        ):
            result["enableQueuingAndPriorities"] = "False"
        if (
            reference_type == ApiClientHelper.AutoscaleAlphaReservationReference
            ...
            if "maxSlots" in result["autoscale"]:
                ...
                if "currentSlots" in result["autoscale"]:
                    result["autoscaleCurrentSlots"] = result["autoscale"][
                        "currentSlots"
                    ]
            # The original 'autoscale' fields is not needed anymore now.
            result.pop("autoscale", None)
        if (
            reference_type == ApiClientHelper.AutoscalePreviewReservationReference
            and "autoscale" in list(result.keys())
        ):
        ...
        for key, value in six.iteritems(capacity_commitment):
            if key == "name":
                (
                    ...
                    location,
                    capacity_commitment_id,
                ) = _ParseCapacityCommitmentPath(value)
    ...
    def _PrepareTransferListRequest(
        self, reference, location, page_size=None, page_token=None, data_source_ids=None
    ):
        """Create and populate a list request."""
        request = dict(parent=_FormatProjectIdentifierForTransfers(reference, location))
    ...
    def _PrepareTransferRunListRequest(
        self, reference, run_attempt, max_results=None, page_token=None, states=None
    ):
    ...
    def ListTransferConfigs(
        ...
        client = self.GetTransferV1ApiClient()
        _Typecheck(
            reference, ApiClientHelper.ProjectReference, method="ListTransferConfigs"
        )
        if page_size is not None:
    ...
    def ListTransferRuns(
        ...
        transfer_client = self.GetTransferV1ApiClient()
        _Typecheck(
            reference, ApiClientHelper.TransferRunReference, method="ListTransferRuns"
        )
        reference = str(reference)
        request = self._PrepareTransferRunListRequest(
    ...
    def ListTransferLogs(
        self, reference, message_type=None, max_results=None, page_token=None
    ):
    ...
    def ListDatasets(
        ...
        reference = self._NormalizeProjectReference(reference)
        _Typecheck(reference, ApiClientHelper.ProjectReference, method="ListDatasets")
    ...
    def ListRowAccessPoliciesWithGrantees(self, table_reference, page_size, page_token):
    ...
    def GetDatasetIAMPolicy(self, reference):
        ...
        """
        _Typecheck(
            reference, ApiClientHelper.DatasetReference, method="GetDatasetIAMPolicy"
        )
    ...
    def SetDatasetIAMPolicy(self, reference, policy):
        ...
        """
        _Typecheck(
            reference, ApiClientHelper.DatasetReference, method="SetDatasetIAMPolicy"
        )
        formatted_resource = "projects/%s/datasets/%s" % (
            reference.projectId,
            reference.datasetId,
    ...
    def CopyTable(
        ...
        **kwds
        ...
        _Typecheck(dest_reference, ApiClientHelper.TableReference, method="CopyTable")
        copy_config = {
    ...
    def DatasetExists(self, reference):
        _Typecheck(reference, ApiClientHelper.DatasetReference, method="DatasetExists")
        try:
    ...
    def RoutineExists(self, reference):
        _Typecheck(reference, ApiClientHelper.RoutineReference, method="RoutineExists")
        try:
            return (
                self.GetRoutinesApiClient()
                .routines()
    ...
    def TransferExists(self, reference):
        ...
        _Typecheck(
            reference, ApiClientHelper.TransferConfigReference, method="TransferExists"
        )
        try:
            transfer_client = self.GetTransferV1ApiClient()
    ...
    def CreateDataset(
        ...
        Raises:
          ...
          BigqueryDuplicateError: if reference exists and ignore_existing
             is False.
        """
        _Typecheck(reference, ApiClientHelper.DatasetReference, method="CreateDataset")
        ...
        if source_dataset_reference is not None:
            ...
            body["linkedDatasetSource"] = {
                "sourceDataset": BigqueryClient.ConstructObjectInfo(
                    source_dataset_reference
                )["datasetReference"]
            }
    ...
    def CreateTransferConfig(
        ...
        data_sources_reference = reference + "/locations/-/dataSources/" + data_source
        data_source_info = (
            transfer_client.projects()
            .locations()
            .dataSources()
            .get(name=data_sources_reference)
            .execute()
        )
    ...
    def UpdateDataset(
        ...
        Raises:
          TypeError: if reference is not a DatasetReference.
        """
        _Typecheck(reference, ApiClientHelper.DatasetReference, method="UpdateDataset")
    ...
    def DeleteDataset(self, reference, ignore_not_found=False, delete_contents=None):
        ...
        """
        _Typecheck(reference, ApiClientHelper.DatasetReference, method="DeleteDataset")
    ...
    def DeleteRoutine(self, reference, ignore_not_found=False):
        ...
        Raises:
          ...
          BigqueryNotFoundError: if reference does not exist and
            ignore_not_found is False.
        """
        _Typecheck(reference, ApiClientHelper.RoutineReference, method="DeleteRoutine")
        try:
    ...
    def WaitJob(
        self, job_reference, status="DONE", wait=sys.maxsize, wait_printer_factory=None
    ):
                ...
                if job_reference is None:
                    ...
                    if self.max_rows_per_request is not None:
                        if rows_to_read is None:
                            ...
                            rows_to_read = min(
                                self.max_rows_per_request, int(rows_to_read)
                            )
                    ...
                    if dry_run:
                        ...
                        if "schema" in result:
                            execution["statistics"]["query"]["schema"] = result[
                                "schema"
                            ]
                        return ([], [], execution)
                    job_reference = ApiClientHelper.JobReference.Create(
...
class _TableReader(object):
    ...
    def _ReadOnePage(self, start_row, max_rows, page_token=None, selected_fields=None):
    ...
    def _ReadOnePage(self, start_row, max_rows, page_token=None, selected_fields=None):
        ...
        if (
            ...
            if len(result_rows) < int(total_rows) and page_token is None:
                raise BigqueryError(
                    "Synchronous query %s did not return all rows, "
...
class ApiClientHelper(object):
    ...
    class ReservationAssignmentReference(Reference):
        ...
        _format_str = (
            "%(projectId)s:%(location)s.%(reservationId)s.%(reservationAssignmentId)s"
        )
    ...
    class ConnectionReference(Reference):
        ...
        _path_str = (
            "projects/%(projectId)s/locations/%(location)s/connections/%(connectionId)s"
        )
        typename = "connection"
