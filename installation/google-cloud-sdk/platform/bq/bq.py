...
TransferConfigReference = bigquery_client.ApiClientHelper.TransferConfigReference
...
RowAccessPolicyReference = bigquery_client.ApiClientHelper.RowAccessPolicyReference
EncryptionServiceAccount = bigquery_client.ApiClientHelper.EncryptionServiceAccount
...
AutoscaleAlphaReservationReference = (
    bigquery_client.ApiClientHelper.AutoscaleAlphaReservationReference
)
AutoscalePreviewReservationReference = (
    bigquery_client.ApiClientHelper.AutoscalePreviewReservationReference
)
BetaReservationReference = bigquery_client.ApiClientHelper.BetaReservationReference
CapacityCommitmentReference = (
    bigquery_client.ApiClientHelper.CapacityCommitmentReference
)  # pylint: disable=line-too-long
ReservationAssignmentReference = (
    bigquery_client.ApiClientHelper.ReservationAssignmentReference
)  # pylint: disable=line-too-long
BetaReservationAssignmentReference = (
    bigquery_client.ApiClientHelper.BetaReservationAssignmentReference
)  # pylint: disable=line-too-long
    ...
    def _GetClientCacheKey(cls, **kwds):
        ...
        return (
            ...
            "apilog={apilog},".format(
                client_args=client_args,
                service_account_credential_file=FLAGS.service_account_credential_file,
                apilog=FLAGS.apilog,
            )
        )
        ...
        try:
            ...
            for argname in self._full_arg_list[: self._min_args]:
                ...
                if flag is not None and flag.present:
                    ...
                    print(
                        "Not enough positional args, still looking for %s" % (argname,)
                    )
                    if self.usage:
    ...
    def RunDebug(self, args, kwds):
        ...
        # pylint: disable=broad-except
            ...
            if (
                ...
                or (
                    isinstance(e, bigquery_client.BigqueryError)
...
class _Load(BigqueryCmd):
    ...
    def __init__(self, name, fv):
        ...
        flags.DEFINE_boolean(
            "allow_jagged_rows",
            None,
            "Whether to allow missing trailing optional columns " "in CSV import data.",
            flag_values=fv,
        )
        flags.DEFINE_boolean(
        ...
        flags.DEFINE_string(
            ...
            None,
            "(experimental) Prefix after which hive partition "
...
class _MakeExternalTableDefinition(BigqueryCmd):
    ...
    def __init__(self, name, fv):
        ...
        flags.DEFINE_boolean(
            ...
            None,
            "(experimental) Whether queries against a table are required to "
...
class _Query(BigqueryCmd):
    ...
    def RunWithArgs(self, *args):
        ...
        if self.schedule:
            ...
            credentials = CheckValidCreds(reference, "scheduled_query", transfer_client)
            auth_info = {}
            if not credentials:
                auth_info = RetrieveAuthorizationInfo(
                    reference, "scheduled_query", transfer_client
                )
            schedule_args = bigquery_client.TransferScheduleArgs(schedule=self.schedule)
            params = {
                "query": query,
            }
        ...
        for (i, child_job_info) in enumerate(statement_child_jobs):
            ...
            if is_json:
                ...
                if len(stack_frames) <= 0:
                    break
                sys.stdout.write(
                    "%s; " % six.ensure_str(stack_frames[0].get("text", ""))
                )
    ...
    def PrintNonScriptQueryJobResults(self, client, job):
        ...
        if (
            ...
            and not _IsSuccessfulDmlOrDdlJob(printable_job_info)
...
class _List(BigqueryCmd):  # pylint: disable=missing-docstring
    ...
    def __init__(self, name, fv):
        ...
        flags.DEFINE_boolean("routines", False, "Show all routines.", flag_values=fv)
        flags.DEFINE_boolean(
            "row_access_policies",
            False,
    ...
    def RunWithArgs(self, identifier=""):
        ...
        if self.j:
            ...
            response = client.ListModels(
                reference=reference, max_results=self.max_results, page_token=page_token
            )
...
class _Delete(BigqueryCmd):
    ...
    def RunWithArgs(self, identifier):
        ...
        if self.t:
            ...
            formatted_identifier = _FormatDataTransferIdentifiers(client, identifier)
            reference = TransferConfigReference(transferConfigName=formatted_identifier)
        elif self.reservation:
            try:
                reference = client.GetReservationReference(
        ...
        if not self.force:
            if (
                (
                    isinstance(reference, DatasetReference)
                ...
                or (isinstance(reference, JobReference) and client.JobExists(reference))
                or (
                    isinstance(reference, ModelReference)
                    and client.ModelExists(reference)
                )
                or (
                    isinstance(reference, RoutineReference)
        ...
        if isinstance(reference, DatasetReference):
            client.DeleteDataset(
                reference, ignore_not_found=self.force, delete_contents=self.recursive
            )
...
class _Copy(BigqueryCmd):
    ...
    def RunWithArgs(self, source_tables, dest_table):
        ...
        if self.destination_kms_key:
            kwds["encryption_configuration"] = {"kmsKeyName": self.destination_kms_key}
        if self.expiration:
...
class _Make(BigqueryCmd):
    ...
    def __init__(self, name, fv):
        ...
        flags.DEFINE_integer(
            "max_concurrency", None, "Reservation maximum concurrency.", flag_values=fv
        )
        flags.DEFINE_bool(
        ...
        flags.DEFINE_boolean("connection", None, "Create a connection.", flag_values=fv)
        flags.DEFINE_enum(
            "connection_type",
            None,
        ...
        flags.DEFINE_string(
            "properties", None, "Connection properties in JSON format.", flag_values=fv
        )
        flags.DEFINE_string(
        ...
        flags.DEFINE_boolean(
            "federated_aws", True, "[Experimental] Federated identity.", flag_values=fv
        )
        flags.DEFINE_string(
    ...
    def RunWithArgs(self, identifier="", schema=""):
        ...
        if ValidateAtMostOneSelected(self.schema, self.view, self.materialized_view):
            raise app.UsageError(
        ...
        if self.t:
            ...
            try:
                object_info = client.CreateCapacityCommitment(
                    reference, self.slots, self.plan, self.renewal_plan
                )
            except BaseException as e:
                raise bigquery_client.BigqueryError(
            ...
            formatted_identifier = _FormatDataTransferIdentifiers(client, identifier)
            reference = TransferConfigReference(transferConfigName=formatted_identifier)
            incompatible_options = (self.start_time or self.end_time) and self.run_time
            incomplete_options = (
            ...
            if self.connection_type == "AWS" and self.iam_role_id:
                ...
                if not self.federated_aws:
                    raise app.UsageError(
                        "Non-federated AWS connections are deprecated."
                    )
...
"""
    ...
    def _GetTableInfosFromDataset(self, dataset_reference):
        ...
        recovery_timestamp_for_dataset_query = (
            (
                """SELECT
  TABLE_NAME,
...
FROM (
  ...
  GROUP BY
    ...
    def _GetTableInfo(self, table_reference):
        ...
        recovery_timestamp_for_table_query = (
            (
                """SELECT
  TABLE_NAME,
  UNIX_MILLIS(replicated_time_at_remote_site),
...
FROM (
  ...
  GROUP BY
    ...
    2 )"""
            ...
            % (table_reference.datasetId, table_reference.tableId)
        )
    ...
    def _TruncateTable(self, table_reference, recovery_timestamp, is_fully_replicated):
        client = Client.Get()
        kwds = {}
...
class _Update(BigqueryCmd):
    ...
    def __init__(self, name, fv):
        ...
        flags.DEFINE_boolean("connection", None, "Update connection.", flag_values=fv)
        flags.DEFINE_enum(
            "connection_type",
            None,
            bigquery_client.CONNECTION_TYPES,
...
def CheckValidCreds(reference, data_source, transfer_client):
    ...
    if FLAGS.location:
        data_source_reference = (
            reference + "/locations/" + FLAGS.location + "/dataSources/" + data_source
        )
        credentials = (
    ...
    def RunWithArgs(self, identifier=""):
        ...
        if self.j:
            ...
            object_info = (
                ...
                .getServiceAccount(projectId=client.GetProjectReference().projectId)
                .execute()
            )
...
def _PrintJobMessages(printable_job_info):
    ...
    if printable_job_info["State"] == "FAILURE":
        ...
        if project_id and dataset_id and table_id and op:
            if "DDL Affected Row Access Policy Count" in printable_job_info:
                ...
                print(
                    ...
                    % (
                        six.ensure_str(op),
                        six.ensure_str(project_id),
                        six.ensure_str(dataset_id),
                        six.ensure_str(table_id),
                    )
                )
            else:
                print(
        ...
        if project_id and dataset_id and table_id and row_access_policy_id and op:
            print(
                "{op} row access policy {policy} on table {project}.{dataset}.{table}".format(
                    op=op,
...
def _PrintObjectInfo(object_info, reference, custom_format, print_reference=True):
...
def _PrintObjectsArray(object_infos, objects_type):
    if FLAGS.format in ["prettyjson", "json"]:
        ...
        if not object_infos:
            return
        formatter = _GetFormatterFromFlags()
        BigqueryClient.ConfigureFormatter(formatter, objects_type, print_format="list")
...
class _Cancel(BigqueryCmd):
    ...
    def RunWithArgs(self, job_id=""):
        ...
        job = client.CancelJob(
            job_id=job_reference_dict["jobId"], location=job_reference_dict["location"]
        )
        _PrintObjectInfo(
...
class _Head(BigqueryCmd):
    ...
    def __init__(self, name, fv):
        ...
        flags.DEFINE_boolean(
            "table", False, "Reads rows from a table.", short_name="t", flag_values=fv
        )
        flags.DEFINE_integer(
            "start_row",
            0,
...
class _Insert(BigqueryCmd):
    ...
    def _DoInsert(
        ...
        reference = client.GetReference(identifier)
        _Typecheck(
            reference, (TableReference,), "Must provide a table identifier for insert."
        )
...
class _RemoveIamPolicyBinding(
    _IamPolicyBindingCmd
...
class CommandLoop(cmd.Cmd):
    ...
    def get_names(self):
        names = dir(self)
        commands = (
            name for name in self._commands if name not in self._special_command_names
        )
    ...
    def RunWithArgs(self):
        ...
        if not projects:
            ...
            if FLAGS.project_id:
                matching_projects = [
                    p
                    for p in map(BigqueryClient.ConstructObjectReference, projects)
                    if p.projectId == FLAGS.project_id
                ]
                if not matching_projects:
                    print(
                        "This user does not have access to project_id %s"
                        % FLAGS.project_id
                    )
                    return 1
