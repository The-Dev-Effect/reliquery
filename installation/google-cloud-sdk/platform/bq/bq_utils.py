...
def ProcessError(
    ...
    if isinstance(
        err,
        (bigquery_client.BigqueryNotFoundError, bigquery_client.BigqueryDuplicateError),
    ):
    ...
    elif isinstance(err, bigquery_client.BigqueryError) and not isinstance(
        err, bigquery_client.BigqueryInterfaceError
    ):
        response.append("BigQuery error in %s operation: %s" % (name, message))
    elif isinstance(err, (app.UsageError, TypeError)):
        response.append(message)
    elif isinstance(err, SyntaxError) or isinstance(
        err, bigquery_client.BigquerySchemaError
    ):
        ...
        if isinstance(err, bigquery_client.BigqueryInterfaceError):
            ...
            isinstance(err, six.moves.http_client.HTTPException)
