...
r"""This module is the base for programs that provide multiple commands.
...
# These globals are initialized by ResetForTest.
_cmd_argv = None  # remaining arguments with index 0 = sys.argv[0]
_cmd_list = None  # list of commands index by name (_Cmd instances)
_cmd_alias_list = None  # list of command_names index by command_alias
_cmd_default = None  # command to execute if none explicitly given
_cmd_gnugetopt = None


def ResetForTest():
    """Reset the module for test purposes; ONLY use for testing."""
    global _cmd_argv, _cmd_list, _cmd_alias_list, _cmd_default, _cmd_gnugetopt  # pylint: disable=
    _cmd_argv = None
    _cmd_list = {}
    _cmd_alias_list = {}
    _cmd_default = "help"
    ...
    if use_gnu_opt_name in os.environ:
        ...
        # By default allow mixed flag/arg ordering in subcommand argv.
...
class Cmd(object):
    ...
    def __init__(
        ...
        self._hidden = hidden
        if type(self) is Cmd:  # pylint: disable=unidiomatic-typecheck
    ...
    def Run(self, unused_argv):
        ...
        """
        raise AppCommandsError(
            "%s.%s.Run() is not implemented"
    ...
    def CommandRun(self, argv):
        ...
        def ReplacementAppUsage(
            shorthelp=0, writeto_stdout=1, detailed_error=None, exitcode=None
        ):
            AppcommandsUsage(
                shorthelp,
                writeto_stdout,
    ...
    def CommandGetHelp(self, unused_argv, cmd_names=None):
        ...
        """
        if (
            isinstance(cmd_names, list)
            and len(cmd_names) > 1
            and self._all_commands_help is not None
        ):
...
def _CheckCmdName(name_or_alias):
    ...
    if not isinstance(name_or_alias, str) or len(name_or_alias) <= 1:
        raise AppCommandsError(
            "Command '%s' not a string or too short" % str(name_or_alias)
        )
    if not name_or_alias[0].isalpha():
        raise AppCommandsError(
            "Command '%s' does not start with a letter" % name_or_alias
        )
    if [c for c in name_or_alias if not (c.isalnum() or c in ("_", "-", ":"))]:
        raise AppCommandsError(
            "Command '%s' contains non alphanumeric characters" % name_or_alias
        )
...
def AddCmdFunc(
    command_name, cmd_func, command_aliases=None, all_commands_help=None, hidden=False
):
...
def _UsageFooter(detailed_error, cmd_names):
    ...
    if not cmd_names or len(cmd_names) == len(GetCommandList()):
        footer.append(
            "Run '%s help <command>' to get help for <command>." % GetAppBasename()
        )
...
def AppcommandsUsage(
    ...
    if len(GetCommandList()) == 1:
        ...
        cmd_names = [
            cmd_name for cmd_name, cmd in GetCommandList().items() if not cmd._hidden
    ...
    for name in cmd_names:
        ...
        if cmd_help:
            stdfile.write(
                flags.text_wrap(cmd_help, flags.get_help_width(), prefix, prefix1)
            )
            stdfile.write("\n\n")
        else:
            stdfile.write("\n")
    ...
    if show_global_flags:
        ...
        stdfile.write(
            "Run '%s --help' to get help for global flags." % GetAppBasename()
        )
...
def _ReplacementAppUsage(
    shorthelp=0, writeto_stdout=0, detailed_error=None, exitcode=None
):
    AppcommandsUsage(
        shorthelp,
        writeto_stdout,
