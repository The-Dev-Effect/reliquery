...
class CustomAuthenticator(baseauthenticator.BaseAuthenticator):
    ...
    def _BuildPluginRequest(self, app_id, challenge_data, origin):
        ...
        for challenge_item in challenge_data:
            ...
            client_data_json = model.ClientData(
                model.ClientData.TYP_AUTHENTICATION, raw_challenge, origin
            ).GetJson()
    ...
    def _CallPlugin(self, cmd, input_json):
        ...
        if response_len != len(response):
            raise errors.PluginError(
                "Plugin response length {} does not match data {} (exit_status={})".format(
                    response_len, len(response), exit_status
                )
            )
        ...
        try:
            ...
            raise errors.PluginError(
                "Plugin returned invalid output (exit_status={})".format(exit_status)
            )

        # Ensure response type
        if json_response.get("type") != "sign_helper_reply":
            raise errors.PluginError(
                "Plugin returned invalid response type "
        ...
        if result_code is None:
            raise errors.PluginError(
                "Plugin missing result code (exit_status={})".format(exit_status)
            )
        ...
        if result_code == SK_SIGNING_PLUGIN_TOUCH_REQUIRED:
            ...
            raise errors.PluginError(
                "Plugin failed with error {} - {} (exit_status={})".format(
                    result_code, json_response.get("errorDetail"), exit_status
                )
            )
        ...
        if response_data is None:
            raise errors.PluginErrors(
                "Plugin returned output with missing responseData (exit_status={})".format(
                    exit_status
                )
            )
