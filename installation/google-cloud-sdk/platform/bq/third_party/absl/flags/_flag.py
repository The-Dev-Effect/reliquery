...
class Flag(object):
    ...
    def __copy__(self):
        raise TypeError(
            "%s does not support shallow copies. "
...
class BooleanFlag(Flag):
    ...
    def __init__(
        self, name, default, help, short_name=None, **args
...
class MultiFlag(Flag):
    ...
    def _parse(self, arguments):
        if isinstance(arguments, abc.Iterable) and not isinstance(
            arguments, six.string_types
        ):
...
class MultiEnumClassFlag(MultiFlag):
    ...
    def __init__(
        self, name, default, help_string, enum_class, case_sensitive=False, **args
    ):
        p = _argument_parser.EnumClassParser(enum_class, case_sensitive=case_sensitive)
