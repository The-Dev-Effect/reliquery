...
def _is_integer_type(instance):
    ...
    return isinstance(instance, six.integer_types) and not isinstance(instance, bool)
...
class FloatParser(NumericParser):
    ...
    def convert(self, argument):
        ...
        if (
            _is_integer_type(argument)
            or isinstance(argument, float)
            or isinstance(argument, six.string_types)
        ):
...
class IntegerParser(NumericParser):
    ...
    def convert(self, argument):
        ...
        if _is_integer_type(argument):
            ...
            raise TypeError(
                "Expect argument to be a string or int, found {}".format(type(argument))
            )
...
class EnumClassParser(ArgumentParser):
    ...
    def parse(self, argument):
        ...
        key = EnumParser(self._member_names, case_sensitive=self._case_sensitive).parse(
            argument
        )
...
class WhitespaceSeparatedListParser(BaseListParser):
    ...
    def _custom_xml_dom_elements(self, doc):
        elements = super(WhitespaceSeparatedListParser, self)._custom_xml_dom_elements(
            doc
        )
