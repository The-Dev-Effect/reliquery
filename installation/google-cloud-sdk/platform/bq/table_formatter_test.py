...
class TableFormatterTest(googletest.TestCase):
    def setUp(self):
        ...
        if type(self) != TableFormatterTest:
            ...
            self.formatter.AddColumns(
                ("foo", "longer header"), kwdss=[{}, {"align": "r"}]
            )
...
class PrettyFormatterTest(TableFormatterTest):
    ...
    def testCenteredPadding(self):
        ...
        self.assertRaises(
            table_formatter.FormatterException, self.PF.CenteredPadding, 1, 5
        )
    ...
    def testFormatCell(self):
        ...
        self.assertRaises(
            table_formatter.FormatterException, self.PF.FormatCell, "ab\na", 5
        )
    ...
    def testFormatRow(self):
        ...
        self.assertEqual(
            ["| a   | b   |"], list(formatter.FormatRow(formatter.rows[0], 1))
        )
        formatter.AddRow(["a", "b\nc"])
        self.assertEqual(
            [
        ...
        self.assertRaises(
            table_formatter.FormatterException,
            formatter.FormatRow,
            formatter.rows[1],
            1,
        )
    ...
    def testHeaderLines(self):
        ...
        self.assertEqual(["|         a          | b |"], list(formatter.HeaderLines()))
    ...
    def testAddColumn(self):
        ...
        self.assertRaises(
            table_formatter.FormatterException, formatter.AddColumn, "bad", align="d"
        )
        formatter.AddRow([3])
        self.assertRaises(
            table_formatter.FormatterException, formatter.AddColumn, "anything"
        )
...
class SparsePrettyFormatterTest(TableFormatterTest):
    ...
    def testPrintEmptyTable(self):
        formatter = table_formatter.SparsePrettyFormatter(skip_header_when_empty=False)
        formatter.AddColumns(("a", "b"))
        table_repr = "\n".join(("  a   b  ", " --- --- "))
        self.assertEqual(table_repr, str(formatter))
        self.assertEqual("", self.wrap_print(formatter))
        ...
        formatter = table_formatter.SparsePrettyFormatter(skip_header_when_empty=True)
        formatter.AddColumns(("a", "b"))
        self.assertEqual("", str(formatter))
        self.assertEqual("", self.wrap_print(formatter))
...
class CsvFormatterTest(TableFormatterTest):
    ...
    def testStr(self):
        table_repr = "\n".join(("foo,longer header", "a,3", "abc,123"))
        self.assertEqual(table_repr, str(self.formatter))
