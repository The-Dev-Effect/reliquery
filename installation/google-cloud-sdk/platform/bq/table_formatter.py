...
class TableFormatter(object):
    ...
    def _EncodedStr(self, encoding):
        if six.PY2:
            ...
            return (
                self.__unicode__().encode(encoding, "backslashreplace").decode(encoding)
            )
...
class PrettyFormatter(TableFormatter):
    ...
    def FormatCell(entry, cell_width, cell_height=1, align="c", valign="t"):
        ...
        """
        entry_lines = [
            PrettyFormatter.Abbreviate(line, cell_width) for line in entry.split("\n")
        ]
        ...
        if valign == "t":
            top_lines = []
            bottom_lines = itertools.repeat(
                " " * (cell_width + 2), cell_height - len(entry_lines)
            )
        elif valign == "c":
        ...
        for line in entry_lines:
            if align == "c":
                ...
                content_lines.append(
                    " %s%s%s " % (" " * left_padding, line, " " * right_padding)
                )
            elif align in ("l", "r"):
    ...
    def FormatRow(
        self, entries, row_height, column_alignments=None, column_widths=None
    ):
        ...
        return (
            self.vertical_char.join(itertools.chain([""], cells, [""]))
    ...
    def HeaderLines(self):
        ...
        return self.FormatRow(
            self.column_names, self.header_height, column_alignments=aligns
        )

    def FormatHrule(self):
        """Return a list containing an hrule for this table."""
        entries = (
            "".join(itertools.repeat("-", width + 2)) for width in self.column_widths
        )
...
class CsvFormatter(TableFormatter):
    ...
    def AddRow(self, row):
        if six.PY2:
            row = [
                six.text_type(entry).encode("utf8", "backslashreplace") for entry in row
            ]
