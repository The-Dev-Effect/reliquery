...
class ApplicationDefaultCredentialFileLoader(CachedCredentialLoader):
    ...
    def _Load(self):
        ...
        if credentials["type"] == oauth2client_4_0.client.AUTHORIZED_USER:
            ...
            service_account_credentials._user_agent = (
                _CLIENT_USER_AGENT  # pylint: disable=protected-access
            )
...
def GetCredentialsFromFlags():
    ...
    if (
        type(credentials) == oauth2client_4_0.client.OAuth2Credentials
...
def _GetReauthCredentials(oauth2_creds):
    reauth_creds = Oauth2WithReauthCredentials.from_OAuth2Credentials(oauth2_creds)
