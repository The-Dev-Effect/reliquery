...
class ProgressCallbackWithTimeout(object):
    ...
    def Progress(self, bytes_processed):
        ...
        cur_time = time.time()
        if (
            self._bytes_processed_since_callback > self._bytes_per_callback
            or (
                self._total_size is not None
                and self._total_bytes_processed + self._bytes_processed_since_callback
                >= self._total_size
            )
            or (self._last_time - cur_time) > self._timeout
        ):
...
class FileProgressCallbackHandler(object):
    ...
    def call(self, last_byte_processed, total_size):  # pylint: disable=invalid-name
