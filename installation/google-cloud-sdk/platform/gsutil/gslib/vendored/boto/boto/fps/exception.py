...
class ResponseError(BotoServerError):
    ...
    def __repr__(self):
        return "{0}({1}, {2},\n\t{3})".format(
            self.__class__.__name__, self.status, self.reason, self.error_message
        )

    def __str__(self):
        return (
            "FPS Response Error: {0.status} {0.__class__.__name__} {1}\n"
            "{2}\n"
            "{0.error_message}".format(
                self, self.retry and "(Retriable)" or "", self.__doc__.strip()
            )
        )
