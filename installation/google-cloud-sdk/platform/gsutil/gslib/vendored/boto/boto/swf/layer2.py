...
DEFAULT_CREDENTIALS = {"aws_access_key_id": None, "aws_secret_access_key": None}
...
class SWFBase(object):
    ...
    def __init__(self, **kwargs):
        ...
        self._swf = Layer1(
            self.aws_access_key_id, self.aws_secret_access_key, region=self.region
        )
...
class Decider(Actor):
    ...
    def complete(self, task_token=None, decisions=None, **kwargs):
        ...
        if task_token is None:
            task_token = self.last_tasktoken
        return self._swf.respond_decision_task_completed(
...
class WorkflowType(SWFBase):
    ...
    def start(self, **kwargs):
        ...
        run_id = self._swf.start_workflow_execution(
            self.domain, workflow_id, self.name, self.version, **kwargs
        )["runId"]
...
class WorkflowExecution(SWFBase):
    ...
    def signal(self, signame, **kwargs):
        ...
        self._swf.signal_workflow_execution(
            self.domain, signame, self.workflowId, **kwargs
        )
    ...
    def terminate(self, **kwargs):
        ...
        return self._swf.terminate_workflow_execution(
            self.domain, self.workflowId, **kwargs
        )
    ...
    def history(self, **kwargs):
        ...
        return self._swf.get_workflow_execution_history(
            self.domain, self.runId, self.workflowId, **kwargs
        )["events"]

    @wraps(Layer1.describe_workflow_execution)
    def describe(self):
        """DescribeWorkflowExecution."""
        return self._swf.describe_workflow_execution(
            self.domain, self.runId, self.workflowId
        )
    ...
    def request_cancel(self):
        ...
        return self._swf.request_cancel_workflow_execution(
            self.domain, self.workflowId, self.runId
        )
