...
BOTO_SWF_UNITTEST_DOMAIN = os.environ.get(
    "BOTO_SWF_UNITTEST_DOMAIN", "boto-swf-unittest-domain"
)
    ...
    def setUp(self):
        ...
        try:
            r = self.conn.register_workflow_type(
                ...
                default_execution_start_to_close_timeout=self._default_execution_start_to_close_timeout,
                default_task_start_to_close_timeout=self._default_task_start_to_close_timeout,
        ...
        try:
            r = self.conn.register_activity_type(
                ...
                task_list=self._task_list,
                default_task_heartbeat_timeout=self._default_task_heartbeat_timeout,
                default_task_schedule_to_close_timeout=self._default_task_schedule_to_close_timeout,
                default_task_schedule_to_start_timeout=self._default_task_schedule_to_start_timeout,
                default_task_start_to_close_timeout=self._default_task_start_to_close_timeout,
...
class SimpleWorkflowLayer1Test(SimpleWorkflowLayer1TestBase):
    def test_list_domains(self):
        ...
        self.assertEqual(
            found["status"], "REGISTERED", "list_domains; status does not match"
        )
    ...
    def test_list_workflow_types(self):
        ...
        for info in r["typeInfos"]:
            if (
                info["workflowType"]["name"] == self._workflow_type_name
                and info["workflowType"]["version"] == self._workflow_type_version
            ):
                found = info
                break
        ...
        self.assertEqual(
            found["status"], "REGISTERED", "list_workflow_types; status does not match"
        )
    ...
    def test_list_activity_types(self):
        ...
        self.assertEqual(
            found["status"], "REGISTERED", "list_activity_types; status does not match"
        )
    ...
    def test_list_closed_workflow_executions(self):
        ...
        self.conn.list_closed_workflow_executions(
            self._domain, start_latest_date=latest_date, start_oldest_date=oldest_date
        )
        # With closeTimeFilter...
        self.conn.list_closed_workflow_executions(
            self._domain, close_latest_date=latest_date, close_oldest_date=oldest_date
        )
    ...
    def test_list_open_workflow_executions(self):
        ...
        self.conn.list_closed_workflow_executions(
            self._domain, latest_date, oldest_date
        )
        # With tagFilter...
        self.conn.list_closed_workflow_executions(
            self._domain, latest_date, oldest_date, tag="ig"
        )
        # With executionFilter...
        self.conn.list_closed_workflow_executions(
            self._domain, latest_date, oldest_date, workflow_id="ig"
        )
        # With typeFilter...
        self.conn.list_closed_workflow_executions(
            self._domain,
        ...
        # With reverseOrder...
        self.conn.list_closed_workflow_executions(
            self._domain, latest_date, oldest_date, reverse_order=True
        )
