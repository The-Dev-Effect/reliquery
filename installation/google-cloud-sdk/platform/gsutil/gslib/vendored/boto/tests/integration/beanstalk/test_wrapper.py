...
class TestApplicationObjects(BasicSuite):
    def create_application(self):
        ...
        self.addCleanup(
            partial(self.beanstalk.delete_application, application_name=self.app_name)
        )
    ...
    def test_create_delete_application_version(self):
        ...
        self.assertEqual(
            version_result.application_version.version_label, self.app_version
        )
    ...
    def test_create_configuration_template(self):
        ...
        self.assertEqual(
            result.solution_stack_name, "32bit Amazon Linux running Tomcat 6"
        )
...
class TestsWithEnvironment(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.beanstalk.create_environment(
            cls.app_name, cls.environment, template_name=cls.template
        )
        cls.wait_for_env(cls.environment)

    @classmethod
    def tearDownClass(cls):
        cls.beanstalk.delete_application(
            application_name=cls.app_name, terminate_env_by_force=True
        )
