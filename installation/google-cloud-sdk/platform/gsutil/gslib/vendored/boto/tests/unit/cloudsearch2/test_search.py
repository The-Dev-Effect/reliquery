...
class CloudSearchSearchBaseTest(unittest.TestCase):
    ...
    hits = [
        {"id": "12341", "fields": {"title": "Document 1", "rank": 1}},
        {"id": "12342", "fields": {"title": "Document 2", "rank": 2}},
        {"id": "12343", "fields": {"title": "Document 3", "rank": 3}},
        {"id": "12344", "fields": {"title": "Document 4", "rank": 4}},
        {"id": "12345", "fields": {"title": "Document 5", "rank": 5}},
        {"id": "12346", "fields": {"title": "Document 6", "rank": 6}},
        {"id": "12347", "fields": {"title": "Document 7", "rank": 7}},
    ]
...
class CloudSearchSearchTest(CloudSearchSearchBaseTest):
    response = {
        "rank": "-text_relevance",
        "match-expr": "Test",
        "hits": {"found": 30, "start": 0, "hit": CloudSearchSearchBaseTest.hits},
        "status": {
    ...
    def test_cloudsearch_facet_sort_multiple(self):
        ...
        search.search(
            q="Test", facet={"author": {"sort": "alpha"}, "cat": {"sort": "count"}}
        )
...
class CloudSearchConnectionTest(AWSMockServiceTestCase):
    ...
    def test_expose_additional_error_info(self):
        ...
        fake.content = json.dumps({"error": "Something went wrong. Oops."}).encode(
            "utf-8"
        )
    ...
    def test_proxy(self):
        ...
        self.assertEqual(
            search.session.proxies, {"http": "http://john.doe:p4ssw0rd@127.0.0.1:8180"}
        )
