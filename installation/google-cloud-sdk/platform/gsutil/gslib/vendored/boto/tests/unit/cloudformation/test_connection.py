...
class TestCloudFormationCreateStack(CloudFormationConnectionBase):
    ...
    def test_create_stack_fails(self):
        ...
        with self.assertRaisesRegexp(
            self.service_connection.ResponseError, "Invalid arg."
        ):
            api_response = self.service_connection.create_stack(
                "stack_name",
                template_body=SAMPLE_TEMPLATE,
...
class TestCloudFormationUpdateStack(CloudFormationConnectionBase):
    ...
    def test_update_stack_fails(self):
        self.set_http_response(
            status_code=400, reason="Bad Request", body=b"Invalid arg."
        )
...
class TestCloudFormationDeleteStack(CloudFormationConnectionBase):
    def default_body(self):
        return json.dumps(
            {u"DeleteStackResponse": {u"ResponseMetadata": {u"RequestId": u"1"}}}
        ).encode("utf-8")
...
class TestCloudFormationGetStackevents(CloudFormationConnectionBase):
    ...
    def test_describe_stack_events(self):
        self.set_http_response(status_code=200)
        first, second = self.service_connection.describe_stack_events(
            "stack_name", next_token="next_token"
        )
...
class TestCloudFormationDescribeStacks(CloudFormationConnectionBase):
    ...
    def test_describe_stacks(self):
        ...
        self.assertEqual(
            stack.notification_arns[0].value,
...
class TestCloudFormationListStackResources(CloudFormationConnectionBase):
    ...
    def test_list_stack_resources(self):
        self.set_http_response(status_code=200)
        resources = self.service_connection.list_stack_resources(
            "MyStack", next_token="next_token"
        )
        self.assertEqual(len(resources), 2)
        self.assertEqual(
            resources[0].last_updated_time, datetime(2011, 6, 21, 20, 25, 57)
        )
        ...
        self.assertEqual(
            resources[1].last_updated_time, datetime(2011, 6, 21, 20, 29, 23)
        )
...
class TestCloudFormationListStacks(CloudFormationConnectionBase):
    ...
    def test_list_stacks(self):
        self.set_http_response(status_code=200)
        stacks = self.service_connection.list_stacks(
            ["CREATE_IN_PROGRESS"], next_token="next_token"
        )
        self.assertEqual(len(stacks), 1)
...
class TestCloudFormationValidateTemplate(CloudFormationConnectionBase):
    ...
    def test_validate_template(self):
        self.set_http_response(status_code=200)
        template = self.service_connection.validate_template(
            template_body=SAMPLE_TEMPLATE, template_url="http://url"
        )
...
class TestCloudFormationSetStackPolicy(CloudFormationConnectionBase):
    ...
    def test_set_stack_policy(self):
        self.set_http_response(status_code=200)
        api_response = self.service_connection.set_stack_policy(
            "stack-id", stack_policy_body="{}"
        )
        self.assertDictEqual(
            api_response, {"SetStackPolicyResult": {"Some": "content"}}
        )
        self.assert_request_parameters(
            {
