...
class TestActors(unittest.TestCase):
    def setUp(self):
        ...
        self.worker = ActivityWorker(
            name="test-worker", domain="test", task_list="test_list"
        )
    ...
    def test_decider_pass_tasktoken(self):
        self.decider._swf.poll_for_decision_task.return_value = {
            ...
            "workflowExecution": {
                "runId": "fwr243dsa324132jmflkfu0943tr09=",
        ...
        self.decider._swf.respond_decision_task_completed.assert_called_with(
            "my_specific_task_token", None
        )
    ...
    def test_worker_pass_tasktoken(self):
        ...
        self.worker._swf.respond_activity_task_canceled.assert_called_with(
            task_token, "Cancelling!"
        )
        self.worker._swf.respond_activity_task_completed.assert_called_with(
            task_token, "Done!"
        )
        self.worker._swf.respond_activity_task_failed.assert_called_with(
            task_token, None, "Failure!"
        )
        self.worker._swf.record_activity_task_heartbeat.assert_called_with(
            task_token, None
        )
