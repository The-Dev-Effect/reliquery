...
class SchemaFieldsTestCase(unittest.TestCase):
    def test_hash_key(self):
        ...
        self.assertEqual(
            hash_key.definition(), {"AttributeName": "hello", "AttributeType": "S"}
        )
        self.assertEqual(
            hash_key.schema(), {"AttributeName": "hello", "KeyType": "HASH"}
        )
    ...
    def test_range_key(self):
        ...
        self.assertEqual(
            range_key.definition(), {"AttributeName": "hello", "AttributeType": "S"}
        )
        self.assertEqual(
            range_key.schema(), {"AttributeName": "hello", "KeyType": "RANGE"}
        )
    ...
    def test_alternate_type(self):
        ...
        self.assertEqual(
            alt_key.definition(), {"AttributeName": "alt", "AttributeType": "N"}
        )
...
class IndexFieldTestCase(unittest.TestCase):
    def test_all_index(self):
        all_index = AllIndex(
            "AllKeys", parts=[HashKey("username"), RangeKey("date_joined")]
        )
        self.assertEqual(all_index.name, "AllKeys")
        self.assertEqual(
            [part.attr_type for part in all_index.parts], ["HASH", "RANGE"]
        )
        ...
        self.assertEqual(
            all_index.schema(),
            {
                ...
                "KeySchema": [
                    ...
                    {"AttributeName": "date_joined", "KeyType": "RANGE"},
                ],
    ...
    def test_keys_only_index(self):
        keys_only = KeysOnlyIndex(
            "KeysOnly", parts=[HashKey("username"), RangeKey("date_joined")]
        )
        self.assertEqual(keys_only.name, "KeysOnly")
        self.assertEqual(
            [part.attr_type for part in keys_only.parts], ["HASH", "RANGE"]
        )
        self.assertEqual(keys_only.projection_type, "KEYS_ONLY")

        self.assertEqual(
            keys_only.definition(),
            [
                {"AttributeName": "username", "AttributeType": "S"},
                {"AttributeName": "date_joined", "AttributeType": "S"},
            ],
    ...
    def test_include_index(self):
        include_index = IncludeIndex(
            "IncludeKeys",
            parts=[HashKey("username"), RangeKey("date_joined")],
            includes=["gender", "friend_count"],
        )
        self.assertEqual(include_index.name, "IncludeKeys")
        self.assertEqual(
            [part.attr_type for part in include_index.parts], ["HASH", "RANGE"]
        )
        self.assertEqual(include_index.projection_type, "INCLUDE")

        self.assertEqual(
            include_index.definition(),
            [
                {"AttributeName": "username", "AttributeType": "S"},
                {"AttributeName": "date_joined", "AttributeType": "S"},
            ],
    ...
    def test_global_include_index(self):
        ...
        include_index = GlobalIncludeIndex(
            "IncludeKeys",
            parts=[HashKey("username"), RangeKey("date_joined")],
            includes=["gender", "friend_count"],
        )
...
class ItemTestCase(unittest.TestCase):
    ...
    def test_load(self):
        ...
        self.assertEqual(
            sorted(empty_item["friends"]), sorted(["alice", "bob", "jane"])
        )
    ...
    def test_save_no_changes(self):
        ...
        with mock.patch.object(
            self.table, "_put_item", return_value=True
        ) as mock_put_item:
    ...
    def test_partial_no_changes(self):
        # Unchanged, no save.
        with mock.patch.object(
            self.table, "_update_item", return_value=True
        ) as mock_update_item:
            # Pretend we loaded it via ``get_item``...
            self.johndoe.mark_clean()
    ...
    def test_delete(self):
        ...
        with mock.patch.object(
            self.table, "delete_item", return_value=True
        ) as mock_delete_item:
            self.johndoe.delete()
...
class ResultSetTestCase(unittest.TestCase):
    ...
    def test_fetch_more(self):
        ...
        self.result_function.assert_called_with(
            "john", greeting="Hello", limit=20, exclusive_start_key=4
        )
        self.result_function.reset_mock()
...
class BatchGetResultSetTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.results = BatchGetResultSet(
            keys=[
                "alice",
                "bob",
                "jane",
...
class TableTestCase(unittest.TestCase):
    ...
    def test__introspect_schema(self):
        ...
        raw_schema_2 = [
            {"AttributeName": "username", "KeyType": "BTREE"},
        ]
        raw_attributes_2 = [
            {"AttributeName": "username", "AttributeType": "S"},
        ]
        ...
        raw_schema_3 = [
            ...
            {"AttributeName": "junk", "KeyType": "RANGE"},
        ]
        raw_attributes_3 = [
            {"AttributeName": "user_id", "AttributeType": "N"},
            {"AttributeName": "junk", "AttributeType": "B"},
        ]
        ...
        raw_indexes_2 = [
            {
                ...
                "Projection": {"ProjectionType": "SOMETHING_CRAZY"},
            },
        ]
        self.assertRaises(
    ...
    def test_create_simple(self):
        ...
        with mock.patch.object(
            conn, "create_table", return_value={}
        ) as mock_create_table:
            retval = Table.create(
                "users",
                schema=[HashKey("username"), RangeKey("date_joined", data_type=NUMBER)],
                connection=conn,
            )
        ...
        mock_create_table.assert_called_once_with(
            ...
            key_schema=[
                ...
                {"KeyType": "RANGE", "AttributeName": "date_joined"},
            ],
            provisioned_throughput={"WriteCapacityUnits": 5, "ReadCapacityUnits": 5},
        )
    ...
    def test_create_full(self):
        ...
        mock_create_table.assert_called_once_with(
            attribute_definitions=[
                {"AttributeName": "username", "AttributeType": "S"},
                {"AttributeName": "date_joined", "AttributeType": "N"},
                {"AttributeName": "friend_count", "AttributeType": "S"},
            ],
            key_schema=[
                {"KeyType": "HASH", "AttributeName": "username"},
                {"KeyType": "RANGE", "AttributeName": "date_joined"},
            ...
            table_name="users",
            provisioned_throughput={"WriteCapacityUnits": 10, "ReadCapacityUnits": 20},
            global_secondary_indexes=[
                {
                    "KeySchema": [
                        {"KeyType": "RANGE", "AttributeName": "friend_count"}
                    ],
    ...
    def test_describe(self):
        ...
        with mock.patch.object(
            self.users.connection, "describe_table", return_value=expected
        ) as mock_describe:
    ...
    def test_update(self):
        with mock.patch.object(
            self.users.connection, "update_table", return_value={}
        ) as mock_update:
            self.assertEqual(self.users.throughput["read"], 5)
            self.assertEqual(self.users.throughput["write"], 5)
            self.users.update(
        ...
        mock_update.assert_called_once_with(
            ...
            provisioned_throughput={"WriteCapacityUnits": 2, "ReadCapacityUnits": 7},
        )
        ...
        with mock.patch.object(
            ...
            self.users.update(
                ...
                global_indexes={
                    "WhateverIndex": {"read": 6, "write": 1},
                    "AnotherIndex": {"read": 1, "write": 2},
                },
            )
    ...
    def test_create_global_secondary_index(self):
        ...
        mock_update.assert_called_once_with(
            ...
            global_secondary_index_updates=[
                {
                    "Create": {
                        ...
                        "KeySchema": [
                            {"KeyType": "HASH", "AttributeName": "requiredHashKey"}
                        ],
                        "Projection": {"ProjectionType": "ALL"},
                        "ProvisionedThroughput": {
                            "WriteCapacityUnits": 2,
            ...
            attribute_definitions=[
                {"AttributeName": "requiredHashKey", "AttributeType": "S"}
            ],
        )
    ...
    def test_update_global_secondary_index(self):
        ...
        with mock.patch.object(
            ...
            self.users.update_global_secondary_index(
                global_indexes={"A_IndexToBeUpdated": {"read": 5, "write": 5}}
            )
        ...
        with mock.patch.object(
            ...
            self.users.update_global_secondary_index(
                global_indexes={
                    "A_IndexToBeUpdated": {"read": 5, "write": 5},
                    "B_IndexToBeUpdated": {"read": 9, "write": 9},
                }
            )
    ...
    def test_delete(self):
        with mock.patch.object(
            self.users.connection, "delete_table", return_value={}
        ) as mock_delete:
    ...
    def test_get_item(self):
        ...
        with mock.patch.object(
            self.users.connection, "get_item", return_value=expected
        ) as mock_get_item:
        ...
        mock_get_item.assert_called_once_with(
            ...
            {"username": {"S": "johndoe"}},
            consistent_read=False,
            attributes_to_get=["username", "first_name"],
        )
    ...
    def test_lookup_hash(self):
        ...
        with mock.patch.object(
            self.users, "get_item", return_value=expected
        ) as mock_get_item:
    ...
    def test_lookup_hash_and_range(self):
        ...
        mock_get_item.assert_called_once_with(
            username="johndoe", date_joined=1366056668
        )

    def test_put_item(self):
        with mock.patch.object(
            self.users.connection, "put_item", return_value={}
        ) as mock_put_item:
            self.users.put_item(
                data={
                    "username": "johndoe",
    ...
    def test_private_update_item(self):
        with mock.patch.object(
            self.users.connection, "update_item", return_value={}
        ) as mock_update_item:
    ...
    def test_delete_item(self):
        with mock.patch.object(
            self.users.connection, "delete_item", return_value={}
        ) as mock_delete_item:
            self.assertTrue(
                self.users.delete_item(username="johndoe", date_joined=23456)
            )

        mock_delete_item.assert_called_once_with(
            "users",
            {"username": {"S": "johndoe"}, "date_joined": {"N": "23456"}},
            expected=None,
    ...
    def test_delete_item_conditionally(self):
        with mock.patch.object(
            ...
            self.assertTrue(
                self.users.delete_item(
                    expected={"balance__eq": 0}, username="johndoe", date_joined=23456
                )
            )
        ...
        with mock.patch.object(
            self.users.connection, "delete_item", side_effect=side_effect
        ) as mock_delete_item:
            self.assertFalse(
                self.users.delete_item(
                    expected={"balance__eq": 0}, username="johndoe", date_joined=23456
    ...
    def test_batch_write_no_writes(self):
        with mock.patch.object(
            self.users.connection, "batch_write_item", return_value={}
        ) as mock_batch:
    ...
    def test_batch_write_unprocessed_items(self):
        ...
        with mock.patch.object(
            self.users.connection, "batch_write_item", return_value=unprocessed
        ) as mock_batch:
            with self.users.batch_write() as batch:
    ...
    def test__build_filters(self):
        ...
        self.assertEqual(
            filters,
            {
                ...
                "first_name": {"ComparisonOperator": "NOT_NULL"},
    ...
    def test_private_query(self):
        expected = {
            "ConsumedCapacity": {"CapacityUnits": 0.5, "TableName": "users"},
            "Count": 4,
            "Items": [
                {
                    "username": {"S": "johndoe"},
        ...
        with mock.patch.object(
            self.users.connection, "query", return_value=expected
        ) as mock_query:
            results = self.users._query(
                limit=4, reverse=True, username__between=["aaa", "mmm"]
            )
    ...
    def test_private_scan(self):
        ...
        with mock.patch.object(
            self.users.connection, "scan", return_value=expected
        ) as mock_scan:
        ...
        mock_scan_2.assert_called_once_with(
            "users",
            scan_filter={
                "friend_count": {
                    "AttributeValueList": [{"N": "2"}],
    ...
    def test_query(self):
        ...
        with mock.patch.object(
            results, "the_callable", return_value=items_1
        ) as mock_query:
            res_1 = next(results)
        ...
        with mock.patch.object(
            results, "the_callable", return_value=items_2
        ) as mock_query_2:
            res_3 = next(results)
    ...
    def test_count(self):
        ...
        with mock.patch.object(
            self.users, "describe", return_value=expected
        ) as mock_count:
    ...
    def test_query_count_simple(self):
        ...
        with mock.patch.object(
            self.users.connection, "query", return_value=expected_0
        ) as mock_query:
        ...
        with mock.patch.object(
            self.users.connection, "query", return_value=expected_1
        ) as mock_query:
            results = self.users.query_count(
                username__gt="somename",
                consistent=True,
                scan_index_forward=False,
                limit=10,
            )
        ...
        def return_side_effect(*args, **kwargs):
            if kwargs.get("exclusive_start_key"):
                ...
                return {
                    ...
                    "LastEvaluatedKey": {
                        "username": {"S": "johndoe"},
                        "date_joined": {"N": "4118642633"},
                    },
                }

        with mock.patch.object(
            self.users.connection, "query", side_effect=return_side_effect
        ) as mock_query:
    ...
    def test_private_batch_get(self):
        ...
        with mock.patch.object(
            self.users.connection, "batch_get_item", return_value=expected
        ) as mock_batch_get:
            results = self.users._batch_get(
                keys=[
        ...
        mock_batch_get.assert_called_once_with(
            request_items={
                "users": {
                    "Keys": [
                        {"username": {"S": "alice"}, "friend_count": {"N": "1"}},
                        {"username": {"S": "bob"}, "friend_count": {"N": "1"}},
                        {
                            "username": {"S": "jane"},
                        },
                    ]
        ...
        mock_batch_get_2.assert_called_once_with(
            request_items={
                "users": {
                    "Keys": [
                        {"username": {"S": "alice"}, "friend_count": {"N": "1"}},
    ...
    def test_private_batch_get_attributes(self):
        ...
        mock_batch_get_attr.assert_called_once_with(
            request_items={
                "users": {
                    "Keys": [
                        {"username": {"S": "alice"}},
