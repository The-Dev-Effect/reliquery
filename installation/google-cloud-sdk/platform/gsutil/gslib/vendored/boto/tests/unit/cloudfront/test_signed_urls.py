...
class CloudfrontSignedUrlsTest(unittest.TestCase):
    ...
    def test_sign_custom_policy_1(self):
        ...
        sig = self.dist._sign_string(
            self.custom_policy_1, private_key_string=self.pk_str
        )
        encoded_sig = self.dist._url_base64_encode(sig)
        self.assertEqual(expected, encoded_sig)
    ...
    def test_sign_custom_policy_2(self):
        ...
        sig = self.dist._sign_string(
            self.custom_policy_2, private_key_string=self.pk_str
        )
        encoded_sig = self.dist._url_base64_encode(sig)
        self.assertEqual(expected, encoded_sig)
    ...
    def test_custom_policy_all(self):
        ...
        valid_after = 111111
        ip_range = "192.168.0.0/24"
        policy = self.dist._custom_policy(
            url, expires=expires, valid_after=valid_after, ip_address=ip_range
        )
        policy = json.loads(policy)
