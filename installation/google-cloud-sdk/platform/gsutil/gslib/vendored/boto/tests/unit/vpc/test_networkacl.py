...
class TestDescribeNetworkAcls(AWSMockServiceTestCase):
    ...
    def test_get_all_network_acls(self):
        ...
        response = self.service_connection.get_all_network_acls(
            ["acl-5566953c", "acl-5d659634"], [("vpc-id", "vpc-5266953b")]
        )
...
class TestReplaceNetworkAclAssociation(AWSMockServiceTestCase):
    ...
    def test_associate_network_acl(self):
        self.https_connection.getresponse.side_effect = [
            self.create_response(
                status_code=200, body=self.get_all_network_acls_subnet_body
            ),
            self.create_response(status_code=200),
        ]
        response = self.service_connection.associate_network_acl(
            "acl-5fb85d36", "subnet-ff669596"
        )
    ...
    def test_disassociate_network_acl(self):
        self.https_connection.getresponse.side_effect = [
            self.create_response(
                status_code=200, body=self.get_all_network_acls_vpc_body
            ),
            self.create_response(
                status_code=200, body=self.get_all_network_acls_subnet_body
            ),
        ...
        response = self.service_connection.disassociate_network_acl(
            "subnet-ff669596", "vpc-5266953b"
        )
...
class TestCreateNetworkAcl(AWSMockServiceTestCase):
    ...
    def test_create_network_acl(self):
        ...
        self.assert_request_parameters(
            {"Action": "CreateNetworkAcl", "VpcId": "vpc-11ad4878"},
            ignore_params_values=[
                "AWSAccessKeyId",
                "SignatureMethod",
                "SignatureVersion",
...
class DeleteCreateNetworkAcl(AWSMockServiceTestCase):
    ...
    def test_delete_network_acl(self):
        ...
        self.assert_request_parameters(
            {"Action": "DeleteNetworkAcl", "NetworkAclId": "acl-2cb85d45"},
            ignore_params_values=[
                "AWSAccessKeyId",
                "SignatureMethod",
                "SignatureVersion",
...
class TestReplaceNetworkAclEntry(AWSMockServiceTestCase):
    ...
    def test_replace_network_acl_icmp(self):
        self.set_http_response(status_code=200)
        response = self.service_connection.replace_network_acl_entry(
            "acl-2cb85d45", 110, "tcp", "deny", "0.0.0.0/0", icmp_code=-1, icmp_type=8
        )
        self.assert_request_parameters(
            {
                "Action": "ReplaceNetworkAclEntry",
...
class TestDeleteNetworkAclEntry(AWSMockServiceTestCase):
    ...
    def test_delete_network_acl(self):
        self.set_http_response(status_code=200)
        response = self.service_connection.delete_network_acl_entry(
            "acl-2cb85d45", 100, egress=False
        )
        self.assert_request_parameters(
            {
