...
class PromoteReadReplicaTest(unittest.TestCase):
    ...
    def test_promote(self):
        ...
        self.masterDB = self.conn.create_dbinstance(
            self.masterDB_name, 5, "db.t1.micro", "root", "bototestpw"
        )
        ...
        # Wait up to 15 minutes for the replicaDB to become available
        print(
            '--- waiting for "%s" to be promoted and available  ---'
            % self.replicaDB_name
        )
        wait_timeout = time.time() + (15 * 60)
        time.sleep(60)
        ...
        print(
            '--- renaming "%s" to "%s" ---' % (self.replicaDB_name, self.renamedDB_name)
        )
