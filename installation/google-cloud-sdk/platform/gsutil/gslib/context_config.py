...
def _split_pem_into_sections(contents, logger):
    ...
    for line in contents.splitlines():
        ...
        if begin:
            if pem_section_name:
                logger.warning(
                    "Section %s missing end line and will be ignored."
                    % pem_section_name
                )
            if name in result.keys():
                logger.warning(
                    "Section %s already exists, and the older section will "
                    "be ignored." % name
                )
            ...
            if not pem_section_name:
                ...
                logger.warning(
                    "Section %s missing a matching end line. Found: %s"
                    % (pem_section_name, name)
                )
                pem_section_name = None
...
class _ContextConfig(object):
    ...
    def __init__(self, logger):
        ...
        self.use_client_certificate = config.getbool(
    ...
    def _unprovision_client_cert(self):
        ...
        if self.client_cert_path is not None:
            try:
                ...
                self.logger.debug(
                    "Unprovisioned client cert: %s" % self.client_cert_path
                )
            except OSError as e:
