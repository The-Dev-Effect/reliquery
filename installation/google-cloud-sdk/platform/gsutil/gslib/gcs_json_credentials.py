...
DEFAULT_GOOGLE_OAUTH2_PROVIDER_TOKEN_URI = "https://oauth2.googleapis.com/token"
...
def SetUpJsonCredentialsAndCache(api, logger, credentials=None):
    ...
    if isinstance(api.credentials, ImpersonationCredentials):
        logger.warn(
            ...
            "API calls will be executed as [%s].",
    ...
    credential_store_key = GetCredentialStoreKey(
        api.credentials, GetGcsJsonApiVersion()
    )
...
def _GetOauth2ServiceAccountCredentials():
    ...
    if keyfile_is_utf8:
        try:
            ...
            raise Exception(
                'Could not parse JSON keyfile "%s" as valid JSON' % private_key_filename
            )
...
def _GetOauth2UserAccountCredentials():
    ...
    client_id = config.get(
        "OAuth2", "client_id", os.environ.get("OAUTH2_CLIENT_ID", gsutil_client_id)
    )
