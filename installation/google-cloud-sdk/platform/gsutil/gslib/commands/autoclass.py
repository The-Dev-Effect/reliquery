...
class AutoclassCommand(Command):
    ...
    def _get_autoclass(self, blr):
        ...
        bucket_metadata = self.gsutil_api.GetBucket(
            bucket_url.bucket_name, fields=["autoclass"], provider=bucket_url.scheme
        )
    ...
    def _set_autoclass(self, blr, setting_arg):
        ...
        print(
            "Setting Autoclass %s for %s" % (setting_arg, str(bucket_url).rstrip("/"))
        )
    ...
    def _autoclass(self):
        ...
        if subcommand == "get":
            ...
            text_util.InsistOnOrOff(
                setting_arg, "Only on and off values allowed for set option"
            )
        ...
        for url_str in url_args:
            ...
            for bucket_listing_ref in bucket_iter:
                if (
                    self.gsutil_api.GetApiSelector(
    ...
    def RunCommand(self):
        ...
        if action_subcommand == "get" or action_subcommand == "set":
            ...
            raise CommandException(
                'Invalid subcommand "%s", use get|set instead.' % action_subcommand
            )
