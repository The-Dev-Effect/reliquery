...
_DESCRIPTION = """
  ...
  and ``serviceaccount``.
""" + (
    _AUTHORIZE_DESCRIPTION + _ENCRYPTION_DESCRIPTION + _SERVICEACCOUNT_DESCRIPTION
)

_DETAILED_HELP_TEXT = CreateHelpText(_SYNOPSIS, _DESCRIPTION)

_authorize_help_text = CreateHelpText(_AUTHORIZE_SYNOPSIS, _AUTHORIZE_DESCRIPTION)
_encryption_help_text = CreateHelpText(_ENCRYPTION_SYNOPSIS, _ENCRYPTION_DESCRIPTION)
...
class KmsCommand(Command):
    ...
    def _EncryptionSetKey(self, bucket_metadata, bucket_url, svc_acct_for_project_num):
        ...
        try:
            ...
            service_account, newly_authorized = (
                svc_acct_for_project_num[bucket_project_number],
                False,
            )
        except KeyError:
    ...
    def _Encryption(self):
        ...
        def _EncryptionForBucket(blr):
            ...
            if self.kms_key:
                self._EncryptionSetKey(
                    bucket_metadata, bucket_url, svc_acct_for_project_num
                )
                return 0
