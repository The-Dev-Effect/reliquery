...
class LsCommand(Command):
    ...
    def _PrintBucketInfo(self, bucket_blr, listing_style):
        ...
        if bucket.autoclass and bucket.autoclass.enabled:
            fields["autoclass_enabled_date"] = bucket.autoclass.toggleTime.strftime(
                "%a, %d %b %Y"
            )
        ...
        for key in fields:
            ...
            if (
                not isinstance(previous_value, six.string_types)
        ...
        if "bucket_policy_only_enabled" in fields:
            bucket_policy_only_enabled_line = (
                "\tBucket Policy Only enabled:\t" "{bucket_policy_only_enabled}\n"
            )
        ...
        for url_str in self.args:
            ...
            if storage_url.IsFileUrl():
                raise CommandException(
                    "Only cloud URLs are supported for %s" % self.command_name
                )
            bucket_fields = None
            if (
                listing_style == ListingStyle.SHORT
            ...
            if storage_url.IsProvider():
                ...
                for blr in self.WildcardIterator(
                    "%s://*" % storage_url.scheme
                ).IterBuckets(bucket_fields=bucket_fields):
                ...
                exp_dirs, exp_objs, exp_bytes = listing_helper.ExpandUrlAndPrint(
                    storage_url
                )
