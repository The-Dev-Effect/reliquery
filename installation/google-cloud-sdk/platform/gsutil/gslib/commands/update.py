...
class UpdateCommand(Command):
    ...
    def _ExplainIfSudoNeeded(self, tf, dirs_to_remove, old_cwd):
        ...
        raise CommandException(
            ...
            + ('\n\tsudo chmod 0644 %s\n\tsudo env BOTO_CONFIG="%s" %s update' "%s")
    ...
    def RunCommand(self):
        ...
        gsutil_bin_parent_dir = os.path.normpath(os.path.join(gslib.GSUTIL_DIR, ".."))
        ...
        if not system_util.IS_WINDOWS:
            ...
            for dirname, subdirs, filenames in os.walk(new_dir):
                for filename in filenames:
                    ...
                    os.fchmod(
                        fd, stat.S_IWRITE | stat.S_IRUSR | stat.S_IRGRP | stat.S_IROTH
                    )
                    os.close(fd)
