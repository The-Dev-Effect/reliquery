...
def _KeyMetadataOutput(metadata):
    ...
    message += FormatInfo("Time Created", metadata.timeCreated.strftime(_TIME_FORMAT))
    message += FormatInfo("Time Last Updated", metadata.updated.strftime(_TIME_FORMAT))
...
class HmacCommand(Command):
    ...
    def _CreateHmacKey(self, thread_state=None):
        ...
        if self.args:
            ...
            raise CommandException(
                err_msg % (self.command_name, self.action_subcommand, _CREATE_SYNOPSIS)
            )
    ...
    def _DeleteHmacKey(self, thread_state=None):
        ...
        if self.args:
            ...
            raise _AccessIdException(
                self.command_name, self.action_subcommand, _DELETE_SYNOPSIS
            )
    ...
    def _GetHmacKey(self, thread_state=None):
        ...
        if self.args:
            ...
            raise _AccessIdException(
                self.command_name, self.action_subcommand, _GET_SYNOPSIS
            )
    ...
    def _ListHmacKeys(self, thread_state=None):
        ...
        response = gsutil_api.ListHmacKeys(
            self.project_id, self.service_account_email, self.show_all, provider="gs"
        )
    ...
    def _UpdateHmacKey(self, thread_state=None):
        ...
        if not self.state:
            ...
            raise CommandException(
                "The state flag value must be one of %s"
