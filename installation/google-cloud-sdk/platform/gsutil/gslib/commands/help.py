...
class HelpCommand(Command):
    ...
    def RunCommand(self):
        ...
        if not self.args:
            ...
            for help_prov in sorted(
                help_type_map["additional_help"], key=lambda hp: hp.help_spec.help_name
            ):
                output.append(
                    format_str
                    % (
    ...
    def _OutputHelp(self, help_str):
        ...
        if "PAGER" in os.environ and num_lines >= GetTermLines():
            ...
            try:
                ...
                Popen(pager, stdin=PIPE, universal_newlines=True).communicate(
                    input=input_for_pager
                )
            except OSError as e:
                raise CommandException(
                    "Unable to open pager (%s): %s" % (" ".join(pager), e)
                )
        else:
            text_util.print_to_fd(help_str)
