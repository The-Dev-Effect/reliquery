...
PerformSlicedDownloadReturnValues = namedtuple(
    ...
    "component_num crc32c bytes_transferred component_total_size " "server_encoding",
)
...
def InsistDstUrlNamesContainer(exp_dst_url, have_existing_dst_container, command_name):
...
def _ShouldTreatDstUrlAsBucketSubDir(
    ...
    if dst_url.IsCloudUrl():
        return have_multiple_srcs or (src_url_names_container and recursion_requested)
...
def _ShouldTreatDstUrlAsSingleton(
    ...
    if dst_url.IsFileUrl():
        ...
        return (
            not have_multiple_srcs
            and not have_existing_dest_subdir
            and dst_url.IsObject()
        )
...
def ConstructDstUrl(
    ...
    if exp_src_url.IsFileUrl() and (exp_src_url.IsStream() or exp_src_url.IsFifo()):
        if have_existing_dest_subdir:
            ...
            raise CommandException(
                "Destination object name needed when " "source is a %s" % type_text
            )
        return exp_dst_url
    ...
    if (
        have_multiple_srcs
        or src_url_names_container
        or (exp_dst_url.IsFileUrl() and exp_dst_url.IsDirectory())
    ...
    if (
        global_copy_helper_opts.perform_mv
        and recursion_requested
        and src_url_names_container
        and not have_existing_dest_subdir
    ):
        ...
        dst_key_name = "%s/%s" % (
            exp_dst_url.object_name.rstrip("/"),
            exp_src_url_tail.strip("/"),
        )

    elif src_url_names_container and (
        exp_dst_url.IsCloudUrl() or exp_dst_url.IsDirectory()
    ):
        ...
        dst_key_name = exp_src_url.versionless_url_string[
            len(src_url_path_sans_final_dir) :
        ].lstrip(src_url.delim)
    ...
    if not recursive_move_to_new_subdir and (
        exp_dst_url.IsFileUrl()
        or _ShouldTreatDstUrlAsBucketSubDir(
            have_multiple_srcs,
            exp_dst_url,
            have_existing_dest_subdir,
            src_url_names_container,
...
def _CreateDigestsFromDigesters(digesters):
    ...
    if digesters:
        for alg in digesters:
            digests[alg] = (
                base64.b64encode(digesters[alg].digest()).rstrip(b"\n").decode("ascii")
            )
    return digests


def _CreateDigestsFromLocalFile(
    status_queue, algs, file_name, src_url, src_obj_metadata
):
        ...
        CalculateHashesFromContents(
            ...
            callback_processor=ProgressCallbackWithTimeout(
                ...
                FileProgressCallbackHandler(
                    status_queue, src_url=src_url, operation_name="Hashing"
                ).call,
            ),
        )
    digests = {}
...
def _CheckCloudHashes(logger, src_url, dst_url, src_obj_metadata, dst_obj_metadata):
    ...
    if not checked_one:
        if is_upload:
            logger.warn(
                ...
                "Integrity cannot be assured without hashes.",
                obj_url,
            )
        else:
            # One known way this can currently happen is when downloading objects larger
...
def CheckForDirFileConflict(exp_src_url, dst_url):
    ...
    if os.path.isfile(final_dir):
        raise CommandException(
            "Cannot retrieve %s because a file exists "
            "where a directory needs to be created (%s)."
            % (exp_src_url.url_string, final_dir)
        )
    if os.path.isdir(dst_path):
        raise CommandException(
            "Cannot retrieve %s because a directory exists "
...
def _PartitionFile(
    ...
    for i in range(num_components):
        ...
        temp_file_name = (
            random_prefix + PARALLEL_UPLOAD_TEMP_NAMESPACE + digest + "_" + str(i)
        )
...
def _DoParallelCompositeUpload(
    ...
    encryption_key_sha256 = (
        encryption_keywrapper.crypto_key_sha256 if encryption_keywrapper else None
    )
    ...
    random_prefix = (
        ...
        else GenerateComponentObjectPrefix(encryption_key_sha256=encryption_key_sha256)
    )
    ...
    for component in components_to_upload:
        components_info[component.dst_url.url_string] = (
            FileMessage.COMPONENT_TO_UPLOAD,
    ...
    cp_results = command_obj.Apply(
        ...
        components_to_upload,
        copy_exception_handler,
        ("op_failure_count", "total_bytes_transferred"),
...
def _ShouldDoParallelCompositeUpload(
    logger, allow_splitting, src_url, dst_url, file_size, gsutil_api, canned_acl=None
):
    ...
    all_factors_but_size = (
        ...
        and not canned_acl
    )  # TODO: Implement canned ACL support for compose.
...
def ExpandUrlToSingleBlr(
    ...
    if ContainsWildcard(url_str):
        blr_expansion = list(
            CreateWildcardIterator(
                url_str, gsutil_api, project_id=project_id, logger=logger
            )
        )
        if len(blr_expansion) != 1:
            raise CommandException(
                "Destination (%s) must match exactly 1 URL" % url_str
            )
        blr = blr_expansion[0]
    ...
    for obj_or_prefix in list_iterator:
        ...
        if (
            obj_or_prefix.datatype == CloudApi.CsObjectOrPrefixType.PREFIX
            and obj_or_prefix.data == prefix + "/"
        ):
            # Case 2: If there is a matching prefix when listing the destination URL.
            return (storage_url, True)
        elif (
            obj_or_prefix.datatype == CloudApi.CsObjectOrPrefixType.OBJECT
            and obj_or_prefix.data.name == storage_url.object_name + "_$folder$"
        ):
            ...
            obj_or_prefix.datatype == CloudApi.CsObjectOrPrefixType.OBJECT
            and obj_or_prefix.data.name == storage_url.object_name
        ):
...
def SrcDstSame(src_url, dst_url):
    ...
    if src_url.IsFileUrl() and dst_url.IsFileUrl():
        ...
        return (
            src_url.url_string == dst_url.url_string
            and src_url.generation == dst_url.generation
        )
...
def _LogCopyOperation(logger, src_url, dst_url, dst_obj_metadata):
    ...
    Args:
      ...
      dst_obj_metadata: Object-specific metadata that should be overidden during
                        the copy.
    """
    if dst_url.IsCloudUrl() and dst_obj_metadata and dst_obj_metadata.contentType:
...
def _CopyObjToObjInTheCloud(
    ...
    result_url.generation = GenerationFromUrlAndString(result_url, dst_obj.generation)
    ...
    return (end_time - start_time, src_obj_metadata.size, result_url, dst_obj.md5Hash)
...
def _SetContentTypeFromFile(src_url, dst_obj_metadata):
    ...
    if (
        dst_obj_metadata.contentType is None
        and src_url.IsFileUrl()
        and not src_url.IsStream()
        and not src_url.IsFifo()
    ):
                ...
                try:
                    ...
                    if p.returncode != 0 or error:
                        raise CommandException(
                            ...
                            "(returncode=%d).\n%s"
                            % (real_file_path, p.returncode, error)
                        )
...
# pylint: disable=undefined-variable
def _UploadFileToObjectNonResumable(
    src_url,
    src_obj_filestream,
    src_obj_size,
    dst_url,
...
def _UploadFileToObjectResumable(
    ...
    encryption_key_sha256 = (
        ...
        if encryption_keywrapper and encryption_keywrapper.crypto_key_sha256
        else None
    )
    ...
    while retryable:
        try:
            ...
            logger.info(
                "Caught ResumableUploadStartOverException for upload of %s."
                % src_url.url_string
            )
            ...
            try:
                logger.info(
                    "Checking that bucket %s exists before retrying upload..."
                ...
                DeleteTrackerFile(tracker_file_name)
                logger.info(
                    "Deleted tracker file %s for resumable upload of %s before "
            ...
            PutToQueueWithTimeout(
                ...
                RetryableErrorMessage(
                    e, time.time(), num_retries=num_startover_attempts
                ),
            )
            time.sleep(
                min(random.random() * (2 ** num_startover_attempts), GetMaxRetryDelay())
            )
...
def _ApplyZippedUploadCompression(src_url, src_obj_filestream, src_obj_size, logger):
    ...
    try:
        ...
        if src_obj_size is not None and (
            ...
            raise CommandException(
                "Inadequate temp space available to compress "
                "%s. See the CHANGING TEMP DIRECTORIES section "
                'of "gsutil help cp" for more info.' % src_url
            )
        compression_level = config.getint(
            "GSUtil", "gzip_compression_level", DEFAULT_GZIP_COMPRESSION_LEVEL
        )
        ...
        if zipped_file:
            ...
            # delete it.
                ...
                logger.warning(
                    ...
                    "temporary file is still locked.",
...
def _UploadFileToObject(
    ...
    non_resumable_upload = (
        (0 if upload_size is None else upload_size) < ResumableThreshold()
        or src_url.IsStream()
        or src_url.IsFifo()
    )

    if (src_url.IsStream() or src_url.IsFifo()) and gsutil_api.GetApiSelector(
        provider=dst_url.scheme
    ) == ApiSelector.JSON:
    ...
    if not parallel_composite_upload and len(hash_algs):
        ...
        wrapped_filestream = HashingFileUploadWrapper(
            upload_stream, digesters, hash_algs, upload_url, logger
        )
    else:
        ...
        try:
            ...
            if _RENAME_ON_HASH_MISMATCH:
                ...
                dst_obj_metadata.name = (
                    dst_url.object_name + _RENAME_ON_HASH_MISMATCH_SUFFIX
                )
                gsutil_api.CopyObject(
                    corrupted_obj_metadata, dst_obj_metadata, provider=dst_url.scheme
                )
...
def _PerformSlicedDownloadObjectToFile(cls, args, thread_state=None):
    ...
    src_obj_metadata = protojson.decode_message(
        apitools_messages.Object, args.src_obj_metadata_json
    )
...
def _MaintainSlicedDownloadTrackerFiles(
    src_obj_metadata, dst_url, download_file_name, logger, api_selector, num_components
):
    ...
    try:
        ...
        if existing_file_size == src_obj_metadata.size:
            ...
            if (
                ...
                and tracker_file_data["generation"] == src_obj_metadata.generation
                and tracker_file_data["num_components"] == num_components
            ):
                return
            else:
                tracker_file.close()
                logger.warn(
                    "Sliced download tracker file doesn't match for "
        ...
        if isinstance(e, ValueError) or e.errno != errno.ENOENT:
            logger.warn(
                "Couldn't read sliced download tracker file (%s): %s. "
                "Restarting download from scratch." % (tracker_file_name, str(e))
            )
    finally:
        if fp:
            fp.close()
...
class SlicedDownloadFileWrapper(object):
    ...
    def write(self, data):  # pylint: disable=invalid-name
        ...
        assert (
            self._start_byte <= current_file_pos
            and current_file_pos + len(data) <= self._end_byte + 1
        )
        ...
        if (
            self._last_tracker_file_byte is None
            or current_file_pos - self._last_tracker_file_byte > threshold
            or current_file_pos == self._end_byte + 1
        ):
...
def _PartitionObject(
    src_url, src_obj_metadata, dst_url, download_file_name, decryption_key=None
):
...
def _DoSlicedDownload(
    ...
    _MaintainSlicedDownloadTrackerFiles(
        src_obj_metadata,
        dst_url,
        download_file_name,
        logger,
    ...
    for (i, component) in enumerate(components_to_download):
        ...
        download_start_byte = GetDownloadStartByte(
            src_obj_metadata, dst_url, api_selector, component.start_byte, size, i
        )
    ...
    for cp_result in cp_results:
        ...
        server_gzip = (
            cp_result.server_encoding
            and cp_result.server_encoding.lower().endswith("gzip")
        )
...
def _DownloadObjectToFileResumable(
    src_url,
    src_obj_metadata,
    dst_url,
    download_file_name,
    ...
    try:
        ...
        if resuming or download_complete:
            ...
            hash_callback = ProgressCallbackWithTimeout(
                ...
                FileProgressCallbackHandler(
                    ...
                    src_url=src_url,
                    dst_url=dst_url,
                    operation_name="Hashing",
                ).call,
            )

            while bytes_digested < total_bytes_to_digest:
                bytes_to_read = min(
                    DEFAULT_FILE_BUFFER_SIZE, total_bytes_to_digest - bytes_digested
                )
        ...
        if is_sliced and src_obj_metadata.size >= ResumableThreshold():
            fp = SlicedDownloadFileWrapper(
                fp, tracker_file_name, src_obj_metadata, start_byte, end_byte
            )
        ...
        logger.warning(
            "Caught ResumableDownloadException (%s) for download of %s.",
            e.reason,
            download_name,
        )
        raise
...
def _DownloadObjectToFileNonResumable(
    src_url,
    src_obj_metadata,
    dst_url,
    download_file_name,
...
def _DownloadObjectToFile(
    ...
    hash_algs = GetDownloadHashAlgs(
        logger, consider_md5=consider_md5, consider_crc32c=src_obj_metadata.crc32c
    )
    digesters = dict((alg, hash_algs[alg]()) for alg in hash_algs or {})
    ...
    if not download_complete:
        if sliced_download:
            (bytes_transferred, crc32c) = _DoSlicedDownload(
                src_url,
                src_obj_metadata,
                dst_url,
                download_file_name,
            ...
            raise CommandException(
                "Invalid download strategy %s chosen for"
                "file %s" % (download_strategy, download_file_name)
            )
    end_time = time.time()
...
def _ValidateAndCompleteDownload(
    ...
    if not digest_verified:
        try:
            ...
            _CheckHashes(
                logger, src_url, src_obj_metadata, final_file_name, local_hashes
            )
            DeleteDownloadTrackerFiles(dst_url, api_selector)
        except HashMismatchException:
            DeleteDownloadTrackerFiles(dst_url, api_selector)
...
def _CopyFileToFile(src_url, dst_url, status_queue=None, src_obj_metadata=None):
    ...
    return (end_time - start_time, os.path.getsize(dst_url.object_name), dst_url, None)
...
# pylint: disable=undefined-variable
def _CopyObjToObjDaisyChainMode(
    src_url,
    src_obj_metadata,
    dst_url,
    dst_obj_metadata,
...
def GetSourceFieldsNeededForCopy(
    dst_is_cloud,
    skip_unsupported_objects,
    preserve_acl,
    is_rsync=False,
...
def WarnIfMvEarlyDeletionChargeApplies(src_url, src_obj_metadata, logger):
    ...
    if (
        src_url.scheme == "gs"
        and src_obj_metadata
        and src_obj_metadata.timeCreated
        ...
        if early_deletion_cutoff_seconds:
            minimum_delete_age = early_deletion_cutoff_seconds + ConvertDatetimeToPOSIX(
                src_obj_metadata.timeCreated
            )
...
def MaybeSkipUnsupportedObject(src_url, src_obj_metadata):
    ...
    if (
        src_url.scheme == "s3"
        and global_copy_helper_opts.skip_unsupported_objects
        and src_obj_metadata.storageClass == "GLACIER"
    ):
...
def PerformCopy(
    ...
    if (
        dst_url.scheme == "s3"
        and src_url != "s3"
        and src_obj_size is not None
        and src_obj_size > S3_MAX_UPLOAD_SIZE  # Can't compare int to None in py3
    ):
        raise CommandException(
    ...
    if global_copy_helper_opts.no_clobber:
        ...
        if preconditions.gen_match:
            raise ArgumentException(
                "Specifying x-goog-if-generation-match is " "not supported with cp -n"
            )
        else:
    ...
    if dst_url.IsCloudUrl():
        ...
        if (
            ...
            and encryption_keywrapper.crypto_type == CryptoKeyType.CMEK
            and dst_url.scheme == "gs"
        ):
    ...
    if src_url.IsCloudUrl():
        if dst_url.IsFileUrl():
            ...
            return _CopyObjToObjInTheCloud(
                src_url,
                src_obj_metadata,
                dst_url,
                dst_obj_metadata,
            ...
            return _CopyObjToObjDaisyChainMode(
                src_url,
                src_obj_metadata,
                dst_url,
                dst_obj_metadata,
        ...
        if dst_url.IsCloudUrl():
            ...
            uploaded_metadata = _UploadFileToObject(
                src_url,
                src_obj_filestream,
                src_obj_size,
                dst_url,
                    ...
                    for row in reader:
                        if first_row:
                            try:
                                ...
                                raise CommandException(
                                    "Missing headers in manifest file: %s"
                                    % self.manifest_path
                                )
                        first_row = False
...
def ResolveWildcardsInPathBeforeFinalDir(src_url_path_sans_final_dir, exp_src_url):
    ...
    for path_segment_index in xrange(len(wildcarded_src_obj_path)):
        if ContainsWildcard(wildcarded_src_obj_path[path_segment_index]):
            ...
            wildcarded_src_obj_path[path_segment_index] = expanded_src_obj_path[
                path_segment_index
            ]
...
def FilterExistingComponents(dst_args, existing_components, bucket_url, gsutil_api):
    ...
    for tracker_object in existing_components:
        if (
            tracker_object.object_name not in dst_args.keys()
    ...
    if uploaded_components:
        logging.info(
            "Found %d existing temporary components to reuse.", len(uploaded_components)
        )
