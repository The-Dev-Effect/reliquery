...
def CheckFreeSpace(path):
    ...
    if IS_WINDOWS:
        try:
            ...
            get_disk_free_space_ex = WINFUNCTYPE(
                c_int,
                c_wchar_p,
                POINTER(c_uint64),
                POINTER(c_uint64),
            ...
            get_disk_free_space_ex = WINFUNCTYPE(
                c_int, c_char_p, POINTER(c_uint64), POINTER(c_uint64), POINTER(c_uint64)
            )
            get_disk_free_space_ex = get_disk_free_space_ex(
...
def MonkeyPatchHttp():
    ...
    if ver.major == 3:
        if (
            ver.minor < 6
            or (ver.minor == 6 and ver.micro < 7)
            or (ver.minor == 7 and ver.micro == 0)
        ):
