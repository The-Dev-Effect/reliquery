...
class POSIXAttributes(object):
    ...
    def __init__(self, atime=NA_TIME, mtime=NA_TIME, uid=NA_ID, gid=NA_ID, mode=None):
...
def SerializeFileAttributesToObjectMetadata(
    posix_attrs, custom_metadata, preserve_posix=False
):
    ...
    if posix_attrs.mtime != NA_TIME:
        CreateCustomMetadata(
            entries={MTIME_ATTR: posix_attrs.mtime}, custom_metadata=custom_metadata
        )
    # Only add other POSIX attributes if the preserve_posix flag is set.
    if preserve_posix:
        if posix_attrs.atime != NA_TIME:
            CreateCustomMetadata(
                entries={ATIME_ATTR: posix_attrs.atime}, custom_metadata=custom_metadata
            )
...
def NeedsPOSIXAttributeUpdate(
    ...
    return posix_attrs, (
        (has_src_atime and not has_dst_atime)
...
def ValidateFilePermissionAccess(url_str, uid=NA_ID, gid=NA_ID, mode=NA_MODE):
    ...
    if uid_present:
        try:
            ...
            return (
                False,
                "UID for %s doesn't exist on current system. uid: %d" % (url_str, uid),
            )
    if gid_present:
        try:
            grp.getgrgid(gid)
            ...
            return (
                False,
                "GID for %s doesn't exist on current system. gid: %d" % (url_str, gid),
            )
...
def ParseAndSetPOSIXAttributes(
    path, obj_metadata, is_rsync=False, preserve_posix=False
):
    ...
    try:
        ...
        raise CommandException(
            "Check POSIX attribute values for %s" % obj_metadata.name
        )
...
def WarnNegativeAttribute(attr_name, url_str):
    ...
    """
    logging.getLogger().warn("%s has a negative %s in its metadata", url_str, attr_name)
...
def WarnFutureTimestamp(attr_name, url_str):
    ...
    logging.getLogger().warn(
        "%s has an %s more than 1 day from current system" " time", url_str, attr_name
    )
