...
class NameExpansionResult(object):
    ...
    def __init__(
        ...
        self.expanded_result = (
            encoding.MessageToJson(expanded_result) if expanded_result else None
        )
...
class _NameExpansionIterator(object):
    ...
    def __iter__(self):
        ...
        for url_str in self.url_strs:
            ...
            if (
                storage_url.IsCloudUrl()
                and storage_url.IsBucket()
...
class SeekAheadNameExpansionIterator(object):
    ...
    def __init__(
        ...
        self.count_data_bytes = (
            command_name in ("cp", "mv", "rewrite") and not file_size_will_change
        )
...
class _ImplicitBucketSubdirIterator(object):
    ...
    def __init__(
        self, name_exp_instance, blr_iter, subdir_exp_wildcard, bucket_listing_fields
    ):
...
class CopyObjectsIterator(six.Iterator):
    ...
    def __next__(self):
        ...
        if (
            not self.is_daisy_chain
            and dst_url_scheme is not None
            and elt.source_storage_url.IsCloudUrl()
            and elt.source_storage_url.scheme != dst_url_scheme
        ):
