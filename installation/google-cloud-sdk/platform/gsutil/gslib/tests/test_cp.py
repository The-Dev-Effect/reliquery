...
def TestCpMvPOSIXBucketToLocalErrors(cls, bucket_uri, obj, tmpdir, is_cp=True):
    ...
    test_params = {
        "test1": {MODE_ATTR: "333", error: POSIX_MODE_ERROR},
        "test2": {GID_ATTR: GetInvalidGid, error: POSIX_GID_ERROR},
        "test3": {GID_ATTR: GetInvalidGid, MODE_ATTR: "420", error: POSIX_GID_ERROR},
        "test4": {UID_ATTR: INVALID_UID, error: POSIX_UID_ERROR},
        "test5": {UID_ATTR: INVALID_UID, MODE_ATTR: "530", error: POSIX_UID_ERROR},
        "test6": {
        ...
        "test12": {UID_ATTR: USER_ID, GID_ATTR: GetInvalidGid, error: POSIX_GID_ERROR},
        "test13": {
            UID_ATTR: USER_ID,
            GID_ATTR: GetInvalidGid,
            MODE_ATTR: "640",
    ...
    for test_name, attrs_dict in six.iteritems(test_params):
        ...
        stderr = cls.RunGsUtil(
            ["cp" if is_cp else "mv", "-P", suri(bucket_uri, obj.object_name), tmpdir],
            expected_status=1,
            return_stderr=True,
        )
        cls.assertIn(
...
def TestCpMvPOSIXBucketToLocalNoErrors(cls, bucket_uri, tmpdir, is_cp=True):
    ...
    test_params = {
        ...
        "obj9": {UID_ATTR: USER_ID, GID_ATTR: primary_gid, MODE_ATTR: "433"},
        "obj10": {UID_ATTR: USER_ID, GID_ATTR: non_primary_gid, MODE_ATTR: "442"},
    }
    ...
    for obj_name in six.iterkeys(test_params):
        ...
        cls.RunGsUtil(
            ["cp" if is_cp else "mv", "-P", suri(bucket_uri, obj_name), tmpdir]
        )
    ...
    cls.VerifyLocalPOSIXPermissions(
        os.path.join(tmpdir, "obj1"), gid=primary_gid, mode=DEFAULT_MODE
    )
    cls.VerifyLocalPOSIXPermissions(
        os.path.join(tmpdir, "obj2"), gid=non_primary_gid, mode=DEFAULT_MODE
    )
    cls.VerifyLocalPOSIXPermissions(
        os.path.join(tmpdir, "obj3"), gid=primary_gid, mode=0o440
    )
    cls.VerifyLocalPOSIXPermissions(
        os.path.join(tmpdir, "obj4"), gid=non_primary_gid, mode=0o444
    )
    cls.VerifyLocalPOSIXPermissions(
        os.path.join(tmpdir, "obj5"), uid=USER_ID, gid=primary_gid, mode=DEFAULT_MODE
    )
    cls.VerifyLocalPOSIXPermissions(
        os.path.join(tmpdir, "obj6"), uid=USER_ID, gid=primary_gid, mode=0o420
    )
    cls.VerifyLocalPOSIXPermissions(
        os.path.join(tmpdir, "obj7"), uid=USER_ID, gid=primary_gid, mode=DEFAULT_MODE
    )
    cls.VerifyLocalPOSIXPermissions(
        os.path.join(tmpdir, "obj8"),
        uid=USER_ID,
        gid=non_primary_gid,
        mode=DEFAULT_MODE,
    )
    cls.VerifyLocalPOSIXPermissions(
        os.path.join(tmpdir, "obj9"), uid=USER_ID, gid=primary_gid, mode=0o433
    )
    cls.VerifyLocalPOSIXPermissions(
        os.path.join(tmpdir, "obj10"), uid=USER_ID, gid=non_primary_gid, mode=0o442
    )
...
def TestCpMvPOSIXLocalToBucketNoErrors(cls, bucket_uri, is_cp=True):
    ...
    for obj_name, attrs_dict in six.iteritems(test_params):
        ...
        cls.RunGsUtil(
            ["cp" if is_cp else "mv", "-P", fpath, suri(bucket_uri, obj_name)]
        )
        if uid != NA_ID:
            cls.VerifyObjectCustomAttribute(
                bucket_uri.bucket_name, obj_name, UID_ATTR, str(uid)
            )
        if gid != NA_ID:
            cls.VerifyObjectCustomAttribute(
                bucket_uri.bucket_name, obj_name, GID_ATTR, str(gid)
            )
        if mode != NA_MODE:
            cls.VerifyObjectCustomAttribute(
                bucket_uri.bucket_name, obj_name, MODE_ATTR, str(mode)
            )
        ...
        if (
            ...
            def DeleteBucket():
                ...
                for k in bucket_list:
                    self._bucket_uri.get_bucket().delete_key(
...
class TestCp(testcase.GsUtilIntegrationTestCase):
    ...
    def test_dest_bucket_not_exist(self):
        ...
        def _Check():
            stderr = self.RunGsUtil(
                ["cp", fpath, invalid_bucket_uri], expected_status=1, return_stderr=True
            )
    ...
    def test_copy_in_cloud_noclobber(self):
        ...
        stderr = self.RunGsUtil(
            ["cp", "-n", suri(key_uri), suri(bucket2_uri)], return_stderr=True
        )
        self.assertIn(
    ...
    def test_cp_from_local_file_to_fifo(self):
        ...
        read_thread = threading.Thread(
            target=_ReadContentsFromFifo, args=(fifo_path, list_for_output)
        )
        read_thread.start()
    ...
    def test_streaming(self):
        bucket_uri = self.CreateBucket()
        stderr = self.RunGsUtil(
            ["cp", "-", "%s" % suri(bucket_uri, "foo")], stdin="bar", return_stderr=True
        )
    ...
    def test_streaming_from_fifo_to_object(self):
        ...
        write_thread = threading.Thread(
            target=_WriteContentsToFifo, args=(object_contents, fifo_path)
        )
        write_thread.start()
    ...
    def test_streaming_from_fifo_to_stdout(self):
        ...
        write_thread = threading.Thread(
            target=_WriteContentsToFifo, args=(contents, fifo_path)
        )
        write_thread.start()
        read_thread = threading.Thread(
            target=self._CpWithFifoViaGsUtilAndAppendOutputToList,
    ...
    def test_detect_content_type(self):
        ...
        def _Check1():
            ...
            if IS_WINDOWS:
                self.assertTrue(
                    re.search(r"Content-Type:\s+audio/x-mpg", stdout)
                    or re.search(r"Content-Type:\s+audio/mpeg", stdout)
                )
            else:
    ...
    def test_content_type_override_default(self):
        ...
        self.RunGsUtil(
            ["-h", "Content-Type:", "cp", self._get_test_file("test.mp3"), dsturi]
        )
        ...
        self.RunGsUtil(
            ["-h", "Content-Type:", "cp", self._get_test_file("test.gif"), dsturi]
        )
    ...
    def test_versioning(self):
        ...
        self.RunGsUtil(
            ["cp", "%s#%s" % (k2_uri.versionless_uri, g1), k2_uri.versionless_uri]
        )
        self.RunGsUtil(["cp", k2_uri.versionless_uri, fpath])
        with open(fpath, "rb") as f:
            self.assertEqual(f.read(), b"data1")
        ...
        stderr = self.RunGsUtil(
            ["cp", fpath, k2_uri.uri], return_stderr=True, expected_status=1
        )
    ...
    def test_recursive_copying_versioned_bucket(self):
        ...
        v1_uri = self.CreateObject(
            bucket_uri=bucket1_uri, object_name="k", contents=b"data0"
        )
        self.CreateObject(
        ...
        def _Check2():
            ...
            listing1 = self.RunGsUtil(
                ["ls", "-la", suri(bucket1_uri)], return_stdout=True
            ).split("\n")
            listing2 = self.RunGsUtil(
                ["ls", "-la", suri(bucket2_uri)], return_stdout=True
            ).split("\n")
        ...
        def _Check3():
            ...
            listing2 = self.RunGsUtil(
                ["ls", "-la", suri(bucket3_uri)], return_stdout=True
            ).split("\n")
            # 2 lines of listing output, 1 summary line, 1 empty line from \n split.
            self.assertEquals(len(listing1), 4)
    ...
    def test_cp_generation_zero_match(self):
        ...
        stderr = self.RunGsUtil(
            ["-h", gen_match_header, "cp", fpath1, suri(bucket_uri)],
            return_stderr=True,
            expected_status=1,
        )
    ...
    def test_cp_v_generation_match(self):
        ...
        stderr = self.RunGsUtil(
            ["-h", gen_match_header, "cp", fpath1, suri(k1_uri)],
            return_stderr=True,
            expected_status=1,
        )
        ...
        # server.
        stderr = self.RunGsUtil(
            ["-h", gen_match_header, "cp", "-n", fpath1, suri(k1_uri)],
            return_stderr=True,
            expected_status=1,
        )
    ...
    @unittest.skipUnless(HAS_S3_CREDS, "Test requires both S3 and GS credentials")
    @unittest.skip(
        "This test performs a large copy but remains here for " "debugging purposes."
    )
    ...
    def test_gs_to_s3_multipart_cp(self):
        ...
        gs_key = self.CreateObject(
            bucket_uri=gs_bucket, contents=b"b" * num_bytes, prefer_json_api=True
        )
        self.RunGsUtil(
            [
                "-o",
    ...
    @unittest.skip(
        "This test is slow due to creating many objects, "
    ...
    def test_daisy_chain_cp(self):
        ...
        stderr = self.RunGsUtil(
            ["cp", "-Dpn", suri(key_uri), suri(bucket2_uri)], return_stderr=True
        )
    ...
    @unittest.skipUnless(
        not HAS_GS_PORT,
        "gs_port is defined in config which can cause "
    ...
    def test_daisy_chain_cp_download_failure(self):
        ...
        key_uri = self.CreateObject(
            bucket_uri=bucket1_uri, contents=b"a" * self.halt_size
        )
        ...
        with SetBotoConfigForTest([boto_config_for_test]):
            ...
            self.assertEqual(
                stderr.count(
                    "ResumableDownloadException: Artifically halting download"
                ),
                3,
            )
    ...
    def test_streaming_gzip_upload(self):
        ...
        self.assertIn(
            "gzip compression is not currently supported on streaming uploads", stderr
        )
    ...
    def test_canned_acl_cp(self):
        ...
        # the ACLs.
        self.RunGsUtil(["acl", "set", "public-read", suri(key_uri)])
        public_read_acl = self.RunGsUtil(
            ["acl", "get", suri(key_uri)], return_stdout=True
        )
    ...
    def test_canned_acl_upload(self):
        ...
        with SetBotoConfigForTest([boto_config_for_test]):
            ...
            self.RunGsUtil(
                ["cp", "-a", "public-read", resumable_fpath, suri(bucket1_uri)]
            )
    ...
    def test_copy_bucket_to_bucket(self):
        ...
        self.CreateObject(
            bucket_uri=src_bucket_uri, object_name="obj0", contents=b"abc"
        )
        self.CreateObject(
            bucket_uri=src_bucket_uri, object_name="obj1", contents=b"def"
        )
        ...
        def _CopyAndCheck():
            ...
            stdout = self.RunGsUtil(
                ["ls", "-R", dst_bucket_uri.uri], return_stdout=True
            )
            self.assertIn(
    ...
    def test_copy_bucket_to_dir(self):
        ...
        def _CopyAndCheck():
            ...
            self.assertEqual(
                os.path.join(dst_dir, src_bucket_uri.bucket_name, "obj0"), dir_list[0]
            )
            self.assertEqual(
                os.path.join(dst_dir, src_bucket_uri.bucket_name, "obj1"), dir_list[1]
            )
    ...
    @SkipForS3(
        "The boto lib used for S3 does not handle objects "
    ...
    def test_cp_manifest_upload_unicode(self):
        return self._ManifestUpload(
            "foo-unicöde".encode(UTF8),
    ...
    def test_copy_unicode_non_ascii_filename(self):
        ...
        with SetBotoConfigForTest(
            [("GSUtil", "resumable_threshold", str(START_CALLBACK_PER_BYTES * 3))]
        ):
            self.RunGsUtil(["cp", fpath, suri(key_uri)], return_stderr=True)
            stdout = self.RunGsUtil(["cat", suri(key_uri)], return_stdout=True)
            self.assertEquals(stdout.encode("ascii"), file_contents)
    ...
    def test_gzip_upload_and_download(self):
        ...
        self.RunGsUtil(
            ["cp", "-z", "js, html", os.path.join(tmpdir, "test.*"), suri(bucket_uri)]
        )
        self.AssertNObjectsInBucket(bucket_uri, 3)
    ...
    def test_gzip_transport_encoded_upload_and_download(self):
        ...
        def _create_test_data():  # pylint: disable=invalid-name
            ...
            for filename in ("test.html", "test.js", "test.txt"):
                local_uris.append(
                    self.CreateTempFile(
                        file_name=filename, tmpdir=tmpdir, contents=contents
                    )
                )
        ...
        def _assert_stored_uncompressed(
            bucket_uri, contents=b"x" * 10000
        ):  # pylint: disable=invalid-name
    ...
    def test_gzip_transport_encoded_parallel_upload_non_resumable(self):
        ...
        with SetBotoConfigForTest([("GSUtil", "resumable_threshold", str(ONE_KIB))]):
            ...
            self.assertIn(
                "send: Using gzip transport encoding for the request.", stderr
            )
    ...
    def test_gzip_transport_encoded_all_upload_and_download(self):
        ...
        # Ensure the correct files were marked for compression.
        self.assertIn(
            "Using compressed transport encoding for file://%s." % (local_uri1), stderr
        )
        self.assertIn(
            "Using compressed transport encoding for file://%s." % (local_uri2), stderr
        )
        # Ensure the progress logger sees a gzip encoding.
    ...
    def test_cp_without_read_access(self):
        ...
        with self.SetAnonymousBotoCreds():
            stderr = self.RunGsUtil(
                ["cp", suri(object_uri), "foo"], return_stderr=True, expected_status=1
            )
    ...
    def test_cp_download_encrypted_object_without_key(self):
        ...
        stderr = self.RunGsUtil(
            ["cp", suri(object_uri), suri(fpath)], expected_status=1, return_stderr=True
        )
        self.assertIn(
    ...
    def test_cp_resumable_upload_gzip_encoded_break(self):
        ...
        with SetBotoConfigForTest([boto_config_for_test]):
            ...
            self.assertIn("Artifically halting upload", stderr)
            stderr = self.RunGsUtil(
                ["-D", "cp", "-J", local_uri, suri(bucket_uri)], return_stderr=True
            )
            self.assertIn("Resuming upload", stderr)
    ...
    def test_cp_resumable_streaming_upload_retry(self):
        ...
        boto_configs_for_test = [
            ("GSUtil", "json_resumable_chunk_size", str(256 * ONE_KIB)),
            ("Boto", "num_retries", "2"),
        ]
    ...
    def test_cp_preserve_no_owner(self):
        ...
        with self.SetAnonymousBotoCreds():
            stderr = self.RunGsUtil(
                ["cp", "-p", suri(object_uri), suri(bucket_uri, "foo")],
                return_stderr=True,
                expected_status=1,
            )
    ...
    def test_cp_progress_callbacks(self):
        ...
        with SetBotoConfigForTest([boto_config_for_test]):
            ...
            self.assertEquals(1, stderr.count(final_progress_callback))
        stderr = self.RunGsUtil(
            ["cp", suri(bucket_uri, "foo"), fpath], return_stderr=True
        )
        self.assertEquals(1, stderr.count(final_progress_callback))
    ...
    def test_resumable_upload_break_leaves_tracker(self):
        ...
        with SetBotoConfigForTest([boto_config_for_test]):
            ...
            try:
                ...
                self.assertIn("Artifically halting upload", stderr)
                self.assertTrue(
                    os.path.exists(tracker_filename),
    ...
    def test_cp_resumable_upload_break_file_size_change(self):
        ...
        tmp_dir = self.CreateTempDir()
        fpath = self.CreateTempFile(
            file_name="foo", tmpdir=tmp_dir, contents=b"a" * self.halt_size
        )
        test_callback_file = self.CreateTempFile(
            contents=pickle.dumps(HaltingCopyCallbackHandler(True, 5))
        )
        ...
        with SetBotoConfigForTest([boto_config_for_test]):
            ...
            self.assertIn("Artifically halting upload", stderr)
            fpath = self.CreateTempFile(
                file_name="foo", tmpdir=tmp_dir, contents=b"a" * self.halt_size * 2
            )
            stderr = self.RunGsUtil(
                ["cp", fpath, suri(bucket_uri)], expected_status=1, return_stderr=True
            )
    ...
    def test_cp_resumable_upload_break_file_content_change(self):
        ...
        tmp_dir = self.CreateTempDir()
        fpath = self.CreateTempFile(
            file_name="foo", tmpdir=tmp_dir, contents=b"a" * ONE_KIB * ONE_KIB
        )
        test_callback_file = self.CreateTempFile(
            contents=pickle.dumps(HaltingCopyCallbackHandler(True, int(ONE_KIB) * 512))
        )
        ...
        with SetBotoConfigForTest(
            ...
            self.assertIn("Artifically halting upload", stderr)
            fpath = self.CreateTempFile(
                file_name="foo", tmpdir=tmp_dir, contents=b"b" * ONE_KIB * ONE_KIB
            )
            stderr = self.RunGsUtil(
                ["cp", fpath, suri(bucket_uri)], expected_status=1, return_stderr=True
            )
    ...
    def test_cp_resumable_upload_break_file_smaller_size(self):
        ...
        with SetBotoConfigForTest(
            ...
            self.assertIn("Artifically halting upload", stderr)
            fpath = self.CreateTempFile(
                file_name="foo", tmpdir=tmp_dir, contents=b"a" * ONE_KIB
            )
            stderr = self.RunGsUtil(
                ["cp", fpath, suri(bucket_uri)], expected_status=1, return_stderr=True
            )
    ...
    def test_cp_composite_encrypted_upload_restart(self):
        ...
        existing_component = ObjectFromTracker(
            existing_component_name, str(object_uri.generation)
        )
        existing_components = [existing_component]
        enc_key_sha256 = TEST_ENCRYPTION_KEY1_SHA256_B64
        WriteParallelUploadTrackerFile(
        ...
        try:
            ...
            with SetBotoConfigForTest(
                ...
                self.assertIn(
                    ...
                    "Deleting old components and restarting upload",
                    stderr,
                )
    ...
    def _test_cp_resumable_download_break_helper(
    ...
    def test_cp_resumable_download_etag_differs(self):
        ...
        bucket_uri = self.CreateBucket()
        object_uri = self.CreateObject(
            bucket_uri=bucket_uri, object_name="foo", contents=b"abc" * self.halt_size
        )
        fpath = self.CreateTempFile()
        test_callback_file = self.CreateTempFile(
            contents=pickle.dumps(HaltingCopyCallbackHandler(False, 5))
    ...
    def test_cp_resumable_download_file_larger(self):
        ...
        fpath = self.CreateTempFile()
        object_uri = self.CreateObject(
            bucket_uri=bucket_uri, object_name="foo", contents=b"a" * self.halt_size
        )
        test_callback_file = self.CreateTempFile(
            contents=pickle.dumps(HaltingCopyCallbackHandler(False, 5))
        )
        ...
        with SetBotoConfigForTest([boto_config_for_test]):
            ...
            with open(fpath + "_.gstmp", "w") as larger_file:
                for _ in range(self.halt_size * 2):
                    larger_file.write("a")
            stderr = self.RunGsUtil(
                ["cp", suri(object_uri), fpath], expected_status=1, return_stderr=True
            )
            self.assertNotIn("Resuming download", stderr)
    ...
    def test_cp_resumable_download_content_differs(self):
        ...
        object_uri = self.CreateObject(
            bucket_uri=bucket_uri, object_name="foo", contents=b"efgh" * ONE_KIB
        )
        ...
        self.assertEqual(
            len(etag_match.groups()), 1, "Did not match expected single ETag"
        )
    ...
    def test_cp_resumable_download_content_matches(self):
        ...
        object_uri = self.CreateObject(
            bucket_uri=bucket_uri, object_name="foo", contents=matching_contents
        )
        stdout = self.RunGsUtil(["ls", "-L", suri(object_uri)], return_stdout=True)
        etag_match = re.search(r"\s*ETag:\s*(.*)", stdout)
        self.assertIsNotNone(etag_match, "Could not get object ETag")
        ...
        try:
            ...
            with SetBotoConfigForTest([boto_config_for_test]):
                stderr = self.RunGsUtil(
                    ["cp", suri(object_uri), fpath], return_stderr=True
                )
                self.assertIn("Download already complete", stderr)
    ...
    def test_cp_resumable_download_gzip(self):
        ...
        with SetBotoConfigForTest([boto_config_for_test]):
            ...
            sliced_download = (
                len(contents) > sliced_download_threshold
                and sliced_download_threshold > 0
                and UsingCrcmodExtension()
            )
            if sliced_download:
    ...
    def test_cp_resumable_download_check_hashes_never(self):
        ...
        object_uri = self.CreateObject(
            bucket_uri=bucket_uri, object_name="foo", contents=contents
        )
        fpath = self.CreateTempFile()
        test_callback_file = self.CreateTempFile(
            contents=pickle.dumps(HaltingCopyCallbackHandler(False, 5))
    ...
    def test_cp_sliced_download(self):
        ...
        bucket_uri = self.CreateBucket()
        object_uri = self.CreateObject(
            bucket_uri=bucket_uri, object_name="foo", contents=b"abc" * ONE_KIB
        )
        fpath = self.CreateTempFile()
    ...
    def test_cp_unresumable_sliced_download(self):
        ...
        bucket_uri = self.CreateBucket()
        object_uri = self.CreateObject(
            bucket_uri=bucket_uri, object_name="foo", contents=b"abcd" * self.halt_size
        )
        fpath = self.CreateTempFile()
        test_callback_file = self.CreateTempFile(
            contents=pickle.dumps(HaltingCopyCallbackHandler(False, 5))
        ...
        with SetBotoConfigForTest(boto_config_for_test):
            ...
            with open(fpath, "rb") as f:
                self.assertEqual(
                    f.read(), b"abcd" * self.halt_size, "File contents differ"
                )
    ...
    def test_cp_sliced_download_resume(self):
        ...
        with SetBotoConfigForTest(boto_config_for_test):
            ...
            with open(fpath, "rb") as f:
                self.assertEqual(
                    f.read(), b"abc" * self.halt_size, "File contents differ"
                )
    ...
    def test_cp_sliced_download_resume_content_differs(self):
        ...
        with SetBotoConfigForTest(boto_config_for_test):
            ...
            stderr = self.RunGsUtil(
                ["cp", suri(object_uri), fpath], return_stderr=True, expected_status=1
            )
            self.assertIn("Resuming download", stderr)
            self.assertIn("doesn't match cloud-supplied digest", stderr)
    ...
    def test_cp_resumable_upload_start_over_http_error(self):
        for start_over_error in (
            ...
            410,
        ):  # If the service tells us to restart the upload from scratch.
    ...
    def test_rewrite_cp_resume_source_changed(self):
        ...
        try:
            ...
            # Copy the new object.
            gsutil_api.CopyObject(
                src_obj_metadata2, dst_obj_metadata, max_bytes_per_call=ONE_MIB
            )
    ...
    def test_rewrite_cp_resume_command_changed(self):
        ...
        try:
            ...
            self.assertTrue(
                found_public_acl, "New object was not written with a public ACL."
            )
        finally:
            # Clean up if something went wrong.
            DeleteTrackerFile(tracker_file_name)
    ...
    def test_cp_minus_s_to_non_cloud_dest_fails(self):
        ...
        stderr = self.RunGsUtil(
            ["cp", "-s", "standard", local_file, dest_dir],
            expected_status=1,
            return_stderr=True,
        )
        self.assertIn(
    ...
    def test_kms_key_correctly_applied_to_dst_obj_from_src_with_no_key(self):
        ...
        obj_uri = self.CreateObject(
            bucket_uri=bucket_uri, object_name=obj1_name, contents=b"foo"
        )
    ...
    def test_cp_minus_m_streaming_upload(self):
        ...
        stderr = self.RunGsUtil(
            ["-m", "cp", "-", "file"], return_stderr=True, expected_status=1
        )
        self.assertIn(
            "CommandException: Cannot upload from a stream when using gsutil -m", stderr
        )
...
class TestCpUnitTests(testcase.GsUtilUnitTestCase):
    ...
    def test_object_and_prefix_same_name(self):
        ...
        self.CreateObject(bucket_uri=bucket_uri, object_name="foo/bar", contents=b"bar")
        fpath = self.CreateTempFile()
    ...
    def test_cp_upload_respects_no_hashes(self):
        ...
        with SetBotoConfigForTest([("GSUtil", "check_hashes", "never")]):
            log_handler = self.RunCommand(
                "cp", [fpath, suri(bucket_uri)], return_log_handler=True
            )
        warning_messages = log_handler.messages["warning"]
