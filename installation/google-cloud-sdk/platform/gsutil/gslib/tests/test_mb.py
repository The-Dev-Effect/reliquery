...
BUCKET_LOCK_SKIP_MSG = (
    "gsutil does not support bucket lock operations for " "S3 buckets."
)
...
class TestMb(testcase.GsUtilIntegrationTestCase):
    ...
    def test_mb_bucket_exists(self):
        ...
        stderr = self.RunGsUtil(
            ["mb", suri(bucket_uri)], expected_status=1, return_stderr=True
        )
        self.assertIn("already exists", stderr)

    def test_non_ascii_project_fails(self):
        stderr = self.RunGsUtil(
            ["mb", "-p", "ã", "gs://fobarbaz"], expected_status=1, return_stderr=True
        )
    ...
    def test_create_with_pap_invalid_arg(self):
        ...
        stderr = self.RunGsUtil(
            ["mb", "--pap", "invalid_arg", suri(bucket_uri)],
            expected_status=1,
            return_stderr=True,
        )
    ...
    def test_create_with_rpo_async_turbo_fails_for_regional_bucket(self):
        ...
        stderr = self.RunGsUtil(
            ["mb", "-l", "us-central1", "--rpo", "ASYNC_TURBO", suri(bucket_uri)],
            return_stderr=True,
            expected_status=1,
        )
    ...
    def test_create_with_rpo_incorrect_value_raises_error(self):
        ...
        stderr = self.RunGsUtil(
            ["mb", "-l", "nam4", "--rpo", "incorrect_value", suri(bucket_uri)],
            return_stderr=True,
            expected_status=1,
        )
        self.assertIn(
            "Invalid value for --rpo. Must be one of: (ASYNC_TURBO|DEFAULT),"
            " provided: incorrect_value",
            stderr,
        )
    ...
    def test_create_with_invalid_placement_flag_raises_error(self):
        ...
        stderr = self.RunGsUtil(
            ["mb", "--placement", "invalid_reg1,invalid_reg2", suri(bucket_uri)],
            return_stderr=True,
            expected_status=1,
        )
        self.assertIn(
    ...
    def test_create_with_incorrect_number_of_placement_values_raises_error(self):
        ...
        # Location nam4 is used for dual-region.
        stderr = self.RunGsUtil(
            ["mb", "--placement", "val1,val2,val3", suri(bucket_uri)],
            return_stderr=True,
            expected_status=1,
        )
        self.assertIn(
            "CommandException: Please specify two regions separated by comma"
            " without space. Specified: val1,val2,val3",
            stderr,
        )
