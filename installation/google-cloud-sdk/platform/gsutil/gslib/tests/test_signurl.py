...
SERVICE_ACCOUNT = boto.config.get_value("GSUtil", "test_impersonate_service_account")
...
class TestSignUrl(testcase.GsUtilIntegrationTestCase):
    ...
    def testSignUrlInvalidDurationWithUseServiceAccount(self):
        ...
        stderr = self.RunGsUtil(
            ["signurl", "-d", "13h", "-u", "gs://uri"],
            return_stderr=True,
            expected_status=1,
        )
        self.assertIn(
            "CommandException: Max valid duration allowed is 12:00:00", stderr
        )
    ...
    def testSignUrlWithWildcard(self):
        ...
        for obj_name in objs:
            obj_urls.append(
                self.CreateObject(bucket_uri=bucket, object_name=obj_name, contents=b"")
            )

        stdout = self.RunGsUtil(
            ["signurl", "-p", "notasecret", self._GetKsFile(), suri(bucket) + "/*"],
            return_stdout=True,
        )
    ...
    @unittest.skipUnless(
        SERVICE_ACCOUNT, "Test requires test_impersonate_service_account."
    )
    ...
    def testSignUrlWithServiceAccount(self):
        ...
        self.assertIn(
            "All API calls will be executed as [%s]" % SERVICE_ACCOUNT, stderr
        )

    def testSignUrlOfNonObjectUrl(self):
        """Tests the signurl output of a non-existent file."""
        self.RunGsUtil(
            ["signurl", self._GetKsFile(), "gs://"],
            expected_status=1,
            stdin="notasecret",
        )
...
class UnitTestSignUrl(testcase.GsUtilUnitTestCase):
    ...
    def testSignPutUsingImersonatedServiceAccount(
        self, mock_api_messages, mock_apiclient
    ):
