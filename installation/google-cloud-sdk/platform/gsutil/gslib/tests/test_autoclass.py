...
class TestAutoclassUnit(testcase.GsUtilUnitTestCase):
    def test_set_too_few_arguments_fails(self):
        with self.assertRaisesRegexp(
            exception.CommandException, "command requires at least"
        ):
    ...
    def test_invalid_subcommand_fails(self):
        with self.assertRaisesRegexp(exception.CommandException, "Invalid subcommand"):
    ...
    def test_gets_multiple_buckets_with_wildcard(self):
        ...
        stdout = self.RunCommand(
            "autoclass", ["get", "gs://bucket*"], return_stdout=True
        )
...
class TestAutoclassE2E(testcase.GsUtilIntegrationTestCase):
    ...
    def test_off_on_default_buckets(self):
        ...
        stdout = self.RunGsUtil(
            self._get_autoclass_cmd + [suri(bucket_uri)], return_stdout=True
        )
    ...
    def test_turning_on_and_off(self):
        ...
        stdout = self.RunGsUtil(
            self._set_autoclass_cmd + ["on", suri(bucket_uri)], return_stdout=True
        )
        self.assertRegex(
            stdout,
            re.escape(
                "Setting Autoclass on for {}\n".format(str(bucket_uri).rstrip("/"))
            ),
        )
        ...
        stdout = self.RunGsUtil(
            self._set_autoclass_cmd + ["off", suri(bucket_uri)], return_stdout=True
        )
        self.assertRegex(
            stdout,
            re.escape(
                "Setting Autoclass off for {}\n".format(str(bucket_uri).rstrip("/"))
            ),
        )
    ...
    def test_multiple_buckets(self):
        ...
        stdout = self.RunGsUtil(
            self._get_autoclass_cmd + [suri(bucket_uri1), suri(bucket_uri2)],
            return_stdout=True,
        )
        output_regex = (
            r"{}:\n"
            ...
            r"{}:\n"
            r"  Enabled: False\n"
            r"  Toggle Time: None".format(suri(bucket_uri1), suri(bucket_uri2))
        )
    ...
    def test_xml_fails(self):
        ...
        with SetBotoConfigForTest(boto_config_hmac_auth_only):
            ...
            stderr = self.RunGsUtil(
                self._set_autoclass_cmd + ["on", bucket_uri],
                return_stderr=True,
                expected_status=1,
            )
            ...
            stderr = self.RunGsUtil(
                self._get_autoclass_cmd + [bucket_uri],
                return_stderr=True,
                expected_status=1,
            )
            self.assertIn("command can only be with the Cloud Storage JSON API", stderr)
    ...
    def test_s3_fails(self):
        bucket_uri = self.CreateBucket()
        stderr = self.RunGsUtil(
            self._set_autoclass_cmd + ["on", suri(bucket_uri)],
            return_stderr=True,
            expected_status=1,
        )
        ...
        stderr = self.RunGsUtil(
            self._get_autoclass_cmd + [suri(bucket_uri)],
            return_stderr=True,
            expected_status=1,
        )
        self.assertIn("command can only be used for GCS Buckets", stderr)
