...
class TestCredsConfig(testcase.GsUtilUnitTestCase):
    ...
    @mock.patch(
        "gslib.third_party.iamcredentials_apitools"
    ...
    def testImpersonationBlockedByIamCredentialsApiErrors(
        ...
        with SetBotoConfigForTest(
            ...
            mock_iam_creds_generate_access_token.side_effect = _Make403(
    ...
    def testImpersonationCredentialsFromOptionOverridesOthers(self):
        with SetBotoConfigForTest(
            ...
            with SetEnvironmentForTest(
                ...
                stderr = self.RunGsUtil(
                    ["-i", SERVICE_ACCOUNT, "ls", "gs://pub"], return_stderr=True
                )
                self.assertIn(
                    "API calls will be executed as [%s" % SERVICE_ACCOUNT, stderr
                )
