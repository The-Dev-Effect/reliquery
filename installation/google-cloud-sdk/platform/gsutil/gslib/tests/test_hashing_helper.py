...
class TestHashingFileUploadWrapper(testcase.GsUtilUnitTestCase):
    ...
    def testReadToEOF(self):
        ...
        with open(tmp_file, "rb") as stream:
            wrapper = HashingFileUploadWrapper(
                stream, digesters, {"md5": GetMd5}, self._dummy_url, self.logger
            )
            wrapper.read()
        with open(tmp_file, "rb") as stream:
    ...
    def testInvalidSeekAway(self):
        ...
        with open(tmp_file, "rb") as stream:
            ...
            try:
                ...
                self.assertIn(
                    "Read called on hashing file pointer in an unknown position", str(e)
                )
