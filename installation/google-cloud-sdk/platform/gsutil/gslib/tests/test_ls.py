...
KMS_XML_SKIP_MSG = (
    "gsutil does not support KMS operations for S3 buckets, "
    "or listing KMS keys with the XML API."
)
BUCKET_LOCK_SKIP_MSG = (
    "gsutil does not support bucket lock operations for "
...
class TestLsUnit(testcase.GsUtilUnitTestCase):
    ...
    def test_one_object_with_L_storage_class_update(self):
        ...
        self.assertEqual(
            time_created, datetime.strftime(current_time, "%a, %d %b %Y %H:%M:%S GMT")
        )
    ...
    def test_satisfies_pzs_is_displayed_if_present(self, mock_wildcard):
        ...
        with SetBotoConfigForTest([("GSUtil", "check_hashes", "never")]):
            stdout = self.RunCommand(
                "ls", ["-Lb", suri(bucket_uri)], return_stdout=True
            )
...
class TestLs(testcase.GsUtilIntegrationTestCase):
    ...
    def test_list_missing_object(self):
        ...
        bucket_uri = self.CreateBucket()
        stderr = self.RunGsUtil(
            ["ls", suri(bucket_uri, "missing")], return_stderr=True, expected_status=1
        )
    ...
    def test_one_object_with_generation(self):
        ...
        object2 = self.CreateObjectJson(
            bucket_name=bucket.name, object_name=object1.name, contents=b"2"
        )
        object_url_string1 = "gs://{}/{}#{}".format(
            object1.bucket, object1.name, object1.generation
        )
        object_url_string2 = "gs://{}/{}#{}".format(
            object2.bucket, object2.name, object2.generation
        )
    ...
    def test_subdir(self):
        ...
        def _Check1():
            stdout = self.RunGsUtil(
                ["ls", "%s/dir" % suri(bucket_uri)], return_stdout=True
            )
            self.assertEqual("%s\n" % suri(k2_uri), stdout)
    ...
    def test_etag(self):
        ...
        def _Check3():
            stdout = self.RunGsUtil(
                ["ls", "-ale", suri(bucket_uri)], return_stdout=True
            )
            if self.test_api == ApiSelector.XML:
                self.assertIn(etag, stdout)
            else:
    ...
    def test_list_sizes(self):
        ...
        def _Check5():
            stdout = self.RunGsUtil(
                ["ls", "-alh", suri(bucket_uri)], return_stdout=True
            )
            self.assertIn("2 KiB", stdout)
    ...
    def test_wildcard_prefix(self):
        ...
        self.CreateObject(
            bucket_uri=bucket_uri, object_name=wildcard_folder_object, contents=b"foo"
        )
        self.CreateObject(
            bucket_uri=bucket_uri, object_name=object_matching_folder, contents=b"foo"
        )
        self.AssertNObjectsInBucket(bucket_uri, 2)
        stderr = self.RunGsUtil(
            ["ls", suri(bucket_uri, "wildcard*")], return_stderr=True, expected_status=1
        )
        self.assertIn(
            "Cloud folder %s%s contains a wildcard" % (suri(bucket_uri), "/wildcard*/"),
            stderr,
        )
        ...
        def _Check():
            stdout = self.RunGsUtil(
                ["ls", "-l", suri(bucket_uri, "**")], return_stdout=True
            )
    ...
    def test_list_encrypted_object(self):
        ...
        with SetBotoConfigForTest([("GSUtil", "encryption_key", TEST_ENCRYPTION_KEY1)]):
            ...
            def _ListExpectDecrypted():
                stdout = self.RunGsUtil(
                    ["ls", "-L", suri(object_uri)], return_stdout=True
                )
    ...
    def test_list_mixed_encryption(self):
        ...
        self.CreateObject(
            bucket_uri=bucket_uri, object_name="foo5", contents=TEST_ENCRYPTION_CONTENT5
        )
    ...
    def test_non_ascii_project_fails(self):
        stderr = self.RunGsUtil(
            ["ls", "-p", "ã", "gs://fobarbaz"], expected_status=1, return_stderr=True
        )
    ...
    def test_list_Lb_displays_custom_dual_region_placement_info(self):
        ...
        self.RunGsUtil(
            ["mb", "--placement", "us-central1,us-west1", bucket_name],
            expected_status=0,
        )
        stdout = self.RunGsUtil(["ls", "-Lb", bucket_name], return_stdout=True)
        self.assertRegex(
            stdout, r"Placement locations:\t\t\['US-CENTRAL1', 'US-WEST1'\]"
        )
