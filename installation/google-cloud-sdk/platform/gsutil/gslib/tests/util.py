...
ORPHANED_FILE = (
    "This sync will orphan file(s), please fix their permissions "
    "before trying again."
)
...
HAS_S3_CREDS = boto.config.get(
    "Credentials", "aws_access_key_id", None
) and boto.config.get("Credentials", "aws_secret_access_key", None)
    ...
    try:
        ...
        if use_existing_config:
            for boto_config in boto_config_list:
                ...
                _SetBotoConfig(
                    boto_config[0], boto_config[1], boto_value, revert_configs
                )
            with open(tmp_filename, "w") as tmp_file:
...
class HaltOneComponentCopyCallbackHandler(object):
    ...
    def call(self, current_progress_byte, total_size_unused):
        ...
        if (
            self._last_progress_byte is not None
            and self._last_progress_byte < self._halt_at_byte < current_progress_byte
        ):
