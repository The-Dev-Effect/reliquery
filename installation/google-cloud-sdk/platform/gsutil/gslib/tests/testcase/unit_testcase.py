...
class GsUtilUnitTestCase(base.GsUtilTestCase):
    ...
    def _test_wildcard_iterator(cls, uri_or_str, debug=0):
        ...
        return wildcard_iterator.CreateWildcardIterator(uri_string, cls.MakeGsUtilApi())
    ...
    def _test_storage_uri(uri_str, default_scheme="file", debug=0, validate=True):
        ...
        return boto.storage_uri(
            uri_str, default_scheme, debug, validate, util.GSMockBucketStorageUri
        )

    def CreateBucket(
        self, bucket_name=None, test_objects=0, storage_class=None, provider="gs"
    ):
