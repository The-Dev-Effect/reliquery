...
class TestCommandRunnerUnitTests(testcase.unit_testcase.GsUtilUnitTestCase):
    ...
    def setUp(self):
        ...
        self.gsutil_tarball_uri = self.CreateObject(
            bucket_uri=self.pub_bucket_uri, object_name="gsutil.tar.gz", contents="foo"
        )
    ...
    def test_py3_prompt_on_py2(self):
        ...
        with mock.patch.object(sys, "version_info") as version_info:
            version_info.major = 2
            self.assertEqual(
                True, self.command_runner.MaybePromptForPythonUpdate("ver")
            )
    ...
    def test_py3_prompt_on_py3(self):
        ...
        with mock.patch.object(sys, "version_info") as version_info:
            ...
            self.assertEqual(
                False, self.command_runner.MaybePromptForPythonUpdate("ver")
            )
    ...
    def test_no_tracker_file_version_old(self):
        ...
        self.assertEqual(
            expected, self.command_runner.MaybeCheckForAndOfferSoftwareUpdate("ls", 0)
        )
    ...
    def test_invalid_commands(self):
        ...
        self.assertEqual(
            False, self.command_runner.MaybeCheckForAndOfferSoftwareUpdate("update", 0)
        )
    ...
    def test_command_argument_parser_setup_invalid_completer(self):
        ...
        command_map = {
            FakeCommandWithInvalidCompleter.command_spec.command_name: FakeCommandWithInvalidCompleter()
        }
    ...
    def test_command_argument_parser_setup_nested_argparse_arguments(self):
        command_map = {
            FakeCommandWithNestedArguments.command_spec.command_name: FakeCommandWithNestedArguments(),
        }
        runner = CommandRunner(
            bucket_storage_uri_class=self.mock_bucket_storage_uri,
            gsutil_api_class_map_factory=self.mock_gsutil_api_class_map_factory,
        ...
        self.assertEqual(
            cur_subparser.name, FakeCommandWithNestedArguments.command_spec.command_name
        )

        # Go one level further, checking the current subparser's first subparser.
        cur_subparser = cur_subparser.subparsers.parsers[0]
        self.assertEqual(
            cur_subparser.name, FakeCommandWithNestedArguments.subcommand_name
        )

        cur_subparser = cur_subparser.subparsers.parsers[0]
        self.assertEqual(
            cur_subparser.name, FakeCommandWithNestedArguments.subcommand_subname
        )
    ...
    def test_command_argument_parser_setup_completers(self):
        ...
        command_map = {
            FakeCommandWithCompleters.command_spec.command_name: FakeCommandWithCompleters()
        }
        ...
        self.assertEqual(
            CloudOrLocalObjectCompleter, type(subparser.arguments[2].completer)
        )
        ...
        self.assertEqual(
            LocalObjectOrCannedACLCompleter, type(subparser.arguments[5].completer)
        )
...
class TestCommandRunnerIntegrationTests(testcase.GsUtilIntegrationTestCase):
    ...
    def test_lookup_version_without_credentials(self):
        ...
        with SetBotoConfigForTest(
            [("GSUtil", "software_update_check_period", "1")], use_existing_config=False
        ):
