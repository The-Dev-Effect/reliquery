...
class TestRm(testcase.GsUtilIntegrationTestCase):
    ...
    def _RunRemoveCommandAndCheck(
        ...
        if not self.multiregional_buckets and self.default_provider == "gs":
            stderr = self.RunGsUtil(
                command_and_args, return_stderr=True, expected_status=None, stdin=stdin
            )
            ...
            if "-q" not in command_and_args:
                if "-m" in command_and_args:
                    self.assertIn(
                        "[%d/%d objects]" % (num_objects, num_objects), stderr
                    )
                else:
            ...
            def _RunRmCommandAndCheck():
                ...
                if (
                    NO_URLS_MATCHED_PREFIX in stderr
                    or "409 BucketNotEmpty" in stderr
                    or "409 VersionedBucketNotEmpty" in stderr
                ):
                    update_lines = False
    ...
    def test_remove_all_versions_recursive_on_bucket(self):
        ...
        def _Check():
            ...
            stderr = self.RunGsUtil(
                ["ls", "-a", suri(bucket_uri)], return_stderr=True, expected_status=1
            )
    ...
    def test_rm_seek_ahead_stdin_args(self):
        ...
        with SetBotoConfigForTest(
            ...
            stderr = self.RunGsUtil(
                ["-m", "rm", "-I"], stdin=suri(object_uri), return_stderr=True
            )
    ...
    def test_missing_first_force(self):
        ...
        if self.multiregional_buckets:
            self.AssertNObjectsInBucket(bucket_uri, 1)
        self.RunGsUtil(
            ["rm", "%s" % suri(bucket_uri, "missing"), suri(object_uri)],
            expected_status=1,
        )
        stderr = self.RunGsUtil(
    ...
    def test_some_missing(self):
        ...
        stderr = self.RunGsUtil(
            ["rm", "-a", suri(key_uri), "%s" % suri(bucket_uri, "missing")],
            return_stderr=True,
            expected_status=1,
        )
        self.assertEqual(stderr.count("Removing %s://" % self.default_provider), 1)
    ...
    def test_folder_objects_deleted(self):
        ...
        def _RemoveAndCheck():
            self.RunGsUtil(
                ["rm", "-r", "%s" % suri(bucket_uri, "abc")], expected_status=None
            )
            self.AssertNObjectsInBucket(bucket_uri, 0, versioned=True)
    ...
    def test_recursive_bucket_rm(self):
        ...
        self._RunRemoveCommandAndCheck(
            ["rm", suri(bucket_uri, "**")], objects_to_remove=["%s" % final_uri]
        )

        stderr = self.RunGsUtil(
            ["rb", suri(bucket_uri)], return_stderr=True, expected_status=1
        )
    ...
    def test_rm_object_with_slashes(self):
        ...
        bucket_uri = self.CreateVersionedBucket()
        ouri1 = self.CreateObject(
            bucket_uri=bucket_uri, object_name="h/e/l//lo", contents=b"z"
        )
    ...
    def test_rm_failing_precondition(self):
        ...
        object_uri = self.CreateObject(bucket_uri, contents=b"foo")
        stderr = self.RunGsUtil(
            ["-h", "x-goog-if-generation-match:12345", "rm", suri(object_uri)],
            return_stderr=True,
            expected_status=1,
        )
    ...
    def test_stdin_args(self):
        ...
        self.CreateObject(bucket_uri=buri1, object_name="bar", contents=b"barcontents")
        ouri3 = self.CreateObject(
        ...
        self._RunRemoveCommandAndCheck(
            ["rm", "-I"], objects_to_remove=objects_to_remove, stdin=stdin
        )
