...
class TestCompose(testcase.GsUtilIntegrationTestCase):
    ...
    def test_compose_too_few_fails(self):
        stderr = self.RunGsUtil(
            ["compose", "gs://b/composite-obj"], expected_status=1, return_stderr=True
        )
        self.assertIn(
    ...
    def test_compose_between_buckets_fails(self):
        ...
        stderr = self.RunGsUtil(
            ["compose"] + components + [target], expected_status=1, return_stderr=True
        )
        expected_msg = (
            "CommandException: GCS does " "not support inter-bucket composing.\n"
        )
    ...
    def test_versioned_target_disallowed(self):
        ...
        expected_msg = (
            ...
            "cannot be the destination for gsutil compose - abort." % "gs://b/o3#1234"
        )
        self.assertIn(expected_msg, stderr)
    ...
    def test_compose_copies_type_and_encoding_from_first_object(self):
        ...
        self.RunGsUtil(
            ["compose", suri(object_uri1), suri(object_uri2), suri(composite)]
        )
    ...
    def test_compose_with_encryption(self):
        ...
        object_uri1 = self.CreateObject(
            bucket_uri=bucket_uri, contents=b"foo", encryption_key=TEST_ENCRYPTION_KEY1
        )
        object_uri2 = self.CreateObject(
            bucket_uri=bucket_uri, contents=b"bar", encryption_key=TEST_ENCRYPTION_KEY1
        )

        # Compose without correct key should fail.
        stderr = self.RunGsUtil(
            ["compose", suri(object_uri1), suri(object_uri2), suri(bucket_uri, "obj")],
            expected_status=1,
            return_stderr=True,
        )
    ...
    def test_compose_different_encryption_keys(self):
        ...
        object_uri2 = self.CreateObject(
            bucket_uri=bucket_uri, contents=b"bar", encryption_key=TEST_ENCRYPTION_KEY2
        )
...
class TestCompatibleCompose(testcase.GsUtilIntegrationTestCase):
    def test_compose_non_gcs_target(self):
        stderr = self.RunGsUtil(
            ["compose", "gs://b/o1", "gs://b/o2", "s3://b/o3"],
            expected_status=1,
            return_stderr=True,
        )
        expected_msg = (
    ...
    def test_compose_non_gcs_component(self):
        stderr = self.RunGsUtil(
            ["compose", "gs://b/o1", "s3://b/o2", "gs://b/o3"],
            expected_status=1,
            return_stderr=True,
        )
        expected_msg = (
