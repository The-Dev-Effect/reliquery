...
class TestDOption(testcase.GsUtilIntegrationTestCase):
    ...
    def assert_header_in_output(self, name, value, output):
        ...
        if not alt_expected in output:
            self.fail(
                "Neither of these two header formats were found in the output:"
    ...
    def test_minus_D_multipart_upload(self):
        ...
        for file_contents in (b"a1b2c3d4", b"a1b2c3d4\n"):
            ...
            with SetBotoConfigForTest(
                ...
                self.assertIn(
                    "total_bytes_transferred: %d" % len(file_contents), stderr
                )
    ...
    def test_minus_D_perf_trace_cp(self):
        ...
        bucket_uri = self.CreateBucket()
        stderr = self.RunGsUtil(
            ["-D", "--perf-trace-token=123", "cp", fpath, suri(bucket_uri)],
            return_stderr=True,
        )
        self.assertIn("'cookie': '123'", stderr)
        stderr2 = self.RunGsUtil(
            ["-D", "--perf-trace-token=123", "cp", suri(bucket_uri, file_name), fpath],
            return_stderr=True,
        )
    ...
    def test_minus_D_cat(self):
        ...
        if self.test_api == ApiSelector.XML:
            ...
            if not match:
                self.fail(
                    "Did not find this regex in stderr:\nRegex: %s\nStderr: %s"
                    % (regex_str, stderr)
                )
            ...
            self.assertRegex(
                stderr, ".*GET.*b/%s/o/%s" % (key_uri.bucket_name, key_uri.object_name)
            )
            self.assertRegex(
                stderr,
                "Python/%s.gsutil/%s.*interactive/False command/cat"
