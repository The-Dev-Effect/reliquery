...
class TestPublicAccessPrevention(testcase.GsUtilIntegrationTestCase):
    ...
    def test_multiple_buckets(self):
        ...
        stdout = self.RunGsUtil(
            self._get_pap_cmd + [suri(bucket_uri1), suri(bucket_uri2)],
            return_stdout=True,
        )
    ...
    def test_xml_fails(self):
        ...
        with SetBotoConfigForTest(boto_config_hmac_auth_only):
            ...
            stderr = self.RunGsUtil(
                self._set_pap_cmd + ["unspecified", bucket_uri],
                return_stderr=True,
                expected_status=1,
            )
            ...
            stderr = self.RunGsUtil(
                self._get_pap_cmd + [bucket_uri], return_stderr=True, expected_status=1
            )
            self.assertIn("command can only be with the Cloud Storage JSON API", stderr)
    ...
    def test_s3_fails(self):
        bucket_uri = self.CreateBucket()
        stderr = self.RunGsUtil(
            self._set_pap_cmd + ["unspecified", suri(bucket_uri)],
            return_stderr=True,
            expected_status=1,
        )
        ...
        stderr = self.RunGsUtil(
            self._get_pap_cmd + [suri(bucket_uri)],
            return_stderr=True,
            expected_status=1,
        )
        self.assertIn("command can only be used for GCS Buckets", stderr)
    ...
    def test_set_too_few_arguments_fails(self):
        stderr = self.RunGsUtil(
            self._set_pap_cmd, return_stderr=True, expected_status=1
        )
        self.assertIn("command requires at least", stderr)

    def test_get_too_few_arguments_fails(self):
        stderr = self.RunGsUtil(
            self._get_pap_cmd, return_stderr=True, expected_status=1
        )
        self.assertIn("command requires at least", stderr)
    ...
    def test_invalid_subcommand_fails(self):
        stderr = self.RunGsUtil(
            ["pap", "fakecommand", "test"], return_stderr=True, expected_status=1
        )
