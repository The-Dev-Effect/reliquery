...
class TestDefacl(case.GsUtilIntegrationTestCase):
    ...
    def testChangeDefaultAcl(self):
        ...
        stderr = self.RunGsUtil(
            self._defacl_ch_prefix
            + ["-g", self.GROUP_TEST_ADDRESS + ":WRITE", suri(bucket)],
            return_stderr=True,
            expected_status=1,
        )
    ...
    def testChangeDefaultAclEmpty(self):
        ...
        self.RunGsUtil(
            self._defacl_ch_prefix + ["-d", self.GROUP_TEST_ADDRESS, suri(bucket)]
        )
        json_text3 = self.RunGsUtil(
            self._defacl_get_prefix + [suri(bucket)], return_stdout=True
        )
    ...
    def testChangeMultipleBuckets(self):
        ...
        self.RunGsUtil(
            self._defacl_ch_prefix
            + ["-g", self.GROUP_TEST_ADDRESS + ":READ", suri(bucket1), suri(bucket2)]
        )
        json_text = self.RunGsUtil(
            self._defacl_get_prefix + [suri(bucket1)], return_stdout=True
        )
    ...
    def testDeletePermissionsWithCh(self):
        ...
        self.RunGsUtil(
            self._defacl_ch_prefix + ["-d", self.USER_TEST_ADDRESS, suri(bucket)]
        )
        json_text = self.RunGsUtil(
            self._defacl_get_prefix + [suri(bucket)], return_stdout=True
        )
