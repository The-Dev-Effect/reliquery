...
def DetectGae():
    ...
    return server_software.startswith("Development/") or server_software.startswith(
        "Google App Engine/"
    )
...
def DetectGce():
    ...
    try:
        o = urllib_request.build_opener(urllib_request.ProxyHandler({})).open(
            urllib_request.Request(metadata_url, headers={"Metadata-Flavor": "Google"})
        )
    except urllib_error.URLError:
        return False
    return (
        o.getcode() == http_client.OK and o.headers.get("metadata-flavor") == "Google"
    )
...
def NormalizeScopes(scope_spec):
    ...
    raise exceptions.TypecheckError(
        "NormalizeScopes expected string or iterable, found %s" % (type(scope_spec),)
    )
...
def ExpandRelativePath(method_config, params, relative_path=None):
    ...
    for param in method_config.path_params:
        ...
        try:
            ...
            path = path.replace(
                param_template,
                urllib_parse.quote(value.encode("utf_8"), reserved_chars),
            )
...
def AcceptableMimeType(accept_patterns, mime_type):
    ...
    if unsupported_patterns:
        raise exceptions.GeneratedClientError(
            'MIME patterns with parameter unsupported: "%s"'
    ...
    def MimeTypeMatches(pattern, mime_type):
        ...
        return all(
            accept in ("*", provided)
            for accept, provided in zip(pattern.split("/"), mime_type.split("/"))
        )
...
def MapParamNames(params, request_type):
    ...
    return [
        encoding.GetCustomJsonFieldMapping(request_type, json_name=p) or p
        for p in params
    ]
    ...
    for param_name, value in params.items():
        ...
        if isinstance(value, messages.Enum):
            new_params[param_name] = encoding.GetCustomJsonEnumMapping(
                type(value), python_name=str(value)
            ) or str(value)
    return new_params
