...
class MessageRegistry(object):
    ...
    PRIMITIVE_TYPE_INFO_MAP = {
        "string": TypeInfo(
            type_name="string", variant=messages.StringField.DEFAULT_VARIANT
        ),
        "integer": TypeInfo(
            type_name="integer", variant=messages.IntegerField.DEFAULT_VARIANT
        ),
        "boolean": TypeInfo(
            type_name="boolean", variant=messages.BooleanField.DEFAULT_VARIANT
        ),
        "number": TypeInfo(
            type_name="number", variant=messages.FloatField.DEFAULT_VARIANT
        ),
        "any": TypeInfo(
            type_name="extra_types.JsonValue", variant=messages.Variant.MESSAGE
        ),
    }

    PRIMITIVE_FORMAT_MAP = {
        "int32": TypeInfo(type_name="integer", variant=messages.Variant.INT32),
        "uint32": TypeInfo(type_name="integer", variant=messages.Variant.UINT32),
        "int64": TypeInfo(type_name="string", variant=messages.Variant.INT64),
        "uint64": TypeInfo(type_name="string", variant=messages.Variant.UINT64),
        "double": TypeInfo(type_name="number", variant=messages.Variant.DOUBLE),
        "float": TypeInfo(type_name="number", variant=messages.Variant.FLOAT),
        "byte": TypeInfo(type_name="byte", variant=messages.BytesField.DEFAULT_VARIANT),
        "date": TypeInfo(
            type_name="extra_types.DateField", variant=messages.Variant.STRING
        ),
        "date-time": TypeInfo(
    ...
    def WriteProtoFile(self, printer):
        ...
        extended_descriptor.WriteMessagesFile(
            self.__file_descriptor, self.__package, self.__client_info.version, printer
        )
    ...
    def __RegisterDescriptor(self, new_descriptor):
        ...
        if not isinstance(
            ...
            raise ValueError(
                "Cannot add descriptor of type %s" % (type(new_descriptor),)
            )
    ...
    def __AddAdditionalProperties(self, message, schema, properties):
        ...
        message.fields.append(
            self.__FieldDescriptorFromProperties(field_name, len(properties) + 1, attrs)
        )
        self.__AddImport("from %s import encoding" % self.__base_files_package)
    ...
    def __FieldDescriptorFromProperties(self, name, index, attrs):
        ...
        if "default" in attrs:
            ...
            if not (
                field.type_name == "string" or field.variant == messages.Variant.ENUM
            ):
    ...
    def __AddIfUnknown(self, type_name):
        ...
        if (
            full_type_name not in self.__message_registry.keys()
            and type_name not in self.__message_registry.keys()
        ):
    ...
    def __GetTypeInfo(self, attrs, name_hint):
        ...
        if "format" in attrs:
            ...
            if type_info is None:
                ...
                raise ValueError(
                    'Unknown type/format "%s"/"%s"' % (attrs["format"], type_name)
                )
            if type_info.type_name.startswith(
                ("apitools.base.protorpclite.message_types.", "message_types.")
            ):
                self.__AddImport(
