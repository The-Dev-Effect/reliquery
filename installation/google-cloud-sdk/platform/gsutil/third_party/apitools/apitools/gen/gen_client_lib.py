...
class DescriptorGenerator(object):
    ...
    def __init__(
        ...
        standard_query_schema = _StandardQueryParametersSchema(self.__discovery_doc)
    ...
    def WriteInit(self, out):
        ...
        if self.__init_wildcards_file:
            printer(
                '"""Common imports for generated %s client library."""',
        ...
        if self.__init_wildcards_file:
            ...
            printer(
                "from %s%s import *", import_prefix, self.__client_info.client_rule_name
            )
            printer(
                "from %s%s import *",
                import_prefix,
                self.__client_info.messages_rule_name,
            )
            printer()
    ...
    def WriteSetupPy(self, out):
        ...
        with printer.Indent(indent="    "):
            ...
            minor = parts.pop(0)
            printer(
                '"google-apitools>=%s,~=%s.%s",', self.apitools_version, major, minor
            )
        ...
        with printer.Indent(indent="    "):
            ...
            printer(
                'description="Autogenerated apitools library for %s",'
                % (self.__package,)
            )
            ...
            printer(
                'keywords="apitools apitools-%s %s",' % (self.__package, self.__package)
            )
        printer(")")
