...
class ReauthTest(unittest.TestCase):
    ...
    def _request_mock_side_effect(self, *args, **kwargs):
        ...
        if uri == (_reauth_client._REAUTH_API + "/session_id_1:continue"):
            ...
            if (
                qp_json.get("proposalResponse", {}).get("credential")
    ...
    class _U2FInterfaceMock(object):
        def Authenticate(self, unused_app_id, challenge, unused_registered_keys):
            client_data = model.ClientData(
                model.ClientData.TYP_AUTHENTICATION, challenge, "some_origin"
            )
            return model.SignResponse(
                "key_handle".encode(), "resp".encode(), client_data
            )
    ...
    def testPassAndGnubbyReauth(self):
        with mock.patch(
            "httplib2.Http.request", side_effect=self._request_mock_side_effect
        ) as request_mock:
    ...
    def accessTokenRefreshError(self, response, content):
        ...
        with mock.patch(
            "httplib2.Http.request", side_effect=side_effect
        ) as request_mock:
    ...
    def testAccessTokenRefreshError(self):
        ...
        self.accessTokenRefreshError(
            _error_response, json.dumps({"error": "non_reauth_error"})
        )
