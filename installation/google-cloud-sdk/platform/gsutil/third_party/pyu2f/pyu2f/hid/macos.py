...
def GetDeviceIntProperty(dev_ref, key):
    ...
    if cf.CFGetTypeID(type_ref) != cf.CFNumberGetTypeID():
        raise errors.OsHidError(
            "Expected number type, got {}".format(cf.CFGetTypeID(type_ref))
        )
...
def GetDeviceStringProperty(dev_ref, key):
    ...
    if cf.CFGetTypeID(type_ref) != cf.CFStringGetTypeID():
        raise errors.OsHidError(
            "Expected string type, got {}".format(cf.CFGetTypeID(type_ref))
        )
...
def DeviceReadThread(hid_device):
    ...
    while (
        ...
        run_loop_run_result = cf.CFRunLoopRunInMode(
            K_CF_RUNLOOP_DEFAULT_MODE, 1000, False  # Timeout in seconds
...
class MacOsHidDevice(base.HidDevice):
    ...
    def __init__(self, path):
        ...
        if not device_entry:
            raise errors.OsHidError(
                "Device path does not match any HID device on " "the system"
            )
        ...
        if not self.device_handle:
            raise errors.OsHidError(
                "Failed to obtain device handle from registry " "entry"
            )
