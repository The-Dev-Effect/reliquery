...
class ApduTest(unittest.TestCase):
    def testSerializeCommandApdu(self):
        ...
        self.assertEqual(
            cmd.ToByteArray(),
            bytearray(
                [0x00, 0x01, 0x03, 0x04, 0x00, 0x00, 0x03, 0x10, 0x20, 0x30, 0x00, 0x00]
            ),
        )
        self.assertEqual(
    ...
    def testSerializeCommandApduNoData(self):
        ...
        self.assertEqual(
            cmd.ToByteArray(), bytearray([0x00, 0x01, 0x03, 0x04, 0x00, 0x00, 0x00])
        )
        self.assertEqual(
            cmd.ToLegacyU2FByteArray(),
            bytearray([0x00, 0x01, 0x03, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00]),
        )
    ...
    def testSerializeCommandApduTooLong(self):
        self.assertRaises(
            ...
            0x03,
            0x04,
            bytearray(0 for x in range(0, 65536)),
        )
    ...
    def testResponseApduParseInvalid(self):
        self.assertRaises(
            errors.InvalidResponseError, apdu.ResponseApdu, bytearray([0x05])
        )
