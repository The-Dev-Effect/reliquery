...
GOOGLE_OAUTH2_PROVIDER_TOKEN_URI = "https://oauth2.googleapis.com/token"
...
def OAuth2ClientFromBotoConfig(
    ...
    if token_cache_type == "file_system":
        ...
        raise Exception(
            "Invalid value for config option OAuth2/token_cache: %s" % token_cache_type
        )
    ...
    provider_authorization_uri = config.get(
        "OAuth2", "provider_authorization_uri", GOOGLE_OAUTH2_PROVIDER_AUTHORIZATION_URI
    )
    ...
    if cred_type == oauth2_client.CredTypes.OAUTH2_SERVICE_ACCOUNT:
        ...
        if keyfile_is_utf8:
            try:
                ...
                raise Exception(
                    'Could not parse JSON keyfile "%s" as valid JSON'
            ...
            for json_entry in (
                ...
                if json_entry not in json_key_dict:
                    raise Exception(
                        "The JSON private key file at %s "
            ...
            return oauth2_client.OAuth2JsonServiceAccountClient(
                ...
                disable_ssl_certificate_validation=not (
                    config.getbool("Boto", "https_validate_certificates", True)
                ),
        ...
        client_id = config.get(
            "OAuth2", "client_id", os.environ.get("OAUTH2_CLIENT_ID", CLIENT_ID)
        )
        if not client_id:
            raise Exception(
        ...
        client_secret = config.get(
            "OAuth2",
            "client_secret",
            os.environ.get("OAUTH2_CLIENT_SECRET", CLIENT_SECRET),
        )
        ...
        raise Exception(
            "You have attempted to create an OAuth2 client without "
