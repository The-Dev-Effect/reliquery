...
DEFAULT_CA_CERTS_FILE = os.path.abspath(os.path.join("gslib", "data", "cacerts.txt"))
...
class MockOAuth2ServiceAccountClient(oauth2_client.OAuth2ServiceAccountClient):
    ...
    def __init__(
        self, client_id, private_key, password, auth_uri, token_uri, datetime_strategy
    ):
...
def GetExpiry(datetime_strategy, length_in_seconds):
    token_expiry = datetime_strategy.utcnow() + datetime.timedelta(
...
def CreateMockUserAccountClient(mock_datetime):
    return MockOAuth2UserAccountClient(
        TOKEN_URI, "clid", "clsecret", "ref_token_abc123", AUTH_URI, mock_datetime
    )
...
class OAuth2AccountClientTest(unittest.TestCase):
    ...
    def _RunGetAccessTokenTest(self, expected_rapt=None):
        ...
        self.assertEqual(
            self.start_time + datetime.timedelta(minutes=60), token_1.expiry
        )
        ...
        self.client.Reset()
        self.mock_datetime.mock_now = self.start_time + datetime.timedelta(
        ...
        self.assertTrue(self.client.fetched_token)
        self.assertEqual(
            self.mock_datetime.mock_now + datetime.timedelta(minutes=60), token_3.expiry
        )
...
class FileSystemTokenCacheTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.token_2 = oauth2_client.AccessToken(
            "token2",
            self.start_time + datetime.timedelta(seconds=492),
    ...
    def testPut(self):
        ...
        if not IS_WINDOWS:
            self.assertEqual(
                0o600, stat.S_IMODE(os.stat(self.cache.CacheFileName(self.key)).st_mode)
            )
    ...
    def testCacheFileName(self):
        ...
        if IS_WINDOWS:
            ...
            uid = os.getuid()
        self.assertEqual(
            "/var/run/ccache/token.%s.abc123" % uid, cache.CacheFileName("abc123")
        )
...
class RefreshTokenTest(unittest.TestCase):
    ...
    def testGetAuthorizationHeader(self):
        self.assertEqual(
            "Bearer %s" % ACCESS_TOKEN, self.client.GetAuthorizationHeader()
        )
