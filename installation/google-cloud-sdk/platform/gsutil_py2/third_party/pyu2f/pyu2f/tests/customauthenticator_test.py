...
SIGN_SUCCESS = {
    ...
    "origin": "test_origin",
    "signature_data_encoded": (
        "AQAAAI8wRQIhALlIPo6Hg8HwzELdYRIXnAnpsiHYCSXHex"
        "CS34eiS2ixAiBt3TRmKE1A9WyMjc3JGrGI7gSPg-QzDSNL"
        "aIj7JwcCTA"
    ),
    "client_data_encoded": (
        "eyJjaGFsbGVuZ2UiOiAiWVhOa1ptRnpaR1kiLCAib3JpZ2luI"
        "jogInRlc3Rfb3JpZ2luIiwgInR5cCI6ICJuYXZpZ2F0b3IuaW"
        "QuZ2V0QXNzZXJ0aW9uIn0"
    ),
    "u2f_version": "U2F_V2",
    "registered_key": model.RegisteredKey(
...
class CustomAuthenticatorTest(unittest.TestCase):
    ...
    @mock.patch.object(
        customauthenticator.os.environ, "get", return_value="gnubbyagent"
    )
    def testSuccessAuthenticate(self, os_get_method, popen_method):
        ...
        self.assertTrue(
            authenticator.IsAvailable(), "Should return true if env var is present"
        )
        ...
        self.assertEquals(
            len(communicate_json),
            communicate_json_len,
            "communicate() should have been called with correct" "length field",
        )
        ...
        self.assertEquals(
            challenge.get("appIdHash"), SIGN_SUCCESS["app_id_hash_encoded"]
        )
        self.assertEquals(
            challenge.get("challengeHash"), SIGN_SUCCESS["challenge_hash_encoded"]
        )
        self.assertEquals(
            challenge.get("keyHandle"), SIGN_SUCCESS["key_handle_encoded"]
        )
        ...
        self.assertEquals(
            result["signatureData"], SIGN_SUCCESS["signature_data_encoded"]
        )
