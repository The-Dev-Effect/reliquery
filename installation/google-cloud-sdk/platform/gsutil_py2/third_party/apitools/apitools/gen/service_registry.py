...
class ServiceRegistry(object):
    ...
    def __WriteSingleService(self, printer, name, method_info_map, client_class_name):
        ...
        with printer.Indent():
            ...
            with printer.Indent():
                printer(
                    "super(%s.%s, self).__init__(client)", client_class_name, class_name
                )
                    ...
                    for method_name, method_info in method_info_map.items():
                        ...
                        if upload_config is not None:
                            ...
                            with printer.Indent(indent="    "):
                                ...
                                for attr in attrs:
                                    printer(
                                        "%s=%r,", attr, getattr(upload_config, attr)
                                    )
                            printer("),")
                    printer("}")
            ...
            for method_name, method_info in method_info_map.items():
                ...
                with printer.Indent():
                    ...
                    if upload_config is not None:
                        printer(
                            "upload_config = self.GetUploadConfig('%s')", method_name
                        )
                ...
                printer()
                printer(
                    "{0}.method_config = lambda: base_api.ApiMethodInfo(".format(
                        method_name
                    )
                )
                with printer.Indent(indent="    "):
        ...
        with printer.Indent():
            for method_name, method_info in method_info_map.items():
                for line in textwrap.wrap(
                    method_info.description, printer.CalculateWidth() - 3
                ):
                    printer("// %s", line)
                printer(
                    "rpc %s (%s) returns (%s);",
    ...
    def WriteProtoFile(self, printer):
        ...
        printer(
            "// Generated services for %s version %s.",
            client_info.package,
            client_info.version,
        )
        printer()
    ...
    def WriteFile(self, printer):
        ...
        if self.__root_package:
            ...
            import_prefix = ""
        printer(
            "%simport %s as messages", import_prefix, client_info.messages_rule_name
        )
        printer()
        printer()
        ...
        with printer.Indent():
            ...
            printer("_CLIENT_CLASS_NAME = {0!r}".format(client_info.client_class_name))
            ...
            with printer.Indent():
                ...
                for name in self.__service_method_info_map.keys():
                    printer(
                        "self.%s = self.%s(self)",
                        name,
                        self.__GetServiceClassName(name),
                    )
    ...
    def __CreateRequestType(self, method_description, body_type=None):
        ...
        schema = {}
        schema["id"] = self.__names.ClassName(
            "%sRequest"
    ...
    def __NeedRequestType(self, method_description, request_type):
        ...
        for param_name, param_info in parameters.items():
            if (
                param_info.get("location") != "path"
                or self.__names.CleanName(param_name) not in field_names
            ):
                break
        else:
            return False
    ...
    def __ComputeMethodInfo(self, method_description, request, response, request_field):
        ...
        relative_path = self.__names.NormalizeRelativePath(
            "".join((self.__client_info.base_path, method_description["path"]))
        )
        ...
        for param, desc in method_description.get("parameters", {}).items():
            ...
            if location == "query":
                ...
                raise ValueError(
                    "Unknown parameter location %s for parameter %s" % (location, param)
                )
