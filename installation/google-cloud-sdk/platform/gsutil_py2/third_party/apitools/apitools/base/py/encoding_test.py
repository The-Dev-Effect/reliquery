...
class NestedAdditionalPropertiesWithEnumMessage(messages.Message):
    class AdditionalProperty(messages.Message):
        key = messages.StringField(1)
        value = messages.MessageField(MapToMessageWithEnum, 2)
...
encoding.AddCustomJsonEnumMapping(
    MessageWithRemappings.SomeEnum, "enum_value", "wire_name"
)
encoding.AddCustomJsonFieldMapping(
    MessageWithRemappings, "double_encoding", "doubleEncoding"
)
encoding.AddCustomJsonFieldMapping(
    MessageWithRemappings, "another_field", "anotherField"
)
encoding.AddCustomJsonFieldMapping(
    MessageWithRemappings, "repeated_field", "repeatedField"
)
...
class EncodingTest(unittest.TestCase):
    ...
    def testCopyProtoMessageInvalidEnum(self):
        ...
        for msg in (orig_msg, new_msg):
            ...
            self.assertEqual(
                msg.get_unrecognized_field_info("field_one", value_default=None),
    ...
    def testCopyProtoMessageAdditionalProperties(self):
        msg = AdditionalPropertiesMessage(
            additionalProperties=[
                AdditionalPropertiesMessage.AdditionalProperty(key="key", value="value")
            ]
        )
        new_msg = encoding.CopyProtoMessage(msg)
    ...
    def testIncludeFields(self):
        ...
        self.assertEqual(
            '{"field": null}', encoding.MessageToJson(msg, include_fields=["field"])
        )
        self.assertEqual(
            '{"repfield": []}', encoding.MessageToJson(msg, include_fields=["repfield"])
        )

    def testNestedIncludeFields(self):
        msg = HasNestedMessage(
            nested=AdditionalPropertiesMessage(additionalProperties=[])
        )
        self.assertEqual(
            '{"nested": null}', encoding.MessageToJson(msg, include_fields=["nested"])
        )
        self.assertEqual(
            '{"nested": {"additionalProperties": []}}',
            encoding.MessageToJson(msg, include_fields=["nested.additionalProperties"]),
        )
    ...
    def testAdditionalPropertyMapping(self):
        ...
        self.assertEqual(
            {"key_one": "value_one", u"key_twð": "value_two"}, json.loads(encoded_msg)
        )
    ...
    def testUnrecognizedEnum(self):
        ...
        self.assertEqual(1, len(result.additionalProperties))
        self.assertEqual(
            UnrecognizedEnumMessage.ThisEnum.VALUE_ONE,
    ...
    def testValidEnums(self):
        ...
        self.assertEqual(
            json.loads(message_json), json.loads(encoding.MessageToJson(message))
        )
    ...
    def testIgnoredEnums(self):
        ...
        self.assertEqual(
            ("VALUE_OEN", messages.Variant.ENUM),
            message.get_unrecognized_field_info("field_one"),
        )
        self.assertEqual(
            json.loads(json_with_typo), json.loads(encoding.MessageToJson(message))
        )
    ...
    def testUnknownNestedRoundtrip(self):
        ...
        self.assertEqual(
            json.loads(json_message), json.loads(encoding.MessageToJson(message))
        )

    def testUnknownEnumNestedRoundtrip(self):
        json_with_typo = (
            '{"outer_key": {"key_one": {"field_one": '
    ...
    def testRepeatedEnumRemapping(self):
        ...
        json_message = encoding.MessageToJson(msg)
        self.assertEqual(
            '{"repeated_enum": ["wire_name", "second_value"]}', json_message
        )
        self.assertEqual(
            msg, encoding.JsonToMessage(MessageWithRemappings, json_message)
        )
    ...
    def testFieldRemappingWithPackage(self):
        ...
        try:
            ...
            self.assertEqual(
                msg,
                encoding.JsonToMessage(MessageWithPackageAndRemappings, json_message),
            )
        finally:
    ...
    def testRepeatedRemapping(self):
        ...
        self.assertRaises(
            exceptions.InvalidDataError,
            encoding.AddCustomJsonEnumMapping,
            MessageWithRemappings.SomeEnum,
            "enum_value",
            "another_name",
        )
        self.assertRaises(
    ...
    def testDictToAdditionalPropertyMessageNumeric(self):
        ...
        expected_msg.additionalProperties = [
            AdditionalIntPropertiesMessage.AdditionalProperty(key="key", value=1)
        ]
        self.assertEqual(encoded_msg, expected_msg)
    ...
    def testUnrecognizedFieldIterRepeated(self):
        m = encoding.DictToMessage(
            {"msg_field": [{"field": "foo"}, {"not_a_field": "bar"}]},
