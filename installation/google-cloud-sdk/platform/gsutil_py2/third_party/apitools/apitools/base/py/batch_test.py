...
class BatchTest(unittest.TestCase):
    ...
    def testRequestServiceUnavailable(self):
        ...
        batch_api_request = batch.BatchApiRequest(
            batch_url=desired_url, retryable_codes=[]
        )
        ...
        with mock.patch.object(
            ...
            self.assertIsInstance(
                api_request_responses[0].exception, exceptions.HttpError
            )
    ...
    def testMultipleRequestInBatchWithMax(self):
        ...
        for i in range(0, number_of_requests, max_batch_size):
            responses.append(
                self._MakeResponse(min(number_of_requests - i, max_batch_size))
            )
        with mock.patch.object(
            http_wrapper, "MakeRequest", autospec=True
        ) as mock_request:
            self.__ConfigureMock(
                mock_request,
                expected_request=http_wrapper.Request(
                    desired_url,
                    "POST",
                    {
                        "content-type": 'multipart/mixed; boundary="None"',
        ...
        self.assertEqual(
            -(-number_of_requests // max_batch_size), mock_request.call_count
        )
    ...
    def _DoTestConvertHeaderToId(self, header, expected_id):
        batch_request = batch.BatchHttpRequest("https://www.example.com")
        self.assertEqual(expected_id, batch_request._ConvertHeaderToId(header))
    ...
    def _DoTestConvertInvalidHeaderToId(self, invalid_header):
        batch_request = batch.BatchHttpRequest("https://www.example.com")
        self.assertRaises(
            exceptions.BatchError, batch_request._ConvertHeaderToId, invalid_header
        )
    ...
    def testDeserializeRequest(self):
        ...
        self.assertEqual(
            expected_response, batch_request._DeserializeResponse(serialized_payload)
        )
    ...
    def testAdd(self):
        ...
        for key in batch_request._BatchHttpRequest__request_response_handlers:
            value = batch_request._BatchHttpRequest__request_response_handlers[key]
    ...
    def testInternalExecuteWithFailedRequest(self):
        with mock.patch.object(
            ...
            self.assertRaises(exceptions.HttpError, batch_request._Execute, None)
    ...
    def testInternalExecuteWithNonMultipartResponse(self):
        with mock.patch.object(
            ...
            self.__ConfigureMock(
                ...
                http_wrapper.Response(
                    {"status": "200", "content-type": "blah/blah"}, "", None
                ),
            )
    ...
    def testInternalExecute(self):
        with mock.patch.object(
            ...
            self.assertEqual(
                http_client.UNAUTHORIZED, test_responses["1"].response.status_code
            )
    ...
    def testInternalExecuteWithEncodedResponse(self):
        with mock.patch.object(
            ...
            batch_request = batch.BatchHttpRequest(
                "https://www.example.com", response_encoding="utf-8"
            )
            batch_request._BatchHttpRequest__request_response_handlers = test_request

            batch_request._Execute(FakeHttp())

            test_responses = batch_request._BatchHttpRequest__request_response_handlers

            self.assertEqual(http_client.OK, test_responses["1"].response.status_code)

            self.assertIn("response", test_responses["1"].response.content)
        ...
        with mock.patch.object(
            ...
            test_requests = {
                "0": batch.RequestResponseAndHandler(
                    None, http_wrapper.Response({"status": "200"}, "Hello!", None), None
                ),
                "1": batch.RequestResponseAndHandler(
                    None,
