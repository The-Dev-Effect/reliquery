...
encoding.AddCustomJsonFieldMapping(MessageWithRemappings, "str_field", "remapped_field")
encoding.AddCustomJsonEnumMapping(MessageWithRemappings.AnEnum, "value_one", "ONE/TWO")
...
class BaseApiTest(unittest.TestCase):
    ...
    def testJsonResponse(self):
        ...
        self.assertEqual(
            response_message, service.ProcessHttpResponse(method_config, http_response)
        )
    ...
    def testCustomCheckResponse(self):
        ...
        method_config = base_api.ApiMethodInfo(
            request_type_name="SimpleMessage", response_type_name="SimpleMessage"
        )
        client = self.__GetFakeClient()
    ...
    def testColonInRelativePath(self):
        method_config = base_api.ApiMethodInfo(
            relative_path="path:withJustColon", request_type_name="SimpleMessage"
        )
        service = FakeService()
        request = SimpleMessage()
        http_request = service.PrepareHttpRequest(method_config, request)
