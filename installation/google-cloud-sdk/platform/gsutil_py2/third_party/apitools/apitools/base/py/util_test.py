...
encoding.AddCustomJsonFieldMapping(MessageWithRemappings, "str_field", "path_field")
encoding.AddCustomJsonFieldMapping(
    MessageWithRemappings, "enum_field_remapping", "enum_field_remapped"
)
encoding.AddCustomJsonEnumMapping(MessageWithRemappings.AnEnum, "value_one", "ONE")


class UtilTest(unittest.TestCase):
    def testExpand(self):
        method_config_xy = MockedMethodConfig(
            relative_path="{x}/y/{z}", path_params=["x", "z"]
        )
        self.assertEquals(
            util.ExpandRelativePath(method_config_xy, {"x": "1", "z": "2"}), "1/y/2"
        )
        self.assertEquals(
            util.ExpandRelativePath(
                method_config_xy, {"x": "1", "z": "2"}, relative_path="{x}/y/{z}/q"
            ),
            "1/y/2/q",
        )

    def testReservedExpansion(self):
        method_config_reserved = MockedMethodConfig(
            relative_path="{+x}/baz", path_params=["x"]
        )
        self.assertEquals(
            "foo/:bar:/baz",
            util.ExpandRelativePath(method_config_reserved, {"x": "foo/:bar:"}),
        )
        method_config_no_reserved = MockedMethodConfig(
            relative_path="{x}/baz", path_params=["x"]
        )
        self.assertEquals(
            "foo%2F%3Abar%3A/baz",
            util.ExpandRelativePath(method_config_no_reserved, {"x": "foo/:bar:"}),
        )
    ...
    def testTypecheck(self):
        ...
        self.assertEquals(
            instance_of_class1, util.Typecheck(instance_of_class1, Class1, "message")
        )

        self.assertEquals(
            instance_of_class1,
            util.Typecheck(instance_of_class1, ((Class1, Class2), Class3), "message"),
        )
    ...
    def testAcceptableMimeType(self):
        ...
        self.assertTrue(
            util.AcceptableMimeType(["application/json", "*/*"], "text/plain")
        )
        self.assertFalse(
            util.AcceptableMimeType(["application/json", "img/*"], "text/plain")
        )

    def testMalformedMimeType(self):
        self.assertRaises(
            exceptions.InvalidUserInputError, util.AcceptableMimeType, ["*/*"], "abcd"
        )

    def testUnsupportedMimeType(self):
        self.assertRaises(
            exceptions.GeneratedClientError,
            util.AcceptableMimeType,
            ["text/html;q=0.9"],
            "text/html",
        )
    ...
    def testMapRequestParams(self):
        ...
        self.assertEqual(
            remapped_params, util.MapRequestParams(params, MessageWithRemappings)
        )
