...
try:
    ...
    if not (
        server_env.startswith("Development")
        or server_env.startswith("Google App Engine")
    ):
        raise import_error
...
def GetCredentials(
    ...
    **kwds
    ...
    credentials = CredentialsFromFile(
        credentials_filename, client_info, oauth2client_args=oauth2client_args
    )
    if credentials is not None:
        return credentials
    ...
    if oauth2client.__version__ > "1.5.2":
        ...
        with open(private_key_filename, "rb") as key_file:
            return oauth2client.client.SignedJwtAssertionCredentials(
                service_account_name, key_file.read(), scopes, user_agent=user_agent
            )
...
class GceAssertionCredentials(gce.AppAssertionCredentials):
    ...
    def GetInstanceScopes(self):
        ...
        response = _GceMetadataRequest(relative_url)
        return util.NormalizeScopes(
            six.ensure_str(scope).strip() for scope in response.readlines()
        )
    ...
    def _do_refresh_request(self, unused_http_request):
        ...
        if "expires_in" in credential_info:
            ...
            self.token_expiry = (
                datetime.timedelta(seconds=expires_in) + datetime.datetime.utcnow()
            )
        else:
...
def CredentialsFromFile(path, client_info, oauth2client_args=None):
    ...
    if _NEW_FILESTORE:
        ...
        credential_store = (
            multistore_file.get_credential_storage_custom_string_key(  # noqa
                path, storage_key
            )
        )
    if hasattr(FLAGS, "auth_local_webserver"):
...
def _GetApplicationDefaultCredentials(
    client_info, skip_application_default_credentials=False, **unused_kwds
):
