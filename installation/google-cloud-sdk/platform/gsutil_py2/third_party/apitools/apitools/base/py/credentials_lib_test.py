...
class CredentialsLibTest(unittest.TestCase):
        ...
        with mock.patch.object(util, "DetectGce", autospec=True) as gce_detect:
            ...
            with mock.patch.object(
                ...
                credentials = self._RunGceAssertionCredentials(
                    service_account_name=service_account_name, scopes=scopes
                )
    ...
    def testGetServiceAccount(self):
        ...
        with mock.patch.object(
            six.moves.urllib.request, "build_opener", return_value=opener, autospec=True
        ) as build_opener:
            ...
            self.assertTrue(
                req.get_full_url().startswith("http://metadata.google.internal/")
            )
