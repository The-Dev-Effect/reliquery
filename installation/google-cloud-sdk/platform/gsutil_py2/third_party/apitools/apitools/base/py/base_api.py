...
class BaseApiClient(object):
    ...
    def ProcessHttpRequest(self, http_request):
        ...
        if self.log_request:
            logging.info(
                "Making http %s to %s", http_request.http_method, http_request.url
            )
    ...
    def ProcessResponse(self, method_config, response):
        if self.log_response:
            logging.info(
                "Response of type %s: %s", method_config.response_type_name, response
            )
        return response
...
class BaseApiService(object):
    ...
    def GetMethodsList(cls):
        return [
            f.__name__
            for f in six.itervalues(cls.__dict__)
    ...
    def __ConstructQueryParams(self, query_params, request, global_params):
        ...
        query_info = dict(
            (
                param,
                self.__FinalUrlValue(
                    getattr(global_params, param), getattr(global_params_type, param)
                ),
            )
        ...
        query_info.update(
            (
                param,
                self.__FinalUrlValue(
                    getattr(request, param, None), getattr(request_type, param)
                ),
            )
    ...
    def __ConstructRelativePath(self, method_config, request, relative_path=None):
        ...
        params = dict(
            [(param, getattr(request, param, None)) for param in python_param_names]
        )
        params = util.MapRequestParams(params, type(request))
        return util.ExpandRelativePath(
            method_config, params, relative_path=relative_path
        )
    ...
    def __FinalizeRequest(self, http_request, url_builder):
        ...
        if (
            ...
            and len(http_request.url) > _MAX_URL_LENGTH
        ):
