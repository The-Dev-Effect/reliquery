...
META_TOKEN_URI = (
    METADATA_SERVER + "/computeMetadata/v1/instance/" "service-accounts/default/token"
)

META_HEADERS = {"Metadata-Flavor": "Google"}

# pylint: disable=invalid-name
_ServiceAccountCredentials = oauth2client.service_account.ServiceAccountCredentials
...
class InMemoryTokenCache(TokenCache):
    ...
    def GetToken(self, key):
        ...
        LOG.debug(
            "InMemoryTokenCache.GetToken: key=%s%s present",
            key,
...
class FileSystemTokenCache(TokenCache):
    ...
    def PutToken(self, key, value):
        ...
        LOG.debug(
            "FileSystemTokenCache.PutToken: key=%s, cache_file=%s", key, cache_file
        )
        try:
        ...
        try:
            ...
            LOG.warning(
                "FileSystemTokenCache.PutToken: " "Failed to create cache file %s: %s",
                cache_file,
                e,
            )
            return
    ...
    def GetToken(self, key):
        ...
        LOG.debug(
            "FileSystemTokenCache.GetToken: key=%s%s present (cache_file=%s)",
            key,
            " not" if value is None else "",
            cache_file,
        )
...
class OAuth2Client(object):
    ...
    def __init__(
        ...
        if proxy_host and proxy_port:
            self._proxy_info = httplib2.ProxyInfo(
    ...
    def FetchAccessToken(self, rapt_token=None):
        ...
        try:
            ...
            response, content = http.request(
                META_TOKEN_URI, method="GET", body=None, headers=META_HEADERS
            )
...
def _IsGCE():
    ...
    try:
        ...
        LOG.warning(
            "Failed to determine whether we're running on GCE, so we'll"
            "assume that we aren't: %s",
            e,
        )
        return False
...
class AccessToken(object):
    ...
    def __init__(
        self, token, expiry, datetime_strategy=datetime.datetime, rapt_token=None
    ):
        self.token = token
        self.expiry = expiry
        self.datetime_strategy = datetime_strategy
    ...
    def ShouldRefresh(self, time_delta=300):
        ...
        if self.expiry is None:
            return False
        return (
            self.datetime_strategy.utcnow() + datetime.timedelta(seconds=time_delta)
            > self.expiry
        )
