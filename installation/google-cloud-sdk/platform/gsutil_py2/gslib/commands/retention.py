...
_event_default_help_text = CreateHelpText(
    _EVENT_DEFAULT_SYNOPSIS, _EVENT_DEFAULT_DESCRIPTION
)
...
class RetentionCommand(Command):
        ...
        if action_subcommand == "set":
            ...
            raise CommandException(
                (
                    ...
                    'See "gsutil help retention".'
                ).format(action_subcommand, self.command_name)
            )
    ...
    def BucketUpdateFunc(
        self, url_args, bucket_metadata_update, fields, log_msg_template
    ):
    ...
    def ObjectUpdateMetadataFunc(
        self, patch_obj_metadata, log_template, name_expansion_result, thread_state=None
    ):
        ...
        for url_str in url_args:
            ...
            for blr in bucket_iter:
                ...
                if not (
                    ...
                    and bucket_metadata.retentionPolicy.retentionPeriod
                ):
                    ...
                    self.logger.error(
                        'Retention Policy on "%s" is already locked.', blr
                    )
                elif ConfirmLockRequest(
                    url.bucket_name, bucket_metadata.retentionPolicy
                ):
        ...
        if self.args[0].lower() == "set":
            ...
            raise CommandException(
                (
                    ...
                    'See "gsutil help retention {}".'
                ).format(self.args[0], sub_command_name, sub_command_name)
            )
        return hold
