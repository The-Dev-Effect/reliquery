...
def _GenSignedUrl(
    ...
    if method == "RESUMABLE":
        ...
        if not content_type:
            logger.warn(
                "Warning: no Content-Type header was specified with the -c "
    ...
    if use_service_account:
        ...
        final_url = GetFinalUrl(
            raw_signature, gs_host, gcs_path, canonical_query_string
        )
    return final_url
...
class UrlSignCommand(Command):
    ...
    def _ParseAndCheckSubOpts(self):
        ...
        if delta is None:
            ...
            if use_service_account and delta > _MAX_EXPIRATION_TIME_WITH_MINUS_U:
                ...
                raise CommandException(
                    "Max valid duration allowed is " "%s" % _MAX_EXPIRATION_TIME
                )

        if method not in ["GET", "PUT", "DELETE", "HEAD", "RESUMABLE"]:
            raise CommandException(
                "HTTP method must be one of" "[GET|HEAD|PUT|DELETE|RESUMABLE]"
            )
    ...
    def _ProbeObjectAccessWithClient(
        self,
        key,
        use_service_account,
        provider,
        ...
        if not use_service_account:
            try:
                ...
                try:
                    ...
                    raise CommandException(
                        "Unable to parse private key from {0}".format(self.args[0])
                    )
        else:
        ...
        for url in storage_urls:
            ...
            if url.IsBucket():
                if region == _AUTO_DETECT_REGION:
                    raise CommandException(
                        "Generating signed URLs for creating buckets"
                ...
                if method == "RESUMABLE":
                    raise CommandException(
                        "Resumable signed URLs require an object " "name."
                    )
            else:
                ...
                gcs_path = "{0}/{1}".format(
                    url.bucket_name,
                    urllib.parse.quote(
                        url.object_name.encode(constants.UTF8), safe=b"/~"
                    ),
                )
                ...
                if url.bucket_name in region_cache:
                    ...
                    try:
                        _, bucket = self.GetSingleBucketUrlFromArg(
                            "gs://{}".format(url.bucket_name),
                            bucket_fields=["location"],
                        )
                    except Exception as e:
                        raise CommandException(
                            "{}: Failed to auto-detect location for bucket '{}'. Please "
                            "ensure you have storage.buckets.get permission on the bucket "
                            "or specify the bucket's location using the '-r' option.".format(
                                e.__class__.__name__, url.bucket_name
                            )
                        )
                ...
                if url.IsBucket() and method != "PUT":
                    ...
                    if method != "PUT" and method != "RESUMABLE":
                        raise CommandException(
                            ...
                            "with that name before a creating signed URL to access it.".format(
                                url
                            )
                        )
