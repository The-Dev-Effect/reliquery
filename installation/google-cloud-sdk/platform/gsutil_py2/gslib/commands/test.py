...
TestProcessData = namedtuple("TestProcessData", "name return_code stdout stderr")
...
def TestCaseToName(test_case):
    ...
    return str(test_case.__class__).split("'")[1] + "." + test_case._testMethodName
...
def CreateTestProcesses(
    ...
    while CountFalseInList(process_done) < max_parallel_tests and test_index < len(
        parallel_tests
    ):
        env = os.environ.copy()
        ...
        if time.time() - last_log_time > 5:
            print(
                (
                    "Created %d new processes (total %d/%d created)"
                    % (
    ...
    if test_index == len(parallel_tests):
        print(
            (
                "Test process creation finished (%d/%d created)"
...
class TestCommand(Command):
    ...
    help_spec = Command.HelpSpec(
        ...
        help_one_line_summary=("Run gsutil unit/integration tests (for developers)"),
        help_text=_DETAILED_HELP_TEXT,
        subcommand_help_text={},
    )

    def RunParallelTests(
        self, parallel_integration_tests, max_parallel_tests, coverage_filename
    ):
            ...
            if len(process_results) < num_parallel_tests:
                if time.time() - last_log_time > 5:
                    ...
                    if progress_less_logging_cycles > 4:
                        ...
                        print(
                            (
                                "{sec} seconds elapsed since beginning parallel tests.\n"
                                "Still running: {procs}"
                            ).format(
                                sec=str(int(elapsed)),
                                procs=still_running,
    ...
    def PrintTestResults(
        ...
        print(
            "Parallel tests complete. Success: %s Fail: %s"
    ...
    def RunCommand(self):
        ...
        logging.debug(
            "Parallel integration tests to run: %s", parallel_integration_tests
        )
        ...
        if perform_coverage:
            ...
            print(
                (
                    "Coverage information was saved to: %s"
                    % coverage_controller.data_files.filename
                )
            )
