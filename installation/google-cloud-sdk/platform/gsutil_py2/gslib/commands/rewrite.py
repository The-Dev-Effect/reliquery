...
class RewriteCommand(Command):
    ...
    def RunCommand(self):
        ...
        if self.read_args_from_stdin:
            ...
            if not self.args:
                raise CommandException(
                    "The rewrite command (without -I) expects at " "least one URL."
                )
            url_strs = self.args
    ...
    def RewriteFunc(self, name_expansion_result, thread_state=None):
        ...
        if (
            src_metadata.customerEncryption
        ...
        if (
            self.boto_file_encryption_keywrapper is not None
            and self.boto_file_encryption_keywrapper.crypto_type == CryptoKeyType.CMEK
        ):
        ...
        if (
            self.boto_file_encryption_keywrapper is not None
            and self.boto_file_encryption_keywrapper.crypto_type == CryptoKeyType.CSEK
        ):
        ...
        encryption_unchanged = (
            src_encryption_sha256 == dest_encryption_sha256
            and src_encryption_kms_key == dest_encryption_kms_key
        )

        # Prevent accidental key rotation.
        if (
            _TransformTypes.CRYPTO_KEY not in self.transform_types
        ...
        if (
            _TransformTypes.STORAGE_CLASS in self.transform_types
            and self.dest_storage_class
            == NormalizeStorageClass(src_metadata.storageClass)
        ):
        ...
        # the object.
        if (
            _TransformTypes.CRYPTO_KEY in self.transform_types
        ...
        if src_encryption_sha256 is not None:
            if src_encryption_sha256 in self.csek_hash_to_keywrapper:
                decryption_keywrapper = self.csek_hash_to_keywrapper[
