...
_CANONICAL_REQUEST_FORMAT = (
    "{method}\n{resource}\n{query_string}\n{headers}"
    "\n{signed_headers}\n{hashed_payload}"
)
_SIGNING_ALGO = "GOOG4-RSA-SHA256"
_STRING_TO_SIGN_FORMAT = (
    "{signing_algo}\n{request_time}\n{credential_scope}" "\n{hashed_request}"
)
...
def CreatePayload(
    ...
    hashed_canonical_request = (
        base64.b16encode(canonical_request_hasher.digest()).lower().decode(UTF8)
    )
    ...
    if string_to_sign_debug and logger:
        ...
        logger.debug(
            "String to sign (ignore opening/closing brackets): [[[%s]]]"
            % string_to_sign
        )
...
def GetFinalUrl(raw_signature, host, path, canonical_query_string):
    ...
    return _SIGNED_URL_FORMAT.format(
        host=host, path=path, sig=signature, query_string=canonical_query_string
    )
