...
def PrintFullInfoAboutObject(bucket_listing_ref, incl_acl=True):
    ...
    if obj.timeStorageClassUpdated and obj.timeStorageClassUpdated != obj.timeCreated:
        text_util.print_to_fd(
            MakeMetadataLine(
    ...
    if obj.timeDeleted:
        text_util.print_to_fd(
            MakeMetadataLine(
                "Noncurrent time", obj.timeDeleted.strftime("%a, %d %b %Y %H:%M:%S GMT")
            )
        )
    marker_props = {}
    ...
    if obj.customerEncryption:
        ...
        text_util.print_to_fd(
            MakeMetadataLine("Encryption key SHA256", obj.customerEncryption.keySha256)
        )
    if obj.crc32c:
...
class LsHelper(object):
    ...
    def ExpandUrlAndPrint(self, url):
        ...
        if url.IsBucket() or self.should_recurse:
            ...
            top_level_iterator = PluralityCheckableIterator(
                self._iterator_func(
                    iteration_url, all_versions=self.all_versions
                ).IterAll(
    ...
    def _RecurseExpandUrlAndPrint(self, url_str, print_initial_newline=True):
        ...
        for blr in self._iterator_func(
            "%s" % url_str, all_versions=self.all_versions
        ).IterAll(
            expand_top_level_buckets=True,
            bucket_listing_fields=self.bucket_listing_fields,
        ):
