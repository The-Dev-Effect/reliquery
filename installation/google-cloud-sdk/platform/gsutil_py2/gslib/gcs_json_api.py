...
NotificationAdditionalProperty = NotificationCustomAttributesValue.AdditionalProperty
...
TRANSLATABLE_APITOOLS_EXCEPTIONS = (
...
class GcsJsonApi(CloudApi):
    ...
    def __init__(
        ...
        if (
            ...
            and context_config.get_context_config().use_client_certificate
        ):
            if gs_json_host:
        ...
        self.global_params = (
            apitools_messages.StandardQueryParameters(trace="token:%s" % trace_token)
            if trace_token
            else None
        )
        ...
        self.api_client.retry_func = LogAndHandleRetries(status_queue=self.status_queue)
        ...
        if isinstance(self.credentials, NoOpCredentials):
            ...
            self.api_client.AddGlobalParam(
                "key", "AIzaSyDnacJHrKma0048b13sh8cgxNUwulubmJM"
            )
    ...
    def GetServiceAccountId(self):
        ...
        if isinstance(self.credentials, ImpersonationCredentials):
            ...
            raise CommandException(
                "Cannot get service account email id for the given " "credential type."
            )
    ...
    def SignUrl(
        ...
        return GetFinalUrl(
            raw_signature, signed_headers["host"], path, canonical_query_string
        )

    def GetBucket(self, bucket_name, provider=None, fields=None):
        """See CloudApi class for function doc strings."""
        projection = (
            apitools_messages.StorageBucketsGetRequest.ProjectionValueValuesEnum.noAcl
        )
        if self._FieldsContainsAclField(fields):
            projection = (
                apitools_messages.StorageBucketsGetRequest.ProjectionValueValuesEnum.full
            )
        apitools_request = apitools_messages.StorageBucketsGetRequest(
            bucket=bucket_name, projection=projection, userProject=self.user_project
        )
        global_params = apitools_messages.StandardQueryParameters()
        if fields:
            global_params.fields = ",".join(set(fields))
    ...
    def PatchBucket(
        ...
        """See CloudApi class for function doc strings."""
        projection = (
            apitools_messages.StorageBucketsPatchRequest.ProjectionValueValuesEnum.noAcl
        )
        if self._FieldsContainsAclField(fields):
            projection = (
                apitools_messages.StorageBucketsPatchRequest.ProjectionValueValuesEnum.full
            )
        ...
        if (
            ...
            and bucket_metadata.defaultObjectAcl[0] == PRIVATE_DEFAULT_OBJ_ACL
        ):
            bucket_metadata.defaultObjectAcl = []
        ...
        if canned_acl:
            ...
            predefined_acl = apitools_messages.StorageBucketsPatchRequest.PredefinedAclValueValuesEnum(
        ...
        if canned_def_acl:
            ...
            apitools_include_fields.append("defaultObjectAcl")
            predefined_def_acl = apitools_messages.StorageBucketsPatchRequest.PredefinedDefaultObjectAclValueValuesEnum(
    ...
    def LockRetentionPolicy(self, bucket_name, metageneration, provider=None):
        ...
        apitools_request = apitools_messages.StorageBucketsLockRetentionPolicyRequest(
            bucket=bucket_name,
    ...
    def CreateBucket(
        ...
        """See CloudApi class for function doc strings."""
        projection = (
            apitools_messages.StorageBucketsInsertRequest.ProjectionValueValuesEnum.noAcl
        )
        if self._FieldsContainsAclField(fields):
            projection = (
                apitools_messages.StorageBucketsInsertRequest.ProjectionValueValuesEnum.full
            )
        if not metadata:
    ...
    def ListBuckets(self, project_id=None, provider=None, fields=None):
        """See CloudApi class for function doc strings."""
        projection = (
            apitools_messages.StorageBucketsListRequest.ProjectionValueValuesEnum.noAcl
        )
        if self._FieldsContainsAclField(fields):
            projection = (
                apitools_messages.StorageBucketsListRequest.ProjectionValueValuesEnum.full
            )
        project_id = PopulateProjectId(project_id)
    ...
    def ListObjects(
        ...
        """See CloudApi class for function doc strings."""
        projection = (
            apitools_messages.StorageObjectsListRequest.ProjectionValueValuesEnum.noAcl
        )
        if self._FieldsContainsAclField(fields):
            projection = (
                apitools_messages.StorageObjectsListRequest.ProjectionValueValuesEnum.full
            )
        ...
        if fields:
            ...
            if "nextPageToken" not in fields:
                fields.add("nextPageToken")
            if ListToGetFields(list_fields=fields).intersection(_ENCRYPTED_HASHES_SET):
        ...
        if object_list.items:
            for cloud_obj in object_list.items:
                yield CloudApi.CsObjectOrPrefix(
                    cloud_obj, CloudApi.CsObjectOrPrefixType.OBJECT
                )
        if object_list.prefixes:
            for prefix in object_list.prefixes:
                yield CloudApi.CsObjectOrPrefix(
                    prefix, CloudApi.CsObjectOrPrefixType.PREFIX
                )
    ...
    def _RewriteCryptoHeadersFromTuples(
        self, decryption_tuple=None, encryption_tuple=None
    ):
        ...
        if decryption_tuple and decryption_tuple.crypto_type == CryptoKeyType.CSEK:
            ...
            headers["x-goog-copy-source-encryption-key"] = decryption_tuple.crypto_key
            headers[
        ...
        if encryption_tuple and encryption_tuple.crypto_type == CryptoKeyType.CSEK:
            ...
            headers["x-goog-encryption-key-sha256"] = encryption_tuple.crypto_key_sha256
    ...
    def _CreateApitoolsObjectMetadataGetRequest(
        ...
        projection = (
            apitools_messages.StorageObjectsGetRequest.ProjectionValueValuesEnum.noAcl
        )
        if self._FieldsContainsAclField(fields):
            projection = (
                apitools_messages.StorageObjectsGetRequest.ProjectionValueValuesEnum.full
            )
        if generation:
            generation = long(generation)
    ...
    def _ObjectIsEncrypted(self, object_metadata):
        return bool(object_metadata.customerEncryption or object_metadata.kmsKeyName)
    ...
    def _RaiseIfNoDesiredHashesPresentForUnencryptedObject(
        ...
        md5_requested_but_missing = "md5Hash" in fields and not object_metadata.md5Hash
        crc32c_requested_but_missing = "crc32c" in fields and not object_metadata.crc32c

        # md5 not available for composite objects; in this case, don't raise an
        # exception if that was the only hash we requested.
        if (
            md5_requested_but_missing
            and not object_metadata.componentCount
            and crc32c_requested_but_missing
        ):
    ...
    def _ObjectKMSEncryptedAndNeedHashes(self, object_metadata, fields=None):
        ...
        return (
            ...
            and not object_metadata.crc32c
            and not object_metadata.md5Hash
        )
    ...
    def _ObjectCSEKEncryptedAndNeedHashes(self, object_metadata, fields=None):
        ...
        return (
            object_metadata.customerEncryption
            and not object_metadata.crc32c
            and not object_metadata.md5Hash
        )
    ...
    def GetObjectMedia(
        ...
        if end_byte:
            ...
            outer_total_size = json.loads(six.ensure_str(serialization_data))[
                "total_size"
            ]
        ...
        if progress_callback:
            if outer_total_size is None:
                raise ArgumentException(
                    "Download size is required when callbacks are "
    ...
    def _PerformDownload(
        ...
        additional_headers.update(self._EncryptionHeadersFromTuple(decryption_tuple))
    ...
    def PatchObjectMetadata(
        ...
        """See CloudApi class for function doc strings."""
        projection = (
            apitools_messages.StorageObjectsPatchRequest.ProjectionValueValuesEnum.noAcl
        )
        if self._FieldsContainsAclField(fields):
            projection = (
                apitools_messages.StorageObjectsPatchRequest.ProjectionValueValuesEnum.full
            )
        ...
        if canned_acl:
            # Must null out existing ACLs to apply a canned ACL.
            apitools_include_fields.append("acl")
            predefined_acl = apitools_messages.StorageObjectsPatchRequest.PredefinedAclValueValuesEnum(
    ...
    def _UploadObject(
        ...
        predefined_acl = None
        if canned_acl:
            predefined_acl = apitools_messages.StorageObjectsInsertRequest.PredefinedAclValueValuesEnum(
                self._ObjectCannedAclToPredefinedAcl(canned_acl)
            )
            ...
            while retries <= self.num_retries:
                try:
                    ...
                    if (
                        not gzip_encoded
                        and size
                        and not JsonResumableChunkSizeDefined()
                    ):
                    ...
                    if size is None and progress_callback:
                        ...
                        progress_callback(
                            apitools_upload.total_size, apitools_upload.total_size
                        )
                        ...
                        try:
                            ...
                            time.sleep(
                                CalculateWaitForRetry(
                                    retries, max_wait=self.max_retry_wait
                                )
                            )
                    if start_byte > last_progress_byte:
                        # We've made progress, so allow a fresh set of retries.
    ...
    def CopyObject(
        ...
        predefined_acl = None
        if canned_acl:
            predefined_acl = apitools_messages.StorageObjectsRewriteRequest.DestinationPredefinedAclValueValuesEnum(
                self._ObjectCannedAclToPredefinedAcl(canned_acl)
            )
        ...
        projection = (
            apitools_messages.StorageObjectsRewriteRequest.ProjectionValueValuesEnum.noAcl
        )
        if self._FieldsContainsAclField(fields):
            projection = (
                apitools_messages.StorageObjectsRewriteRequest.ProjectionValueValuesEnum.full
            )
        global_params = apitools_messages.StandardQueryParameters()
        if fields:
        ...
        resume_rewrite_token = ReadRewriteTrackerFile(
            tracker_file_name, rewrite_params_hash
        )
    ...
    def ComposeObject(
        ...
        with self._ApitoolsRequestHeaders(encryption_headers):
            ...
            try:
                ...
                if (
                    isinstance(e, apitools_exceptions.HttpError)
    ...
    def WatchBucket(
        ...
        """See CloudApi class for function doc strings."""
        projection = (
            apitools_messages.StorageObjectsWatchAllRequest.ProjectionValueValuesEnum.full
        )
    ...
    def ListHmacKeys(self, project_id, service_account_email, show_deleted_keys=True):
        """See CloudApi class for function doc strings."""
        try:
    ...
    def _ValidateHttpAccessTokenRefreshError(self, e):
        if isinstance(e, oauth2client.client.HttpAccessTokenRefreshError) and not (
    ...
    def _TranslateExceptionAndRaise(
        ...
        """
        if self.logger.isEnabledFor(logging.DEBUG):
            self.logger.debug("TranslateExceptionAndRaise: %s", traceback.format_exc())
    ...
    def _TranslateApitoolsResumableUploadException(self, e):
        ...
        if isinstance(e, apitools_exceptions.TransferError):
            if "Aborting transfer" in str(e) or "Not enough bytes in stream" in str(e):
    ...
    def _TranslateApitoolsException(
        self,
        e,
        bucket_name=None,
        object_name=None,
        ...
        if isinstance(e, apitools_exceptions.HttpError):
            ...
            if e.status_code == 400:
                ...
                if "The account for the specified project has been disabled" in str(e):
                    ...
                    return AccessDeniedException(
                        "Rate limit exceeded. Please retry this " "request later.",
                        status=e.status_code,
                    )
                    ...
                    return PublishPermissionDeniedException(
                        message, status=e.status_code
                    )
                else:
                    return AccessDeniedException(
                        message or str(e), status=e.status_code
                    )
            elif e.status_code == 404:
                if not_found_exception:
                    ...
                    return CreateBucketNotFoundException(
                        e.status_code, self.provider, bucket_name
                    )
            ...
            return ServiceException(
                "Transfer invalid (possible encoding error: %s)" % str(e)
            )
