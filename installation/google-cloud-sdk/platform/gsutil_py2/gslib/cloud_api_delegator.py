...
class CloudApiDelegator(CloudApi):
    ...
    def _LoadApi(self, provider, api_selector):
        ...
        self.loaded_apis[provider][api_selector] = self.api_map[
            ApiMapConstants.API_MAP
        ][provider][api_selector](
    ...
    def GetApiSelector(self, provider=None):
        ...
        if (
            selected_provider not in self.api_map[ApiMapConstants.DEFAULT_MAP]
        ...
        if selected_provider not in self.api_map[ApiMapConstants.SUPPORT_MAP]:
            raise ArgumentException(
                "No supported apis available for provider %s" % selected_provider
            )
        ...
        configured_encryption = provider == "gs" and (
            config.has_option("GSUtil", "encryption_key")
        ...
        if (
            api == ApiSelector.XML
            and context_config.get_context_config()
    ...
    def ListBuckets(self, project_id=None, provider=None, fields=None):
        return self._GetApi(provider).ListBuckets(project_id=project_id, fields=fields)
    ...
    def DeleteObject(
        ...
        return self._GetApi(provider).DeleteObject(
            bucket_name, object_name, preconditions=preconditions, generation=generation
        )
    ...
    def ListObjectAccessControls(self, bucket_name, object_name, provider=None):
        return self._GetApi(provider).ListObjectAccessControls(bucket_name, object_name)

    def CreateHmacKey(self, project_id, service_account_email, provider=None):
        return self._GetApi(provider).CreateHmacKey(project_id, service_account_email)
    ...
    def ListHmacKeys(
        self, project_id, service_account_email, show_deleted_keys=False, provider=None
    ):
    ...
    def UpdateHmacKey(self, project_id, access_id, state, etag, provider=None):
        return self._GetApi(provider).UpdateHmacKey(project_id, access_id, state, etag)
    ...
    def XmlPassThroughSetAcl(
        self, acl_text, storage_url, canned=True, def_obj_acl=False, provider=None
    ):
    ...
    def XmlPassThroughSetLifecycle(self, lifecycle_text, storage_url, provider=None):
        ...
        Returns:
          None.
        """
        self._GetApi(provider).XmlPassThroughSetLifecycle(lifecycle_text, storage_url)
