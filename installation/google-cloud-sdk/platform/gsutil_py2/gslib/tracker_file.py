...
def GetRewriteTrackerFilePath(
    src_bucket_name, src_obj_name, dst_bucket_name, dst_obj_name, api_selector
):
    ...
    res_tracker_file_name = re.sub(
        "[/\\\\]",
        "_",
        "rewrite__%s__%s__%s__%s__%s.token"
        % (src_bucket_name, src_obj_name, dst_bucket_name, dst_obj_name, api_selector),
    )
...
def GetTrackerFilePath(
    dst_url, tracker_file_type, api_selector, src_url=None, component_num=None
):
    """Gets the tracker file name described by the arguments.
    ...
    if tracker_file_type == TrackerFileType.UPLOAD:
        ...
        res_tracker_file_name = re.sub(
            "[/\\\\]",
            "_",
            "parallel_upload__%s__%s__%s__%s.url"
...
def _HashAndReturnPath(res_tracker_file_name, tracker_file_type):
    ...
    tracker_file_name = "%s_%s" % (
        str(tracker_file_type).lower(),
...
def HashRewriteParameters(
    ...
    """
    if (
        not src_obj_metadata
        or not src_obj_metadata.bucket
        or not src_obj_metadata.name
        or not src_obj_metadata.etag
        or not dst_obj_metadata
        or not dst_obj_metadata.bucket
        or not dst_obj_metadata.name
        or not projection
    ):
        return
    md5_hash = GetMd5()
...
def ReadOrCreateDownloadTrackerFile(
    src_obj_metadata,
    dst_url,
    logger,
    api_selector,
    ...
    try:
        ...
        if tracker_file_type is TrackerFileType.DOWNLOAD:
            ...
            if (
                ...
                and component_data["generation"] == src_obj_metadata.generation
            ):
                return tracker_file_name, component_data["download_start_byte"]

        logger.warn(
            "Tracker file doesn't match for download of %s. Restarting "
        ...
        if isinstance(e, ValueError) or e.errno != errno.ENOENT:
            logger.warn(
                "Couldn't read download tracker file (%s): %s. Restarting "
    ...
    if tracker_file_type is TrackerFileType.DOWNLOAD:
        ...
        WriteDownloadComponentTrackerFile(
            tracker_file_name, src_obj_metadata, start_byte
        )
    return tracker_file_name, start_byte


def GetDownloadStartByte(
    src_obj_metadata,
    dst_url,
    api_selector,
    start_byte,
...
def WriteDownloadComponentTrackerFile(
    tracker_file_name, src_obj_metadata, current_file_pos
):
...
def _WriteTrackerFile(tracker_file_name, data):
    ...
    try:
        fd = os.open(tracker_file_name, os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o600)
...
def RaiseUnwritableTrackerFileException(tracker_file_name, error_str):
    ...
    raise CommandException(
        TRACKER_FILE_UNWRITABLE_EXCEPTION_TEXT % (tracker_file_name, error_str)
    )
