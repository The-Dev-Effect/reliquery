...
def _GetTaskEstimationThreshold():
    return boto.config.getint(
        "GSUtil", "task_estimation_threshold", DEFAULT_TASK_ESTIMATION_THRESHOLD
    )
...
class Command(HelpProvider):
    ...
    _commands_with_subcommands_and_subopts = (
        "acl",
        "defacl",
        "iam",
        "kms",
    ...
    def __init__(
        ...
        if self.command_spec is None:
            raise CommandException(
                '"%s" command implementation is missing a '
        ...
        if (
            len(self.args) < self.command_spec.min_args
        ...
        support_map = {"gs": self.command_spec.gs_api_support, "s3": [ApiSelector.XML]}
        default_map = {"gs": self.command_spec.gs_default_api, "s3": ApiSelector.XML}
    ...
    def RaiseWrongNumberOfArgumentsException(self):
        ...
        if len(self.args) < self.command_spec.min_args:
            ...
            message = "The %s command requires at least %d argument%s." % (
                self.command_name,
                self.command_spec.min_args,
                tail_str,
            )
        else:
            message = "The %s command accepts at most %d arguments." % (
                self.command_name,
                self.command_spec.max_args,
            )
        message += " Usage:\n%s\nFor additional help run:\n  gsutil help %s" % (
            self.command_spec.usage_synopsis,
            self.command_name,
        )
    ...
    def CheckArguments(self):
        ...
        if not self.command_spec.file_url_ok and HaveFileUrls(
            self.args[self.command_spec.urls_start_arg :]
        ):
            raise CommandException(
                '"%s" command does not support "file://" URLs. '
                "Did you mean to use a gs:// URL?" % self.command_name
            )
        if not self.command_spec.provider_url_ok and HaveProviderUrls(
            self.args[self.command_spec.urls_start_arg :]
        ):
            raise CommandException(
    ...
    def ApplyAclFunc(self, acl_func, acl_excep_handler, url_strs, object_fields=None):
    ...
    def SetAclFunc(self, name_expansion_result, thread_state=None):
        ...
        if (
            gsutil_api.GetApiSelector(url.scheme) == ApiSelector.XML
            and url.scheme != "gs"
        ):
    ...
    def GetAndPrintAcl(self, url_str):
        ...
        url = StorageUrlFromString(url_str)
        if (
            self.gsutil_api.GetApiSelector(url.scheme) == ApiSelector.XML
            and url.scheme != "gs"
        ):
    ...
    def GetAclCommandBucketListingReference(self, url_str):
        ...
        if wildcard_url.IsObject():
            ...
            if self.command_name == "defacl":
                ...
                bucket_fields = ["acl"]
            plurality_iter = PluralityCheckableIterator(
                self.WildcardIterator(url_str).IterBuckets(bucket_fields=bucket_fields)
            )
    ...
    def GetBucketUrlIterFromArg(self, arg, bucket_fields=None):
        ...
        if not arg_url.IsCloudUrl() or arg_url.IsObject():
            raise CommandException(
                '"%s" command must specify a bucket' % self.command_name
            )
    ...
    def _GetProcessAndThreadCount(
        self, process_count, thread_count, parallel_operations_override
    ):
        ...
        if self.parallel_operations or parallel_operations_override:
            ...
            if process_count < 1:
                raise CommandException(
                    'Invalid parallel_process_count "%d".' % process_count
                )
            if not thread_count:
            ...
            if thread_count < 1:
                raise CommandException(
                    'Invalid parallel_thread_count "%d".' % thread_count
                )
        else:
    ...
    def _CreateNewConsumerPool(self, num_processes, num_threads, status_queue):
        ...
        for _ in range(num_processes):
            ...
            p = multiprocessing_context.Process(
                ...
                args=(num_threads, num_processes, recursive_apply_level, status_queue),
            )
            p.daemon = True
            processes.append(p)
            _CryptoRandomAtFork()
    ...
    def Apply(
        ...
        is_main_thread = (
            self.recursive_apply_level == 0 and self.sequential_caller_id == -1
        )
        ...
        if (
            is_main_thread
            and not self.multiprocessing_is_available
            and process_count > 1
        ):
        ...
        usable_processes_count = (
            process_count if self.multiprocessing_is_available else 1
        )
    ...
    def _MaybeSuggestGsutilDashM(self):
        ...
        if not (
            boto.config.getint("GSUtil", "parallel_process_count", 0) == 1
            and boto.config.getint("GSUtil", "parallel_thread_count", 0) == 1
        ):
            self.logger.info(
                "\n"
                + textwrap.fill(
    ...
    def _SequentialApply(
        ...
        while True:
            ...
            if (
                sequential_call_count == OFFER_GSUTIL_M_SUGGESTION_THRESHOLD
                or sequential_call_count % OFFER_GSUTIL_M_SUGGESTION_FREQUENCY == 0
            ):
        ...
        lines_since_suggestion_last_printed = (
            sequential_call_count % OFFER_GSUTIL_M_SUGGESTION_FREQUENCY
        )
    ...
    """
        ...
        if (
            parallel_operations_override == self.ParallelOverrideReason.SLICE
        ...
        if not IS_WINDOWS and is_main_thread:
            ...
            for signal_num in (signal.SIGINT, signal.SIGTERM):
                RegisterSignalHandler(
                    signal_num, MultithreadedMainSignalHandler, is_final_handler=True
                )
            ...
            try:
                ...
                if self.recursive_apply_level >= current_max_recursive_level.GetValue():
                    with need_pool_or_done_cond:
                        ...
                        if is_main_thread:
                            self._CreateNewConsumerPool(
                                process_count, thread_count, glob_status_queue
                            )
                        else:
        ...
        while True:
            with need_pool_or_done_cond:
                if call_completed_map[caller_id]:
                    ...
                    process_count > 1 and is_main_thread and new_pool_needed.GetValue()
                ):
    ...
    def _ApplyThreads(
        self, thread_count, process_count, recursive_apply_level, status_queue
    ):
...
def _StartSeekAheadThread(seek_ahead_iterator, seek_ahead_thread_cancel_event):
    ...
    # pylint: enable=global-variable-not-assigned
    # pylint: enable=global-variable-undefined
    return SeekAheadThread(
        seek_ahead_iterator, seek_ahead_thread_cancel_event, glob_status_queue
    )
        ...
        try:
                ...
                if self.arg_checker(self.cls, args):
                    ...
                    if self.status_queue:
                        if not num_tasks % 100:
                            ...
                            if (
                                isinstance(args, NameExpansionResult)
                                or isinstance(args, CopyObjectInfo)
                                or isinstance(args, RsyncDiffToApply)
                            ):
                                PutToQueueWithTimeout(
                                    self.status_queue,
                                    ProducerThreadMessage(
                                        num_tasks, total_size, time.time()
                                    ),
                                )
                        if isinstance(args, NameExpansionResult) or isinstance(
                            args, CopyObjectInfo
                        ):
                    ...
                    if not seek_ahead_thread_considered:
                        ...
                        if task_estimation_threshold <= 0:
                            ...
                            if self.seek_ahead_iterator:
                                ...
                                if boto.config.get(
                                    "GSUtil", "task_estimation_force", None
                                ):
                                    seek_ahead_thread.join(
                                        timeout=SEEK_AHEAD_JOIN_TIMEOUT
                                    )
            ...
            if self.status_queue and (
                ...
                PutToQueueWithTimeout(
                    self.status_queue,
                    ProducerThreadMessage(
                        num_tasks, total_size, time.time(), finished=True
                    ),
                )
    ...
    def PerformTask(self, task, cls):
        ...
        try:
            ...
            if task.should_return_results:
                global_return_values_map.Increment(
                    caller_id, [results], default_value=[]
                )
        except Exception as e:  # pylint: disable=broad-except
            _IncrementFailureCount()
...
def _AggregateThreadStats():
    ...
    LogPerformanceSummaryParams(
        thread_idle_time=total_idle_time, thread_execution_time=total_execution_time
    )
