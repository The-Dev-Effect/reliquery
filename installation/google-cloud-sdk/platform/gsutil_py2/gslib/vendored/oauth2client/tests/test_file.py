...
class OAuth2ClientFileTests(unittest.TestCase):
    ...
    def _create_test_credentials(self, client_id="some_client_id", expiration=None):
        access_token = "foo"
    ...
    def test_non_existent_file_storage(self, warn_mock):
        ...
        warn_mock.assert_called_with(_helpers._MISSING_FILE_MESSAGE.format(FILENAME))
    ...
    def test_token_refresh_store_expires_soon(self):
        ...
        http = http_mock.HttpMockSequence(
            [
                ({"status": http_client.UNAUTHORIZED}, b"Initial token expired"),
                ({"status": http_client.UNAUTHORIZED}, b"Store token expired"),
                (
                    {"status": http_client.OK},
                    json.dumps(token_response).encode("utf-8"),
                ),
                ({"status": http_client.OK}, b"Valid response to original request"),
            ]
        )
