...
NONEXISTENT_FILE = os.path.join(os.path.dirname(__file__), "afilethatisntthere.json")
...
class Test__validate_clientsecrets(unittest.TestCase):
    ...
    def test_missing_required_type_web(self):
        required = clientsecrets.VALID_CLIENT[clientsecrets.TYPE_WEB]["required"]
    ...
    def test_string_not_configured_type_web(self):
        string_props = clientsecrets.VALID_CLIENT[clientsecrets.TYPE_WEB]["string"]
    ...
    def test_missing_required_type_installed(self):
        required = clientsecrets.VALID_CLIENT[clientsecrets.TYPE_INSTALLED]["required"]
        # We will certainly have less than all 5 keys.
        self.assertEqual(len(required), 5)
    ...
    def test_string_not_configured_type_installed(self):
        string_props = clientsecrets.VALID_CLIENT[clientsecrets.TYPE_INSTALLED][
            "string"
        ]
