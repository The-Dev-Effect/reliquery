    ...
    def test_it(self):
        ...
        with mock.patch("oauth2client.contrib.xsrfutil.hmac") as hmac:
            ...
            token = xsrfutil.generate_token(
                TEST_KEY, TEST_USER_ID_1, action_id=TEST_ACTION_ID_1, when=TEST_TIME
            )
            ...
            expected_token_as_bytes = (
                digest + xsrfutil.DELIMITER + _helpers._to_bytes(str(TEST_TIME))
            )
    ...
    def test_with_system_time(self):
        ...
        with mock.patch("oauth2client.contrib.xsrfutil.hmac") as hmac:
            ...
            with mock.patch("oauth2client.contrib.xsrfutil.time") as time:
                ...
                # when= is omitted
                token = xsrfutil.generate_token(
                    TEST_KEY, TEST_USER_ID_1, action_id=TEST_ACTION_ID_1
                )
...
class Test_validate_token(unittest.TestCase):
    ...
    def test_token_too_old_explicit_current_time(self):
        ...
        token = base64.b64encode(_helpers._to_bytes(str(token_time)))
        self.assertFalse(
            xsrfutil.validate_token(key, token, user_id, current_time=curr_time)
        )
    ...
    def test_token_length_differs_from_generated(self):
        ...
        with mock.patch(
            "oauth2client.contrib.xsrfutil.generate_token", return_value=generated_token
        ) as gen_tok:
            self.assertFalse(
                xsrfutil.validate_token(
                    key, token, user_id, current_time=curr_time, action_id=action_id
                )
            )
            gen_tok.assert_called_once_with(
    ...
    def test_success(self):
        ...
        token = base64.b64encode(_helpers._to_bytes(str(token_time)))
        with mock.patch(
            "oauth2client.contrib.xsrfutil.generate_token", return_value=token
        ) as gen_tok:
            self.assertTrue(
                xsrfutil.validate_token(
                    key, token, user_id, current_time=curr_time, action_id=action_id
...
class XsrfUtilTests(unittest.TestCase):
    ...
    def testGenerateAndValidateToken(self):
        ...
        self.assertFalse(
            xsrfutil.validate_token(
                TEST_KEY, None, TEST_USER_ID_1, action_id=TEST_ACTION_ID_1
            )
        )
