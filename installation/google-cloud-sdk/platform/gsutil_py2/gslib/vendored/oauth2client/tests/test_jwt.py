...
class CryptTests(unittest.TestCase):
    ...
    def _check_sign_and_verify(self, private_key_file):
        ...
        signer = self.signer.from_string(private_key, password=u"notasecret")
    ...
    def test_verify_id_token_with_certs_uri_fails(self):
        ...
        http = http_mock.HttpMock(
            headers={"status": http_client.NOT_FOUND}, data=datafile("certs.json")
        )
    ...
    def test_verify_id_token_bad_tokens(self):
        ...
        # Bad signature
        jwt = b".".join([b"foo", _helpers._urlsafe_b64encode('{"a":"b"}'), b"baz"])
...
class SignedJwtAssertionCredentialsTests(unittest.TestCase):
    ...
    def _make_credentials(self):
        ...
        if self.format_ == "pem":
            ...
            credentials._private_key_password = service_account._PASSWORD_DEFAULT
    ...
    def test_credentials_to_from_json(self):
        ...
        self.assertEqual(credentials._private_key_pkcs12, restored._private_key_pkcs12)
        self.assertEqual(
            credentials._private_key_password, restored._private_key_password
        )
...
class PEMSignedJwtAssertionCredentialsOpenSSLTests(SignedJwtAssertionCredentialsTests):
    def setUp(self):
        self.orig_signer = crypt.Signer
        self.format_ = "pem"
        crypt.Signer = crypt.OpenSSLSigner
...
class PEMSignedJwtAssertionCredentialsPyCryptoTests(SignedJwtAssertionCredentialsTests):
    def setUp(self):
        self.orig_signer = crypt.Signer
        self.format_ = "pem"
