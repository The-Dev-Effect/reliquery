...
class TestRsaVerifier(unittest.TestCase):
    ...
    PUBLIC_KEY_FILENAME = os.path.join(
        os.path.dirname(__file__), "data", "privatekey.pub"
    )
    ...
    def test_from_string_pub_key(self):
        ...
        verifier = crypt.RsaVerifier.from_string(public_key, is_x509_cert=False)
    ...
    def test_from_string_pub_cert(self):
        ...
        verifier = crypt.RsaVerifier.from_string(public_cert, is_x509_cert=True)
        self.assertIsInstance(verifier, crypt.RsaVerifier)
        self.assertIsInstance(verifier._pubkey, rsa.key.PublicKey)
...
class TestRsaSigner(unittest.TestCase):
    ...
    PKCS1_KEY_FILENAME = os.path.join(
        os.path.dirname(__file__), "data", "privatekey.pem"
    )
    PKCS8_KEY_FILENAME = os.path.join(
        os.path.dirname(__file__), "data", "pem_from_pkcs12.pem"
    )
    PKCS12_KEY_FILENAME = os.path.join(
        os.path.dirname(__file__), "data", "privatekey.p12"
    )
