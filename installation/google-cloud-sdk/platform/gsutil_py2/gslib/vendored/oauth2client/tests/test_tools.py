...
class TestClientRedirectServer(unittest.TestCase):
    ...
    def test_ClientRedirectServer(self):
        ...
        httpd = tools.ClientRedirectServer(
            ("localhost", 0), tools.ClientRedirectHandler
        )
        code = "foo"
        url = "http://localhost:{0}?code={1}".format(httpd.server_address[1], code)
...
class TestRunFlow(unittest.TestCase):
    def setUp(self):
        ...
        self.flow.step1_get_authorize_url.return_value = "http://example.com/auth"
    ...
    @mock.patch("oauth2client.tools.logging")
    @mock.patch("oauth2client.tools.input")
    def test_run_flow_no_webserver_explicit_flags(self, input_mock, logging_mock):
        input_mock.return_value = "auth_code"

        # Successful exchange.
        returned_credentials = tools.run_flow(self.flow, self.storage, flags=self.flags)
