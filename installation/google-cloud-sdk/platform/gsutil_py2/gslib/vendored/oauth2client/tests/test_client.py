...
class CredentialsTests(unittest.TestCase):
    ...
    @mock.patch.object(client.Credentials, "_to_json", return_value=object())
    def test_to_json(self, to_json):
        credentials = client.Credentials()
        self.assertEqual(credentials.to_json(), to_json.return_value)
        to_json.assert_called_once_with(client.Credentials.NON_SERIALIZED_MEMBERS)
...
class GoogleCredentialsTests(unittest.TestCase):
    ...
    def validate_service_account_credentials(self, credentials):
        self.assertIsInstance(credentials, service_account.ServiceAccountCredentials)
    ...
    def get_a_google_credentials_object(self):
        return client.GoogleCredentials(None, None, None, None, None, None, None, None)
    ...
    def test_create_scoped(self):
        ...
        self.assertEqual(credentials, credentials.create_scoped(["dummy_scope"]))

    @mock.patch.object(
        client.GoogleCredentials, "_implicit_credentials_from_files", return_value=None
    )
    @mock.patch.object(client.GoogleCredentials, "_implicit_credentials_from_gce")
    @mock.patch.object(client, "_in_gae_environment", return_value=True)
    @mock.patch.object(
        client, "_get_application_default_credential_GAE", return_value=object()
    )
    def test_get_application_default_in_gae(
        self, gae_adc, in_gae, from_gce, from_files
    ):
    ...
    @mock.patch.object(
        client.GoogleCredentials, "_implicit_credentials_from_gae", return_value=None
    )
    @mock.patch.object(
        client.GoogleCredentials, "_implicit_credentials_from_files", return_value=None
    )
    @mock.patch.object(client, "_in_gce_environment", return_value=True)
    @mock.patch.object(
        client, "_get_application_default_credential_GCE", return_value=object()
    )
    def test_get_application_default_in_gce(
        self, gce_adc, in_gce, from_files, from_gae
    ):
        credentials = client.GoogleCredentials.get_application_default()
    ...
    def test_environment_check_gae_production(self):
        with mock_module_import("google.appengine"):
            self._environment_check_gce_helper(server_software="Google App Engine/XYZ")

    def test_environment_check_gae_local(self):
        with mock_module_import("google.appengine"):
            self._environment_check_gce_helper(server_software="Development/XYZ")
    ...
    def _environment_check_gce_helper(self, status_ok=True, server_software=""):
        if status_ok:
    ...
    @mock.patch("oauth2client.client.os.environ", new={client._SERVER_SOFTWARE: ""})
    @mock.patch("oauth2client.transport.get_http_object", return_value=object())
    @mock.patch("oauth2client.transport.request", side_effect=socket.timeout())
    def test_environment_check_gce_timeout(self, mock_request, new_http):
        ...
        mock_request.assert_called_once_with(
            new_http.return_value, client._GCE_METADATA_URI, headers=client._GCE_HEADERS
        )
    ...
    def test_get_environment_variable_file(self):
        ...
        os.environ[client.GOOGLE_APPLICATION_CREDENTIALS] = environment_variable_file
        self.assertEqual(
            environment_variable_file, client._get_environment_variable_file()
        )
    ...
    def test_get_environment_variable_file_error(self):
        ...
        expected_err_msg = (
            ...
            "exist!".format(nonexistent_file, client.GOOGLE_APPLICATION_CREDENTIALS)
        )
        with self.assertRaisesRegexp(
            client.ApplicationDefaultCredentialsError, expected_err_msg
        ):
    ...
    def test_get_well_known_file_on_windows(self):
        well_known_file = datafile(
            os.path.join(
                client._CLOUDSDK_CONFIG_DIRECTORY, client._WELL_KNOWN_CREDENTIALS_FILE
            )
        )
    ...
    @mock.patch.dict(
        os.environ, {client._CLOUDSDK_CONFIG_ENV_VAR: "CUSTOM_DIR"}, clear=True
    )
    def test_get_well_known_file_with_custom_config_dir(self):
        CUSTOM_DIR = os.environ[client._CLOUDSDK_CONFIG_ENV_VAR]
        EXPECTED_FILE = os.path.join(CUSTOM_DIR, client._WELL_KNOWN_CREDENTIALS_FILE)
    ...
    def test_save_well_known_file_with_non_existent_config_dir(self, isdir_mock):
        credential_file = datafile(
            os.path.join("gcloud", client._WELL_KNOWN_CREDENTIALS_FILE)
        )
        credentials = client._get_application_default_credential_from_file(
    ...
    def test_get_adc_from_file_authorized_user(self):
        credentials_file = datafile(
            os.path.join(
                "gcloud", "application_default_credentials_authorized_user.json"
            )
        )
        credentials = client._get_application_default_credential_from_file(
            credentials_file
    ...
    def test_get_application_default_credential_from_malformed_file_1(self):
        credentials_file = datafile(
            os.path.join("gcloud", "application_default_credentials_malformed_1.json")
        )
        expected_err_msg = (
            "'type' field should be defined \(and have one of the '{0}' or "
            "'{1}' values\)".format(client.AUTHORIZED_USER, client.SERVICE_ACCOUNT)
        )

        with self.assertRaisesRegexp(
            client.ApplicationDefaultCredentialsError, expected_err_msg
        ):
            client._get_application_default_credential_from_file(credentials_file)

    def test_get_application_default_credential_from_malformed_file_2(self):
        credentials_file = datafile(
            os.path.join("gcloud", "application_default_credentials_malformed_2.json")
        )
        expected_err_msg = "The following field\(s\) must be defined: private_key_id"
        with self.assertRaisesRegexp(
            client.ApplicationDefaultCredentialsError, expected_err_msg
        ):
            client._get_application_default_credential_from_file(credentials_file)
    ...
    def test_get_application_default_credential_from_malformed_file_3(self):
        credentials_file = datafile(
            os.path.join("gcloud", "application_default_credentials_malformed_3.json")
        )
    ...
    def test_raise_exception_for_missing_fields(self):
        ...
        expected_err_msg = "The following field\(s\) must be defined: " + ", ".join(
            missing_fields
        )
        with self.assertRaisesRegexp(
            client.ApplicationDefaultCredentialsError, expected_err_msg
    ...
    def test_raise_exception_for_reading_json(self):
        ...
        expected_err_msg = (
            ...
            "json file: " + credential_file + extra_help + ": " + str(error)
        )
        with self.assertRaisesRegexp(
            client.ApplicationDefaultCredentialsError, expected_err_msg
        ):
            client._raise_exception_for_reading_json(credential_file, extra_help, error)
    ...
    def test_get_adc_from_env_var_malformed_file(self, *stubs):
        ...
        self.assertTrue(
            str(exc_manager.exception).startswith(
                "An error was encountered while reading json file: "
                + get_env_file.return_value
                + " (pointed to by "
    ...
    @mock.patch("oauth2client.client._in_gae_environment", return_value=False)
    @mock.patch("oauth2client.client._get_environment_variable_file", return_value=None)
    @mock.patch("oauth2client.client._get_well_known_file", return_value="BOGUS_FILE")
    ...
    def test_get_adc_env_from_well_known(self, *stubs):
        ...
        result_creds = object()
        with mock.patch(method_name, return_value=result_creds) as get_from_file:
    ...
    def test_from_stream_missing_file(self):
        ...
        expected_err_msg = (
            r"The parameter passed to the from_stream\(\) "
            r"method should point to a file."
        )
        with self.assertRaisesRegexp(
            client.ApplicationDefaultCredentialsError, expected_err_msg
        ):
            self.get_a_google_credentials_object().from_stream(credentials_filename)
...
def _token_revoke_test_helper(
    testcase, revoke_raise, valid_bool_value, token_attr, http_mock
):
    ...
    if revoke_raise:
        testcase.assertRaises(
            client.TokenRevokeError, testcase.credentials.revoke, http_mock
        )
    else:
        testcase.credentials.revoke(http_mock)

    testcase.assertEqual(
        getattr(testcase.credentials, token_attr), testcase.token_from_revoke
    )
...
class BasicCredentialsTests(unittest.TestCase):
    def setUp(self):
        ...
        self.old_positional_enforcement = _helpers.positional_parameters_enforcement
        _helpers.positional_parameters_enforcement = _helpers.POSITIONAL_EXCEPTION

    def tearDown(self):
        _helpers.positional_parameters_enforcement = self.old_positional_enforcement
    ...
    def test_token_refresh_failure(self):
        for status_code in client.REFRESH_STATUS_CODES:
            http = http_mock.HttpMockSequence(
                [
                    ({"status": status_code}, b""),
                    ({"status": http_client.BAD_REQUEST}, b'{"error":"access_denied"}'),
                ]
            )
            http = self.credentials.authorize(http)
            with self.assertRaises(client.HttpAccessTokenRefreshError) as exc_manager:
                transport.request(http, "http://example.com")
            self.assertEqual(http_client.BAD_REQUEST, exc_manager.exception.status)
    ...
    def test_unicode_header_checks(self):
        ...
        transport.request(http, u"http://example.com", method=u"GET", headers=headers)
    ...
    def test_no_unicode_in_request_params(self):
        ...
        transport.request(
            http, u"http://example.com", method=u"GET", headers={u"foo": u"bar"}
        )
        ...
        instance = client.OAuth2Credentials.from_json(self.credentials.to_json())
        self.assertEqual("foobar", instance.token_response)

    def test__expires_in_no_expiry(self):
        credentials = client.OAuth2Credentials(None, None, None, None, None, None, None)
    ...
    def test_get_access_token(self, utcnow):
        ...
        EXPIRY_TIME = datetime.datetime(1992, 12, 31, second=seconds + lifetime)
    ...
    @mock.patch.object(client.OAuth2Credentials, "_expires_in", return_value=1835)
    def test_get_access_token_without_http(self, expires_in, refresh_mock):
        ...
        with mock.patch(
            "oauth2client.transport.get_http_object", return_value=object()
        ) as new_http:
        ...
        self.assertEqual(token_info.access_token, credentials.access_token)
        self.assertEqual(token_info.expires_in, expires_in.return_value)
    ...
    def _do_refresh_request_test_helper(
        self, response, content, error_msg, logger, gen_body, gen_headers, store=None
    ):
        token_uri = "http://token_uri"
        credentials = client.OAuth2Credentials(
            None, None, None, None, None, token_uri, None
        )
    ...
    def test__do_refresh_request_basic_failure(self):
        response = http_mock.ResponseMock({"status": http_client.INTERNAL_SERVER_ERROR})
        content = u"{}"
        error_msg = "Invalid response {0}.".format(int(response.status))
        self._do_refresh_request_test_helper(response, content, error_msg)
    ...
    def test__do_refresh_request_failure_w_json_error_and_store(self):
        ...
        content = json.dumps({"error": error_msg})
        store = mock.Mock()
        self._do_refresh_request_test_helper(response, content, error_msg, store=store)

    def test__do_refresh_request_failure_w_json_error_and_desc(self):
        response = http_mock.ResponseMock({"status": http_client.SERVICE_UNAVAILABLE})
        base_error = "Ruckus"
    ...
    @mock.patch("oauth2client.client.logger")
    def _do_revoke_test_helper(self, response, content, error_msg, logger, store=None):
        credentials = client.OAuth2Credentials(
            None,
            None,
            None,
    ...
    def test__do_revoke_failure_w_json_error_and_store(self):
        ...
        content = json.dumps({"error": error_msg})
        store = mock.Mock()
        self._do_revoke_test_helper(response, content, error_msg, store=store)

    @mock.patch("oauth2client.client.logger")
    def _do_retrieve_scopes_test_helper(
        self, response, content, error_msg, logger, scopes=None
    ):
        credentials = client.OAuth2Credentials(
            None,
            None,
        ...
        if response.status == http_client.OK:
            ...
            self.assertIsNone(credentials._do_retrieve_scopes(http, token))
    ...
    def test__do_retrieve_scopes_success(self):
        ...
        self._do_retrieve_scopes_test_helper(
            response, content, None, scopes=set(["foo", "bar"])
        )
...
class AccessTokenCredentialsTests(unittest.TestCase):
    def setUp(self):
        ...
        self.credentials = client.AccessTokenCredentials(
            access_token, user_agent, revoke_uri=oauth2client.GOOGLE_REVOKE_URI
        )

    def test_token_refresh_success(self):
        for status_code in client.REFRESH_STATUS_CODES:
            http = http_mock.HttpMock(headers={"status": status_code}, data=b"")
            http = self.credentials.authorize(http)
...
class OAuth2WebServerFlowTest(unittest.TestCase):
    ...
    def test__oauth2_web_server_flow_params(self):
        ...
        params = client._oauth2_web_server_flow_params({"approval_prompt": "force"})
        self.assertEqual(params["prompt"], "consent")
        self.assertNotIn("approval_prompt", params)

        params = client._oauth2_web_server_flow_params({"approval_prompt": "other"})
        self.assertEqual(params["approval_prompt"], "other")

    @mock.patch("oauth2client.client.logger")
    def test_step1_get_authorize_url_redirect_override(self, logger):
        flow = client.OAuth2WebServerFlow(
            "client_id+1", scope="foo", redirect_uri=client.OOB_CALLBACK_URN
        )
    ...
    def test_step1_get_authorize_url_pkce(self, fake_verifier, fake_challenge):
        ...
        flow = client.OAuth2WebServerFlow(
            "client_id+1", scope="foo", redirect_uri="http://example.com", pkce=True
        )
        ...
        self.assertEqual(results["code_challenge"], self.good_challenger.decode())
    ...
    def test_step1_get_authorize_url_without_redirect(self):
        flow = client.OAuth2WebServerFlow("client_id+1", scope="foo", redirect_uri=None)
        with self.assertRaises(ValueError):
    ...
    def _step1_get_device_and_user_codes_helper(
        self, extra_headers=None, user_agent=None, default_http=False, content=None
    ):
        flow = client.OAuth2WebServerFlow("CID", scope="foo", user_agent=user_agent)
        ...
        expected = client.DeviceFlowInfo(device_code, user_code, None, ver_url, None)
    ...
    def test_step1_get_device_and_user_codes_bad_payload(self):
        ...
        with self.assertRaises(client.OAuth2DeviceCodeError):
            self._step1_get_device_and_user_codes_helper(content=non_json_content)

    def _step1_get_device_and_user_codes_fail_helper(self, status, content, error_msg):
    ...
    def test_step1_get_device_and_user_codes_non_json_failure(self):
        ...
        self._step1_get_device_and_user_codes_fail_helper(status, content, error_msg)
    ...
    def test_step1_get_device_and_user_codes_failure_w_json_error(self):
        ...
        error_msg = "Invalid response {0}. Error: {1}".format(status, base_error)
        self._step1_get_device_and_user_codes_fail_helper(status, content, error_msg)
    ...
    def test_urlencoded_exchange_failure(self):
        ...
        with self.assertRaisesRegexp(client.FlowExchangeError, "invalid_request"):
            self.flow.step2_exchange(code="some random code", http=http)
    ...
    def test_urlencoded_exchange_success(self):
        http = http_mock.HttpMock(data=b"access_token=SlAV32hkKG&expires_in=3600")

        credentials = self.flow.step2_exchange(code="some random code", http=http)
        self.assertEqual("SlAV32hkKG", credentials.access_token)
        self.assertNotEqual(None, credentials.token_expiry)
    ...
    def test_exchange_fails_if_no_code(self):
        ...
        with self.assertRaisesRegexp(client.FlowExchangeError, "shall not pass"):
    ...
    def test_exchange_id_token(self):
        ...
        payload = base64.urlsafe_b64encode(body_json).strip(b"=")
        jwt = (
            base64.urlsafe_b64encode(b"stuff")
            + b"."
            + payload
            + b"."
...
class FlowFromCachedClientsecrets(unittest.TestCase):
    ...
    def _flow_from_clientsecrets_success_helper(
        self, loadfile_mock, device_uri=None, revoke_uri=None
    ):
        ...
        if device_uri is not None:
            ...
            result = client.flow_from_clientsecrets(filename, scope, cache=cache)
    ...
    def test_flow_from_clientsecrets_invalid(self, loadfile_mock):
        ...
        with self.assertRaises(clientsecrets.InvalidClientSecretsError):
            client.flow_from_clientsecrets(filename, None, cache=cache, message=None)
        loadfile_mock.assert_called_once_with(filename, cache=cache)
    ...
    @mock.patch("sys.exit")
    def test_flow_from_clientsecrets_invalid_w_msg(self, sys_exit, loadfile_mock):
        filename = object()
        cache = object()
        message = "hi mom"

        client.flow_from_clientsecrets(filename, None, cache=cache, message=message)
    ...
    @mock.patch("sys.exit")
    def test_flow_from_clientsecrets_invalid_w_msg_and_text(
        self, sys_exit, loadfile_mock
    ):
        filename = object()
        cache = object()
        message = "hi mom"
    ...
    def test_flow_from_clientsecrets_unknown_flow(self, loadfile_mock):
        ...
        err_msg = "This OAuth 2.0 flow is unsupported: " "{0!r}".format(client_type)
        with self.assertRaisesRegexp(client.UnknownClientSecretsFlowError, err_msg):
...
class CredentialsFromCodeTests(unittest.TestCase):
    ...
    def test_exchange_code_and_file_for_token(self):
        payload = b"{" b'  "access_token":"asdfghjkl",' b'  "expires_in":3600' b"}"
        http = http_mock.HttpMock(data=payload)
...
class Test__get_application_default_credential_GAE(unittest.TestCase):
    @mock.patch.dict("sys.modules", {"oauth2client.contrib.appengine": mock.Mock()})
    def test_it(self):
...
class Test__get_application_default_credential_GCE(unittest.TestCase):
    @mock.patch.dict("sys.modules", {"oauth2client.contrib.gce": mock.Mock()})
    def test_it(self):
