...
class Oauth2AuthorizeStorageModelTest(tests_django_util.TestWithDjangoEnvironment):
    def setUp(self):
    ...
    def test_authorized_user_no_credentials_redirects(self):
        ...
        tests_models.CredentialsModel.objects.create(
            user_id=authorized_user, credentials=None
        )
    ...
    def test_already_authorized(self):
        ...
        tests_models.CredentialsModel.objects.create(
            user_id=authorized_user, credentials=credentials
        )
...
class Oauth2CallbackTest(tests_django_util.TestWithDjangoEnvironment):
    ...
    def test_callback_works(self, jsonpickle_mock):
        request = self.factory.get(
            ...
            data={"state": json.dumps(self.fake_state), "code": 123},
        )
    ...
    def test_no_session(self):
        request = self.factory.get(
            "oauth2/oauth2callback",
            data={"code": 123, "state": json.dumps(self.fake_state)},
        )
    ...
    def test_missing_state_returns_bad_request(self):
        request = self.factory.get("oauth2/oauth2callback", data={"code": 123})
    ...
    def test_bad_state(self):
        request = self.factory.get(
            "oauth2/oauth2callback",
            data={"code": 123, "state": json.dumps({"wrong": "state"})},
        )
        self.session["google_oauth2_csrf_token"] = "token"
        request.session = self.session
        response = views.oauth2_callback(request)
