...
class Test_pkcs12_key_as_pem(unittest.TestCase):
    def _make_svc_account_creds(self, private_key_file="privatekey.p12"):
        ...
        credentials = service_account.ServiceAccountCredentials.from_p12_keyfile(
            "some_account@example.com", filename, scopes="read+write"
        )
...
class Test__verify_signature(unittest.TestCase):
    def test_success_single_cert(self):
        ...
        with mock.patch("oauth2client.crypt.Verifier") as Verifier:
            Verifier.from_string = mock.Mock(name="from_string", return_value=verifier)
            result = crypt._verify_signature(message, signature, certs)
            self.assertEqual(result, None)

            # Make sure our mocks were called as expected.
            Verifier.from_string.assert_called_once_with(cert_value, is_x509_cert=True)
    ...
    def test_success_multiple_certs(self):
        ...
        verifier.verify = mock.Mock(name="verify", side_effect=[False, False, True])
        with mock.patch("oauth2client.crypt.Verifier") as Verifier:
            Verifier.from_string = mock.Mock(name="from_string", return_value=verifier)
            result = crypt._verify_signature(message, signature, certs)
            self.assertEqual(result, None)
            ...
            self.assertEqual(
                Verifier.from_string.mock_calls, expected_from_string_calls
            )
            expected_verify_calls = [mock.call(message, signature)] * 3
            self.assertEqual(verifier.verify.mock_calls, expected_verify_calls)
...
class Test__verify_time_range(unittest.TestCase):
    ...
    def test_without_issued_at(self):
        ...
        self.assertTrue(str(exception_caught).startswith("No iat field in token"))

    def test_without_expiration(self):
        payload_dict = {"iat": "iat"}
        exception_caught = self._exception_helper(payload_dict)
        self.assertNotEqual(exception_caught, None)
        self.assertTrue(str(exception_caught).startswith("No exp field in token"))
    ...
    def test_with_bad_token_lifetime(self):
        ...
        with mock.patch("oauth2client.crypt.time") as time:
            time.time = mock.Mock(name="time", return_value=current_time)

            exception_caught = self._exception_helper(payload_dict)
            self.assertNotEqual(exception_caught, None)
            self.assertTrue(
                str(exception_caught).startswith("exp field too far in future")
            )
    ...
    def test_with_issued_at_in_future(self):
        ...
        with mock.patch("oauth2client.crypt.time") as time:
            ...
            exception_caught = self._exception_helper(payload_dict)
            self.assertNotEqual(exception_caught, None)
            self.assertTrue(str(exception_caught).startswith("Token used too early"))
    ...
    def test_with_expiration_in_the_past(self):
        ...
        with mock.patch("oauth2client.crypt.time") as time:
            ...
            exception_caught = self._exception_helper(payload_dict)
            self.assertNotEqual(exception_caught, None)
            self.assertTrue(str(exception_caught).startswith("Token used too late"))
...
class Test_verify_signed_jwt_with_certs(unittest.TestCase):
    def test_jwt_no_segments(self):
        ...
        self.assertNotEqual(exception_caught, None)
        self.assertTrue(
            str(exception_caught).startswith("Wrong number of segments in token")
        )
    ...
    def test_success(self, verify_sig, verify_time, check_aud):
        certs = mock.Mock()
        cert_values = object()
        certs.values = mock.Mock(name="values", return_value=cert_values)
        audience = object()
        ...
        result = crypt.verify_signed_jwt_with_certs(jwt, certs, audience=audience)
