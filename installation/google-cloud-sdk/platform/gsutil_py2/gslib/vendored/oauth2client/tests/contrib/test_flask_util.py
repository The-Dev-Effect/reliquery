...
class FlaskOAuth2Tests(unittest.TestCase):
    def setUp(self):
        ...
        self.oauth2 = flask_util.UserOAuth2(
            self.app, client_id="client_idz", client_secret="client_secretz"
        )
    ...
    def test_explicit_configuration(self):
        ...
        with mock.patch("oauth2client.clientsecrets.loadfile", return_value=return_val):
    ...
    def test_create_flow(self):
        ...
        oauth2 = flask_util.UserOAuth2(
            app, client_id="client_id", client_secret="secret", extra_arg="test"
        )
    ...
    def test_callback_view_errors(self):
        ...
        with self.app.test_client() as client:
            ...
            self.assertEqual(response.status_code, httplib.BAD_REQUEST)
            self.assertIn(
                "&lt;script&gt;something&lt;script&gt;", response.data.decode("utf-8")
            )
        ...
        with self.app.test_client() as client:
            ...
            state = json.dumps({"csrf_token": "badstate", "return_url": "/return_url"})

            response = client.get("/oauth2callback?state={0}&code=codez".format(state))
            self.assertEqual(response.status_code, httplib.BAD_REQUEST)
        ...
        with self.app.test_client() as client:
            ...
            with mock.patch("oauth2client.transport.get_http_object") as new_http:
                ...
                new_http.return_value = http_mock.HttpMock(
                    headers={"status": httplib.INTERNAL_SERVER_ERROR}, data=DEFAULT_RESP
                )
                # Run tests.
                response = client.get(
                    "/oauth2callback?state={0}&code=codez".format(state)
        ...
        with self.app.test_client() as client:
            ...
            state = json.dumps({"csrf_token": "tokenz", "return_url": "/return_url"})
    ...
    def test_with_credentials(self):
        ...
        with self.app.test_request_context():
            ...
            self.assertEqual(
                self.oauth2.credentials.refresh_token, credentials.refresh_token
            )
    ...
    def test_incremental_auth(self):
        ...
        with self.app.test_client() as client:
            ...
            self.assertEqual(
                set(q["scope"][0].split(" ")), set(["one", "email", "two", "three"])
            )

        # Actually call two() without a redirect.
        credentials2 = self._generate_credentials(scopes=["email", "two", "three"])
    ...
    def test_incremental_auth_exchange(self):
        ...
        with mock.patch("oauth2client.transport.get_http_object") as new_http:
            ...
            with self.app.test_client() as client:
                ...
                self.assertTrue(credentials.has_scopes(["email", "one", "two"]))
    ...
    def test_refresh(self):
        ...
        with self.app.test_request_context():
            with mock.patch("flask.session"):
                ...
                self.assertEqual(self.oauth2.storage.get().access_token, token_val)
