...
class KeyringStorageTests(unittest.TestCase):
    ...
    def test_locked_get(self):
        ...
        with mock.patch.object(
            keyring, "get_password", return_value=mock_content, autospec=True
        ) as get_password:
    ...
    def test_locked_put(self):
        ...
        store = keyring_storage.Storage(service_name, user_name)
        with mock.patch.object(
            keyring, "set_password", return_value=None, autospec=True
        ) as set_password:
            ...
            set_password.assert_called_once_with(service_name, user_name, to_json_ret)
    ...
    def test_get_with_no_credentials_stored(self):
        with mock.patch.object(
            keyring, "get_password", return_value=None, autospec=True
        ) as get_password:
            store = keyring_storage.Storage("my_unit_test", "me")
    ...
    def test_get_with_malformed_json_credentials_stored(self):
        with mock.patch.object(
            keyring, "get_password", return_value="{", autospec=True
        ) as get_password:
            store = keyring_storage.Storage("my_unit_test", "me")
            credentials = store.get()
            self.assertEquals(None, credentials)
    ...
    def test_get_and_set_with_json_credentials_stored(self):
        ...
        with mock.patch.object(
            keyring, "get_password", return_value=credentials.to_json(), autospec=True
        ) as get_password:
            restored = store.get()
