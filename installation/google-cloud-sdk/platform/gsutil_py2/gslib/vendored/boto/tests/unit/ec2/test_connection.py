...
class TestCreateImage(TestEC2ConnectionBase):
   ...
   <imageId>ami-4fa54026</imageId>
    ...
    def test_minimal(self):
        ...
        self.assert_request_parameters(
            {"Action": "CreateImage", "InstanceId": "instance_id", "Name": "name"},
            ignore_params_values=[
                "AWSAccessKeyId",
                "SignatureMethod",
                "SignatureVersion",
...
class TestCopyImage(TestEC2ConnectionBase):
    ...
    def test_copy_image_client_token(self):
        self.set_http_response(status_code=200)
        copied_ami = self.ec2.copy_image(
            "us-west-2", "ami-id", client_token="client-token"
        )
        self.assertEqual(copied_ami.image_id, "ami-copied-id")
        self.assert_request_parameters(
            {
    ...
    def test_copy_image_encrypted_with_kms_key(self):
        self.set_http_response(status_code=200)
        copied_ami = self.ec2.copy_image(
            "us-west-2", "ami-id", encrypted=False, kms_key_id="kms-key"
        )
        self.assertEqual(copied_ami.image_id, "ami-copied-id")
...
class TestAccountAttributes(TestEC2ConnectionBase):
    ...
    def test_describe_account_attributes(self):
        ...
        self.assertEqual(
            parsed[0].attribute_name, "vpc-max-security-groups-per-interface"
        )
...
<DescribeNetworkInterfacesResponse xmlns="http://ec2.amazonaws.com/\
    ...
    def test_get_all_network_interfaces(self):
        self.set_http_response(status_code=200)
        result = self.ec2.get_all_network_interfaces(
            network_interface_ids=["eni-0f62d866"]
        )
        self.assert_request_parameters(
            {
...
<DescribeImagesResponse xmlns="http://ec2.amazonaws.com/doc/2013-02-01/">
    ...
    def test_get_all_images(self):
        ...
        self.assertEquals(
            "111111111111/windows2008r2-hvm-i386-20130702", parsed[0].location
        )
...
"""
    ...
    def test_modify_delete_on_termination_str(self):
        ...
        self.ec2.modify_network_interface_attribute(
            "id", "deleteOnTermination", True, attachment_id="bar"
        )
    ...
    def test_modify_delete_on_termination_bool(self):
        self.set_http_response(status_code=200)
        self.ec2.modify_network_interface_attribute(
            "id", "deleteOnTermination", "false", attachment_id="bar"
        )
    ...
    def test_modify_delete_on_termination_invalid(self):
        ...
        with self.assertRaises(ValueError):
            self.ec2.modify_network_interface_attribute(
                "id", "deleteOnTermination", 123, attachment_id="bar"
            )
...
class TestDescribeReservedInstancesModifications(TestEC2ConnectionBase):
    ...
    def test_serialized_api_args(self):
        ...
        self.assertEqual(
            response[0].modification_id, "rimod-49b9433e-fdc7-464a-a6e5-9dabcexample"
        )
        self.assertEqual(
            response[0].create_date, datetime(2013, 9, 2, 21, 20, 19, 637000)
        )
        self.assertEqual(
            response[0].update_date, datetime(2013, 9, 2, 21, 38, 24, 143000)
        )
        ...
        self.assertEqual(
            response[0].client_token, "token-f5b56c05-09b0-4d17-8d8c-c75d8a67b806"
        )
        self.assertEqual(
            response[0].reserved_instances[0].id, "2567o137-8a55-48d6-82fb-7258506bb497"
        )
        self.assertEqual(
            response[0].modification_results[0].availability_zone, "us-east-1b"
        )
...
class TestRegisterImage(TestEC2ConnectionBase):
    ...
    def test_vm_type_hvm(self):
        ...
        self.ec2.register_image(
            "name", "description", image_location="s3://foo", virtualization_type="hvm"
        )
    ...
    def test_sriov_net_support_simple(self):
        self.set_http_response(status_code=200)
        self.ec2.register_image(
            "name", "description", image_location="s3://foo", sriov_net_support="simple"
        )
...
class TestDescribeInstances(TestEC2ConnectionBase):
    ...
    def test_max_results(self):
        ...
        self.assert_request_parameters(
            {"Action": "DescribeInstances", "MaxResults": 10},
            ignore_params_values=[
                "AWSAccessKeyId",
                "SignatureMethod",
                "SignatureVersion",
...
class TestDescribeTags(TestEC2ConnectionBase):
    ...
    def test_max_results(self):
        ...
        self.assert_request_parameters(
            {"Action": "DescribeTags", "MaxResults": 10},
            ignore_params_values=[
                "AWSAccessKeyId",
                "SignatureMethod",
                "SignatureVersion",
...
class TestSignatureAlteration(TestEC2ConnectionBase):
    def test_unchanged(self):
        self.assertEqual(
            self.service_connection._required_auth_capability(), ["hmac-v4"]
        )
    ...
    def test_switched(self):
        ...
        conn = self.connection_class(
            aws_access_key_id="less", aws_secret_access_key="more", region=region
        )
...
class TestAssociateAddress(TestEC2ConnectionBase):
    ...
    def test_associate_address_object(self):
        self.set_http_response(status_code=200)
        result = self.ec2.associate_address_object(
            instance_id="i-1234", public_ip="192.0.2.1"
        )
...
class TestDescribeSnapshots(TestEC2ConnectionBase):
    ...
    def test_get_all_snapshots(self):
        self.set_http_response(status_code=200)
        result = self.ec2.get_all_snapshots(
            snapshot_ids=["snap-1a2b3c4d", "snap-5e6f7a8b"]
        )
        self.assert_request_parameters(
            {
...
class TestCreateVolume(TestEC2ConnectionBase):
    ...
    def test_create_volume(self):
        ...
        result = self.ec2.create_volume(
            80, "us-east-1e", snapshot="snap-1a2b3c4d", encrypted=True
        )
        self.assert_request_parameters(
            {
                "Action": "CreateVolume",
