...
class CFInvalidationListTest(unittest.TestCase):
    ...
    def _get_mock_responses(self, num, max_items):
        ...
        cfid_groups = list(
            self._group_iter([self._get_random_id() for i in range(num)], max_items)
        )
    ...
    def test_manual_pagination(self, num_invals=30, max_items=4):
        ...
        while ir.is_truncated:
            ir = self.cf.get_invalidation_requests(
                "dist-id-here", marker=ir.next_marker, max_items=max_items
            )
            invals = list(ir)
