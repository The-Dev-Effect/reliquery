...
class SwfL1WorkflowExecutionTest(SimpleWorkflowLayer1TestBase):
    ...
    def run_decider(self):
        ...
        tries = 0
        while True:
            dtask = self.conn.poll_for_decision_task(
                self._domain, self._task_list, reverse_order=True
            )
        ...
        r = self.conn.respond_decision_task_completed(
            dtask["taskToken"], decisions=decisions._data, execution_context=None
        )
        assert r is None
    ...
    def run_worker(self):
        ...
        tries = 0
        while True:
            atask = self.conn.poll_for_activity_task(
                self._domain, self._task_list, identity="test worker"
            )
    ...
    def test_workflow_execution(self):
        ...
        r = self.conn.start_workflow_execution(
            self._domain,
            workflow_id,
        ...
        r = self.conn.get_workflow_execution_history(
            self._domain, run_id, workflow_id, reverse_order=True
        )["events"][0]
