...
class CommandRunner(object):
    ...
    def _ConfigureCommandArgumentParserArguments(
        ...
        if isinstance(subcommands_or_arguments, list):
            ...
            error_format = (
                "subcommands_or_arguments {} should be list or dict, " "found type {}"
            )
            raise TypeError(
                error_format.format(
    ...
    def ConfigureCommandArgumentParsers(self, main_parser):
        ...
        command_to_argparse_arguments = {
            command.command_spec.command_name: command.command_spec.argparse_arguments
    ...
    def SkipUpdateCheck(self):
        ...
        if (
            ...
            or boto_util.HasUserSpecifiedGsHost()
        ):
            return True
        return False
    ...
    def MaybePromptForPythonUpdate(self, command_name):
        ...
        if (
            ...
            or self.SkipUpdateCheck()
            or command_name not in ("update", "ver", "version")
            or boto.config.getbool("GSUtil", "skip_python_update_prompt", False)
        ):
            return False
