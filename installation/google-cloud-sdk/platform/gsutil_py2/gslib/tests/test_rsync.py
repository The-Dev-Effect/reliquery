...
class TestRsyncUnit(testcase.GsUtilUnitTestCase):
    ...
    def testUrlEncodeAndDecode(self):
        ...
        self.assertEqual(
            rsync._EncodeUrl(six.ensure_text(decoded_url)), six.ensure_str(encoded_url)
        )
        # Decode accepts language-appropriate string type, returns unicode.
        self.assertEqual(
            rsync._DecodeUrl(six.ensure_str(encoded_url)), six.ensure_text(decoded_url)
        )
...
class TestRsync(testcase.GsUtilIntegrationTestCase):
    ...
    def _GetMetadataAttribute(self, bucket_name, object_name, attr_name):
        ...
        metadata = gsutil_api.GetObjectMetadata(
            bucket_name, object_name, provider=self.default_provider, fields=[attr_name]
        )
        return getattr(metadata, attr_name, None)

    def _VerifyObjectMtime(
        self, bucket_name, object_name, expected_mtime, expected_present=True
    ):
    ...
    def test_invalid_args(self):
        ...
        self.RunGsUtil(
            ["rsync", suri(bucket_uri), self.nonexistent_bucket_name], expected_status=1
        )
        # rsync object to dir.
        ...
        self.RunGsUtil(
            ["rsync", tmpdir, suri(obj1), self.nonexistent_bucket_name],
            expected_status=1,
        )
    ...
    def test_invalid_src_mtime(self):
        ...
        self.CreateObject(
            bucket_uri=bucket1_uri, object_name="obj1", contents=b"obj1", mtime="xyz"
        )
        self.CreateObject(
        ...
        self.CreateObject(
            bucket_uri=bucket1_uri, object_name="obj4", contents=b"obj4", mtime=-100
        )
        self.CreateObject(
        ...
        def _Check1():
            ...
            self.assertIn(
                "obj3 has an mtime more than 1 day from current system " "time", stderr
            )
    ...
    def test_bucket_to_bucket_preserve_posix(self):
        ...
        self.CreateObject(
            bucket_uri=src_bucket, object_name="obj2", contents=b"obj2", gid=primary_gid
        )
        self.CreateObject(
        ...
        def _Check1():
            ...
            stderr = self.RunGsUtil(
                ["rsync", "-P", suri(src_bucket), suri(dst_bucket)], return_stderr=True
            )
            ...
            self.assertEquals(
                listing1, set(["/obj1", "/obj2", "/obj3", "/obj4", "/obj5"])
            )
            # dst_bucket should have new content from src_bucket.
            self.assertEquals(
                listing2, set(["/obj1", "/obj2", "/obj3", "/obj4", "/obj5"])
            )
        ...
        self.VerifyObjectCustomAttribute(
            dst_bucket.bucket_name, "obj1", MODE_ATTR, "444"
        )
        self.VerifyObjectCustomAttribute(
            dst_bucket.bucket_name, "obj2", GID_ATTR, str(primary_gid)
        )
        self.VerifyObjectCustomAttribute(
            dst_bucket.bucket_name, "obj3", GID_ATTR, str(non_primary_gid)
        )
        ...
        self.VerifyObjectCustomAttribute(
            dst_bucket.bucket_name, "obj4", GID_ATTR, str(INVALID_GID())
        )
        self.VerifyObjectCustomAttribute(
            dst_bucket.bucket_name, "obj4", UID_ATTR, str(INVALID_UID())
        )
        self.VerifyObjectCustomAttribute(
            dst_bucket.bucket_name, "obj4", MODE_ATTR, "222"
        )
        # Verify obj5 attributes were copied.
        self.VerifyObjectCustomAttribute(
            dst_bucket.bucket_name, "obj5", UID_ATTR, str(USER_ID)
        )
        self.VerifyObjectCustomAttribute(
            dst_bucket.bucket_name, "obj5", GID_ATTR, str(primary_gid)
        )
        self.VerifyObjectCustomAttribute(
            dst_bucket.bucket_name, "obj5", MODE_ATTR, str(DEFAULT_MODE)
        )
    ...
    def test_bucket_to_bucket_same_objects_src_mtime(self):
        ...
        self.CreateObject(
            bucket_uri=dst_bucket, object_name="subdir/obj2", contents=b"subdir/obj2"
        )
    ...
    def test_bucket_to_bucket_dst_mtime(self):
        ...
        self.CreateObject(
            bucket_uri=src_bucket, object_name="subdir/obj2", contents=b"subdir/obj2"
        )
        self.CreateObject(bucket_uri=src_bucket, object_name=".obj3", contents=b".obj3")
        self.CreateObject(
            bucket_uri=src_bucket, object_name="subdir/obj4", contents=b"subdir/obj4"
        )
        self.CreateObject(
        ...
        def _Check5():
            ...
            self.RunGsUtil(
                ["rsync", "-r", "-d", "-c", suri(src_bucket), suri(dst_bucket)]
            )
            listing1 = TailSet(suri(src_bucket), self.FlatListBucket(src_bucket))
            listing2 = TailSet(suri(dst_bucket), self.FlatListBucket(dst_bucket))
            # First bucket should have un-altered content.
    ...
    def test_dir_to_bucket_mtime(self):
        ...
        self._SetObjectCustomMetadataAttribute(
            self.default_provider, bucket_uri.bucket_name, ".obj2", "test", "test"
        )
        ...
        copied_over_object_notice = (
            ...
            "don't have owner permission on the object." % suri(bucket_uri, ".obj2")
        )
    ...
    def test_dir_to_bucket_minus_d(self):
        ...
        self.CreateObject(bucket_uri=bucket_uri, object_name="obj4", contents=b"obj4")
        self.CreateObject(
            bucket_uri=bucket_uri, object_name="subdir/obj5", contents=b"subdir/obj5"
        )
        ...
        def _Check1():
            ...
            self.assertEquals(
                ".obj2",
                self.RunGsUtil(["cat", suri(bucket_uri, ".obj2")], return_stdout=True),
            )
    ...
    def test_dir_to_dir_minus_d_more_files_than_bufsize(self):
        ...
        rsync_buffer_config = [
            ("GSUtil", "rsync_buffer_lines", "50" if IS_WINDOWS else "2")
        ]
    ...
    def test_bucket_to_dir_mtime(self):
        ...
        self.CreateObject(
            bucket_uri=bucket_uri, object_name="subdir/obj3", contents=b"subdir/obj3"
        )
        self.CreateObject(bucket_uri=bucket_uri, object_name="obj4", contents=b"OBJ4")
        self.CreateObject(
        ...
        time_created = ConvertDatetimeToPOSIX(
            self._GetMetadataAttribute(bucket_uri.bucket_name, "obj10", "timeCreated")
        )
        self.CreateObject(
            bucket_uri=bucket_uri, object_name="obj11", contents=b"obj11_", mtime=75
        )
        self.CreateTempFile(
            tmpdir=tmpdir, file_name=".obj2", contents=b".OBJ2", mtime=10
        )
        ...
        def _Check2():
            ...
            self.assertEquals(long(os.path.getmtime(os.path.join(tmpdir, ".obj2"))), 5)
            self.assertEquals(long(os.path.getmtime(os.path.join(tmpdir, "obj6"))), 50)
            self.assertEquals(long(os.path.getmtime(os.path.join(tmpdir, "obj8"))), 100)
            self.assertEquals(long(os.path.getmtime(os.path.join(tmpdir, "obj9"))), 25)
    ...
    def test_bucket_to_dir_preserve_posix_errors(self):
        ...
        def _Check1():
            ...
            self.assertTrue(
                BuildErrorRegex(obj16, POSIX_INSUFFICIENT_ACCESS_ERROR).search(stderr)
            )
        ...
        self._SetObjectCustomMetadataAttribute(
            self.default_provider, bucket_uri.bucket_name, ".obj2", MODE_ATTR, "640"
        )
    ...
    def test_bucket_to_dir_preserve_posix_no_errors(self):
        ...
        self.CreateObject(
            bucket_uri=bucket_uri, object_name="obj8", contents=b"obj8", uid=USER_ID
        )
        self.CreateObject(
            bucket_uri=bucket_uri,
            object_name="obj9",
        ...
        self.VerifyLocalPOSIXPermissions(
            os.path.join(tmpdir, "obj13"), uid=USER_ID, gid=non_primary_gid, mode=0o533
        )
        self.VerifyLocalPOSIXPermissions(
    ...
    def test_bucket_to_bucket_minus_d_empty_src(self):
        ...
        def _Check1():
            self.RunGsUtil(["rsync", "-d", suri(bucket1_uri), suri(bucket2_uri)])
            stderr = self.RunGsUtil(
                ["ls", suri(bucket1_uri, "**")], expected_status=1, return_stderr=True
            )
            self.assertIn("One or more URLs matched no objects", stderr)
            stderr = self.RunGsUtil(
                ["ls", suri(bucket2_uri, "**")], expected_status=1, return_stderr=True
            )
            self.assertIn("One or more URLs matched no objects", stderr)
    ...
    def test_rsync_to_nonexistent_bucket_subdir_prefix_of_existing_obj(self):
        ...
        def _Check1():
            ...
            listing2 = TailSet(
                suri(bucket_url, "foo"),
                self.FlatListBucket(self.StorageUriCloneReplaceName(bucket_url, "foo")),
            )
            # Dir should have un-altered content.
            self.assertEquals(listing1, set(["/obj1", "/.obj2"]))
            # Bucket subdir should have content like dir.
        ...
        def _Check2():
            ...
            self.assertEquals(
                NO_CHANGES,
                self.RunGsUtil(
                    ["rsync", "-r", tmpdir, suri(bucket_url, "foo")], return_stderr=True
                ),
            )
    ...
    @unittest.skipIf(
        IS_WINDOWS, "Windows Unicode support is problematic in Python 2.x."
    )
    ...
    def test_dir_to_bucket_minus_u(self):
        ...
        self.CreateTempFile(
            tmpdir=tmpdir, file_name="obj1", contents=b"obj1-2", mtime=ORIG_MTIME - 1
        )
        # Same size, same contents.
        self.CreateTempFile(
            tmpdir=tmpdir, file_name="obj2", contents=b"obj2-1", mtime=ORIG_MTIME - 1
        )
        ...
        self.CreateTempFile(
            tmpdir=tmpdir, file_name="obj5", contents=b"obj5-bigger", mtime=ORIG_MTIME
        )

        # Source files with newer mtimes should be copied:
        # Same size and contents => SHOULD still be copied.
        self.CreateTempFile(
            tmpdir=tmpdir, file_name="obj6", contents=b"obj6-1", mtime=ORIG_MTIME + 1
        )
    ...
    def test_dir_to_bucket_minus_i(self):
        ...
        # Same size, same contents.
        self.CreateTempFile(
            tmpdir=tmpdir, file_name="obj2", contents=b"obj2-1", mtime=ORIG_MTIME
        )
        # Different size and contents.
        self.CreateTempFile(
            tmpdir=tmpdir,
    ...
    def test_gzip_transport_encoded_all_upload(self):
        ...
        # Upload the data.
        stderr = self.RunGsUtil(
            ["-D", "rsync", "-J", "-r", tmpdir, suri(bucket_uri)], return_stderr=True
        )
    ...
    def test_kms_key_specified_will_not_prevent_non_kms_copy_to_s3(self):
        ...
        bucket_uri = self.CreateBucket()
        dummy_key = (
            "projects/myproject/locations/global/keyRings/mykeyring/" "cryptoKeys/mykey"
        )
