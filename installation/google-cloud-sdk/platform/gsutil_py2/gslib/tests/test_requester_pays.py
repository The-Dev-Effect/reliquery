...
class TestRequesterPays(testcase.GsUtilIntegrationTestCase):
    ...
    def testTooFewArgumentsFails(self):
        ...
        stderr = self.RunGsUtil(self._set_rp_cmd, return_stderr=True, expected_status=1)
        self.assertIn("command requires at least", stderr)

        # No arguments for get, but valid subcommand.
        stderr = self.RunGsUtil(self._get_rp_cmd, return_stderr=True, expected_status=1)
        self.assertIn("command requires at least", stderr)

        # Neither arguments nor subcommand.
        stderr = self.RunGsUtil(
            ["requesterpays"], return_stderr=True, expected_status=1
        )
        self.assertIn("command requires at least", stderr)
    ...
    def test_acl(self):
        ...
        self._run_requester_pays_test(["acl", "get", suri(requester_pays_bucket_uri)])
    ...
    def test_compose(self):
        ...
        composite = self.StorageUriCloneReplaceName(
            bucket_uri, self.MakeTempName("obj")
        )
        self._run_non_requester_pays_test(["compose"] + components + [composite.uri])
    ...
    def test_du_obj(self):
        ...
        def _check():
            self._run_requester_pays_test(["du", suri(self.requester_pays_object_uri)])
            self._run_non_requester_pays_test(
    ...
    def test_mv(self):
        ...
        for obj in [object1_uri, object2_uri]:
            self._run_non_requester_pays_test(["mv", suri(obj), suri(dest_bucket_uri)])
        self.AssertNObjectsInBucket(bucket_uri, 0)

    def test_rewrite(self):
        object_uri = self.CreateObject(contents=b"bar")
        self._run_non_requester_pays_test(["rewrite", "-s", "dra", suri(object_uri)])
        ...
        self._run_requester_pays_test(["rewrite", "-s", "dra", suri(req_pays_obj_uri)])
    ...
    def test_stat(self):
        self._run_requester_pays_test(["stat", suri(self.requester_pays_object_uri)])
        self._run_non_requester_pays_test(
