...
class TestHmacIntegration(testcase.GsUtilIntegrationTestCase):
    ...
    def AssertKeyMetadataMatches(
        ...
        self.assertRegexpMatches(
            output_string, r"\s+Service Account:\s+%s\n" % service_account
        )
    ...
    def _CreateWithRetry(self, service_account):
        ...
        try:
            return self.RunGsUtil(
                ["hmac", "create", service_account], return_stdout=True
            )
        except AssertionError as e:
    ...
    @unittest.skipUnless(
        SERVICE_ACCOUNT, "Test requires service account configuration."
    )
    ...
    def test_create_sa_not_found(self):
        stderr = self.RunGsUtil(
            ["hmac", "create", "DNE@mail.com"], return_stderr=True, expected_status=1
        )
        self.assertIn("Service Account 'DNE@mail.com' not found.", stderr)

    @unittest.skipUnless(
        ALT_SERVICE_ACCOUNT, "Test requires service account configuration."
    )
    def test_delete(self):
        ...
        stdout = self.RunGsUtil(
            ["hmac", "list", "-u", ALT_SERVICE_ACCOUNT], return_stdout=True
        )
    ...
    def test_delete_active_key(self):
        ...
        stderr = self.RunGsUtil(
            ["hmac", "delete", access_id], return_stderr=True, expected_status=1
        )
    ...
    def test_delete_key_not_found(self):
        stderr = self.RunGsUtil(
            ["hmac", "delete", "GOOG1234DNE"], return_stderr=True, expected_status=1
        )
    ...
    def test_get_not_found(self):
        stderr = self.RunGsUtil(
            ["hmac", "get", "GOOG1234DNE"], return_stderr=True, expected_status=1
        )
    ...
    def setUpListTest(self):
        for _ in range(MAX_SA_HMAC_KEYS):
            self.RunGsUtil(
                ["hmac", "create", LIST_SERVICE_ACCOUNT], expected_status=None
            )

    @unittest.skipUnless(
        LIST_SERVICE_ACCOUNT and SERVICE_ACCOUNT,
    ...
    def test_list(self):
        ...
        stdout = self.RunGsUtil(
            ["hmac", "list", "-u", LIST_SERVICE_ACCOUNT], return_stdout=True
        )
    ...
    @unittest.skipUnless(
        LIST_SERVICE_ACCOUNT and ALT_SERVICE_ACCOUNT,
        "Test requires service account configuration.",
    )
    ...
    def test_update(self):
        ...
        try:
            ...
            stdout = self.RunGsUtil(
                ["hmac", "update", "-s", "ACTIVE", access_id], return_stdout=True
            )
            self.AssertKeyMetadataMatches(stdout, state="ACTIVE")
            stdout = self.RunGsUtil(["hmac", "get", access_id], return_stdout=True)
            self.AssertKeyMetadataMatches(stdout, state="ACTIVE")
