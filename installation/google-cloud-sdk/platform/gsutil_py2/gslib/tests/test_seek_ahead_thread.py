...
class TestSeekAheadThread(testcase.GsUtilUnitTestCase):
    ...
    def testCancellation(self):
        ...
        class TrackingCancellationIterator(object):
            ...
            def __init__(
                self, num_iterations, num_iterations_before_cancel, cancel_event
            ):
        ...
        for num_iterations, num_iterations_before_cancel, expected_iterations in (
            ...
            seek_ahead_thread = SeekAheadThread(
                seek_ahead_iterator, cancel_event, status_queue
            )
            ...
            if message:
                self.fail(
                    "Status queue should be empty but contains message: %s" % message
                )
