...
DEFAULT_CERT_PROVIDER_FILE_CONTENTS = {
    "cert_provider_command": [os.path.join("some", "helper"), "--print_certificate"]
}
...
class TestPemFileParser(testcase.GsUtilUnitTestCase):
    ...
    def test_pem_file_with_bad_format_missing_ending(self):
        sections = context_config._split_pem_into_sections(
            BAD_CERT_KEY_MISSING_END, self.logger
        )
...
class TestContextConfig(testcase.GsUtilUnitTestCase):
    ...
    def test_writes_and_deletes_certificate_file_storing_password_to_memory(
        ...
        with SetBotoConfigForTest(
            ...
            self.assertEqual(
                context_config._singleton_config.client_cert_password, PASSWORD
            )
