...
class TestUbla(testcase.GsUtilIntegrationTestCase):
    ...
    def _AssertEnabled(self, bucket_uri, value):
        stdout = self.RunGsUtil(
            self._get_ubla_cmd + [suri(bucket_uri)], return_stdout=True
        )
    ...
    def testTooFewArgumentsFails(self):
        ...
        stderr = self.RunGsUtil(
            self._set_ubla_cmd, return_stderr=True, expected_status=1
        )
        self.assertIn("command requires at least", stderr)

        # No arguments for get, but valid subcommand.
        stderr = self.RunGsUtil(
            self._get_ubla_cmd, return_stderr=True, expected_status=1
        )
        self.assertIn("command requires at least", stderr)
