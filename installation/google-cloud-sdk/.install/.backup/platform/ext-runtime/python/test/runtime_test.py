...
class RuntimeTests(testutil.TestBase):
    def setUp(self):
        ...
        self.DOCKERFILE_PREAMBLE = ROOT.read_file("data", "Dockerfile.preamble")
        self.DOCKERFILE_VIRTUALENV_TEMPLATE = ROOT.read_file(
            "data", "Dockerfile.virtualenv.template"
        )
        self.DOCKERFILE_REQUIREMENTS_TXT = ROOT.read_file(
            "data", "Dockerfile.requirements_txt"
        )
        self.DOCKERFILE_INSTALL_APP = ROOT.read_file("data", "Dockerfile.install_app")
    ...
    def test_python(self):
        ...
        self.assertEqual(
            ...
            {"requirements.txt", "app.yaml", "Dockerfile", ".dockerignore"},
        )
    ...
    def test_python_no_write(self):
        ...
        self.assertEqual(
            set(os.listdir(self.temp_path)), {"requirements.txt", "app.yaml"}
        )
        self.assertEqual(
            {f.filename for f in cfg_files}, {"Dockerfile", ".dockerignore"}
        )
    ...
    def test_python_with_app_yaml(self):
        ...
        config = testutil.AppInfoFake(
            runtime="python", entrypoint="run_me_some_python!"
        )
        ...
        self.assertEqual(
            set(os.listdir(self.temp_path)), {"test.py", "Dockerfile", ".dockerignore"}
        )
    ...
    def test_python_with_runtime_config_but_no_python_version(self):
        ...
        self.assertEqual(
            set(os.listdir(self.temp_path)), {"test.py", ".dockerignore", "Dockerfile"}
        )
    ...
    def test_python_with_explicit_python3(self):
        ...
        self.assert_file_exists_with_contents(
            "Dockerfile",
            self.DOCKERFILE_PREAMBLE
            + self.DOCKERFILE_VIRTUALENV_TEMPLATE.format(python_version="3.6")
            + self.DOCKERFILE_INSTALL_APP
            + "CMD run_me_some_python!\n",
        )
    ...
    def test_python_with_explicit_python34(self):
        ...
        self.assert_file_exists_with_contents(
            "Dockerfile",
            self.DOCKERFILE_PREAMBLE
            + self.DOCKERFILE_VIRTUALENV_TEMPLATE.format(python_version="3.4")
            + self.DOCKERFILE_INSTALL_APP
            + "CMD run_me_some_python!\n",
        )
    ...
    def test_python_with_explicit_python35(self):
        ...
        self.assert_file_exists_with_contents(
            "Dockerfile",
            self.DOCKERFILE_PREAMBLE
            + self.DOCKERFILE_VIRTUALENV_TEMPLATE.format(python_version="3.5")
            + self.DOCKERFILE_INSTALL_APP
            + "CMD run_me_some_python!\n",
        )
    ...
    def test_python_with_invalid_version(self):
        ...
        self.assertRaises(
            testutil.InvalidRuntime, self.generate_config_data, appinfo=config
        )
